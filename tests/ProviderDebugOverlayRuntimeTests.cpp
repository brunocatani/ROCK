...
int main()
{
    ...
    assert(provider_debug_overlay::publish(1, publication) ==
    ...
    publication.textCount = 1;
    assert(provider_debug_overlay::publish(2, publication) ==
           RockProviderResultV1::InvalidArgument);
    ...
    for (std::uint64_t owner = 1;
         ...
         ++owner) {
        assert(provider_debug_overlay::publish(owner, publication) ==
               RockProviderResultV1::Ok);
    }
    assert(provider_debug_overlay::publish(
               ROCK_PROVIDER_MAX_DEBUG_OVERLAY_PUBLISHERS_V1 + 1,
               publication) == RockProviderResultV1::CapacityFull);
