...
Require-Text 'data/config/ROCK.ini' 'bGrabPhysicsRateForceScalingEnabled\s*=\s*true[\s\S]*fGrabPhysicsRateReferenceHz\s*=\s*90\.0[\s\S]*fGrabPhysicsRateForceScaleExponent\s*=\s*0\.5[\s\S]*fGrabPhysicsRateMinForceScale\s*=\s*0\.75[\s\S]*fGrabPhysicsRateMaxForceScale\s*=\s*1\.35' 'Packaged ROCK.ini must expose physics-rate grab force scaling defaults.'
...
Require-Text 'data/config/ROCK.ini' 'bDebugDrawGrabSupportFrame\s*=\s*false' 'Packaged ROCK.ini must expose grab support-frame visualization disabled by default.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFingerPosePublished\s*=\s*g_rockConfig\.rockGrabMeshFingerPoseEnabled\s*&&\s*_grabAcquisitionPhase == grab_three_phase::AcquisitionPhase::TouchHeld' 'Grab commit must defer the mesh finger probe solve until TouchHeld unless capture is already touching.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraint.cpp' 'if\s*\(inertiaModified\)\s*\{[\s\S]*rebuildMotionMassProperties\(world,\s*motionIndex\)[\s\S]*packed\[0\]\s*=\s*desiredPackedInertia\[0\][\s\S]*if\s*\(savedMotionState\.massModified\)\s*\{[\s\S]*packed\[3\]\s*=\s*desiredPackedMass' 'Grab motion mass rebuild must be gated by actual inertia changes and reapply packed loose-weapon mass overrides.'
