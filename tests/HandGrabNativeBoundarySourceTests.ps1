...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'if\s*\(!looseWeaponPrimaryAttachApplied\s*&&\s*!usingPinchPocket\s*&&\s*pocket\.valid\)\s*\{[\s\S]*computeGrabSeatDepthStop\(\s*grabLocalMeshTriangles,[\s\S]*grabPivotAWorld\s*=\s*grabPivotAWorld\s*\+\s*pocket\.palmNormalWorld\s*\*\s*seatDepthOffsetGameUnits;[\s\S]*shiftObjectToAlignGripWithPocket\(\s*grabBodyWorldAtGrab,\s*grabPivotAWorld,\s*grabGripPoint\)' 'Grab capture must apply the seat depth stop by moving pivot A along the palm normal (the only translation authority at freeze), excluding weapon attach frames and pinch pockets.'
