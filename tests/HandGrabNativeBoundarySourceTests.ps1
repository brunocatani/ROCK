...
Require-Text 'data/config/ROCK.ini' 'fGrabLooseWeaponSharedConstraintAngularForceMultiplier\s*=\s*2\.0' 'Packaged ROCK.ini must expose loose non-equipped weapon angular-only force tuning.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraint.cpp' 'rebuildMotionMassProperties\(world,\s*savedMotion\.motionIndex\)[\s\S]*if\s*\(savedMotion\.massModified\)\s*\{[\s\S]*packed\[3\]\s*=\s*savedMotion\.savedPackedMass' 'Grab motion restore must reapply saved packed mass after Havok rebuilds restored inertia.'
