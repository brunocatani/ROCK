...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'reacquireSolve = solveGrabSeat\(\s*_grabFrame\.localMeshTriangles,\s*grabBodyWorld,\s*_grabFrame\.bodyLocal,\s*promotedPointWorld,\s*livePivotAWorld\);[\s\S]*seatPivotAWorld = reacquireSolve\.pivotAWorld;[\s\S]*\.pivotAWorld = seatPivotAWorld' 'Seated pivot reacquire must re-seat through the SAME pose solve as the capture path, so promotion can never undo capture-time seat quality with weaker math.'
