...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'grabAuthorityFrameWorldAtGrab\s*=\s*makeRawHandGrabAuthorityFrame\(handWorldTransform,\s*_isLeft\)[\s\S]*resolveGrabAuthorityProxyFrame\(\s*world,\s*handWorldTransform,\s*&handBodyWorldAtGrab,\s*proxyFrameWorldAtGrab,[\s\S]*grabAuthorityPivotAWorld\s*=\s*grabAuthorityFrameWorldAtGrab\.translate[\s\S]*palmPocketPivotAWorld\s*=\s*grabAuthorityFrameWorldAtGrab\.translate[\s\S]*grabPivotAForPrimaryChoice\s*=\s*palmPocketPivotAWorld' 'Close dynamic grab must use raw hand authority for palm-pocket mesh/contact acquisition while keeping the hidden proxy as body-A transport.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmPocketPivotAWorld\s*=\s*grabAuthorityFrameWorldAtGrab\.translate' 'Close dynamic grab post-prep authority must use the raw hand authority pivot for the palm-pocket point.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'authorityFrame\s*=\s*makeRawHandGrabAuthorityFrame\(handWorldTransform,\s*_isLeft\)[\s\S]*livePivotAWorld\s*=\s*authorityFrame\.translate[\s\S]*_grabAuthorityPivotAProxyLocalGame\s*=\s*frozenSeatAuthorityFrame\.pivotAHandBodyLocalGame' 'Seated palm-pocket promotion must reacquire from raw hand authority while keeping promoted pivot A as an explicit local point on the hidden proxy body.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyWorld\s*=\s*multiplyTransforms\(handWorldTransform,\s*_grabFrame\.rawBodyHandSpace\)[\s\S]*desiredObjectWorld\s*=\s*deriveNodeWorldFromBodyWorld\(desiredBodyWorld,\s*_grabFrame\.bodyLocal\)[\s\S]*queueProxyGrabAuthorityTarget\(\s*proxyAuthorityWorld,\s*handWorldTransform,\s*desiredObjectWorld,\s*desiredBodyWorld' 'Held-object targets must use raw hand authority and queue that desired object/BODY pose with the hidden proxy transport frame.'
...
if ($applyFrozenStart -lt 0 -or $applyFrozenEnd -lt 0) {
    ...
    if ($outsideApplyFrozen -match '_grabFrame\.(rawHandSpace|rawBodyHandSpace|handBodyToRawHandAtGrab|proxyAuthorityHandSpace|proxyAuthorityBodyHandSpace|bodyLocal|rootBodyLocal|ownerBodyLocal|gripPointLocal|gripPointBodyLocalGame|pivotBBodyLocalGame|pivotBConstraintLocalGame|pivotAHandBodyLocalGame|desiredObjectWorldAtGrab|desiredBodyWorldAtGrab|hasFrozenPivotB|hasGripPoint)\s*=') {
...
Require-Text 'src/physics-interaction/debug/DebugBodyOverlay.cpp' 'targetBodyOverlayFrameSource[\s\S]*BodyOverlayRole::Target[\s\S]*BodyOverlayFrameSource::BodyArrayTransform[\s\S]*extractBody\(frame\.world,\s*entry\.bodyId,\s*targetBodyOverlayFrameSource\(entry\.role\)' 'Target collider visualizer must draw selected/held object colliders from the same hknp BODY frame used by grab body-B authority, not the generic MOTION/COM frame.'
Require-Text 'src/physics-interaction/debug/DebugBodyOverlay.cpp' 'targetAxisOverlayFrameSource[\s\S]*AxisOverlayRole::TargetBody[\s\S]*BodyOverlayFrameSource::BodyArrayTransform[\s\S]*extractBody\(world,\s*entry\.bodyId,\s*targetAxisOverlayFrameSource\(entry\.role\)' 'Target body axes in the visualizer must also use the hknp BODY frame so the drawn orientation matches body-B authority.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'publishPullCatchAcquisitionPose[\s\S]*grabbedFromPullCatch[\s\S]*_grabObjectGripAtGrab\.valid[\s\S]*_grabFingerPosePublished\s*=[\s\S]*g_rockConfig\.rockGrabMeshFingerPoseEnabled[\s\S]*_grabAcquisitionPhase == grab_three_phase::AcquisitionPhase::TouchHeld \|\| publishPullCatchAcquisitionPose' 'Grab commit must defer ordinary mesh finger probe solve until TouchHeld while allowing the explicit pull-catch acquisition pose exception.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'publishPullCatchAcquisitionPose[\s\S]*fingerPoseObjectWorld\s*=[\s\S]*desiredObjectWorld[\s\S]*rebuildFingerPoseWorldTrianglesFromGrabFrame\(_grabFrame,\s*fingerPoseObjectWorld\)[\s\S]*pull-catch acquisition pose published from desired relation' 'Pull-catch pre-touch finger pose must solve from the frozen desired object relation, not the still-converging live body.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'mesh probe solve deferred until TouchHeld' 'Near/gravity acquisition must log that finger probes are deferred instead of firing pre-touch.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'rebuildFingerPoseWorldTrianglesFromGrabFrame\(_grabFrame,\s*currentNodeWorld\)[\s\S]*_grabFingerPose\s*=\s*grab_finger_pose_runtime::solveGrabFingerPoseFromTriangles' 'TouchHeld transition must fire finger probes from cached local mesh evidence after the final seated relation is known.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'captureSurfaceAimObjectLocal\(_grabFingerPose,\s*currentNodeWorld\)' 'TouchHeld finger solve must freeze mesh-solved surface aim points in object-local space.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'kMaxGrabRuntimeFingerPoseTriangles[\s\S]*selectNearestGrabFingerPoseTriangles[\s\S]*fingerPoseLocalMeshTriangles[\s\S]*rebuildFingerPoseWorldTrianglesFromGrabFrame' 'Held finger pose must use a bounded local mesh cache while keeping the full grab mesh available for authority and lever diagnostics.'
...
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'struct PullCatchSeatSafetyInput[\s\S]*grabbedFromPullCatch[\s\S]*usingPinchPocket[\s\S]*struct PullCatchSeatSafetyDecision[\s\S]*allowImmediateTouchHeld[\s\S]*allowPulledAdjust[\s\S]*adjustDistanceGameUnits' 'Pull-catch palm seating safety must be an explicit testable policy contract.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'evaluatePullCatchSeatSafety[\s\S]*\.grabbedFromPullCatch\s*=\s*grabbedFromPullCatch[\s\S]*\.usingPinchPocket\s*=\s*usingPinchPocket[\s\S]*const float pulledGrabAdjust\s*=\s*pullCatchSeatSafety\.adjustDistanceGameUnits' 'Pulled hand-back adjustment must be gated by pull-catch palm seating safety.'
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'struct PullCatchSeatSourceInput[\s\S]*hasPalmPocketSurface[\s\S]*hasTransitPoint[\s\S]*struct PullCatchSeatSourceDecision[\s\S]*usePalmPocketSurface[\s\S]*preserveTransitEvidence[\s\S]*choosePullCatchSeatSource' 'Pull-catch final seat source selection must be an explicit testable policy contract.'
Require-Text 'src/physics-interaction/hand/Hand.h' 'struct PullCatchIntent[\s\S]*transitPointWorld[\s\S]*transitPointBodyLocal[\s\S]*arrivalTransitPointWorld' 'Pull-catch intent must retain the transit grab point as durable evidence across pull arrival and close reacquire.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'pull-catch wide reacquired close selection[\s\S]*pointSource[\s\S]*transitDist[\s\S]*bodyDist' 'Wide pull-catch close reacquire must rebuild selection from preserved transit evidence instead of only body origin.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'choosePullCatchSeatSource[\s\S]*\.hasPalmPocketSurface\s*=\s*palmPocketMeshAvailable[\s\S]*\.hasTransitPoint\s*=[\s\S]*hasTransitPointBodyLocal[\s\S]*if \(pullCatchSeatSourceDecision\.usePalmPocketSurface' 'Dynamic pull-catch grab commit must prefer fresh palm-pocket mesh evidence while preserving transit evidence as fallback.'
