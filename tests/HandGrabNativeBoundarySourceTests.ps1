...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'publishPullCatchAcquisitionPose[\s\S]*_grabFingerPosePublished\s*=[\s\S]*_grabAcquisitionPhase == grab_three_phase::AcquisitionPhase::TouchHeld \|\| publishPullCatchAcquisitionPose' 'Grab commit must solve finger pose immediately for TouchHeld and pull-catch acquisition, while ordinary near/gravity acquisition still defers.'
...
Require-Text 'src/physics-interaction/hand/Hand.h' 'arrivalTransitMotionOffsetWorld' 'Pull-catch retry must store transit evidence relative to the pulled body, not only as a stale world point.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'close-evidence-detached-from-body[\s\S]*pullCatchLiveTransitOffset' 'Pull-catch close reacquire must rebuild live transit evidence and reject detached stale fallback points.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'const auto transition\s*=\s*applyTransition\(HandTransitionRequest\{ \.event = HandInteractionEvent::SelectionFoundClose \}\);[\s\S]*!transition\.accepted[\s\S]*state-transition-rejected[\s\S]*_currentSelection = selection' 'Pull-catch close reacquire must not mutate selection when the hand state rejects close-selection transition.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'pullCatchCommitSeated[\s\S]*safetyGripToPocketDistance\s*=[\s\S]*\?\s*0\.0f\s*:\s*finalGripToPocketDistance[\s\S]*evaluatePullCatchSeatSafety' 'Pull-catch safety must evaluate the corrected commit seat instead of the stale live seat distance.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'publishPullCatchAcquisitionPose[\s\S]*_grabAcquisitionPhase == grab_three_phase::AcquisitionPhase::TouchHeld \|\| publishPullCatchAcquisitionPose' 'Pull-catch must publish an acquisition finger pose without waiting for final TouchHeld promotion.'
