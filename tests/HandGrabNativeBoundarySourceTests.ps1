...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'targetObjectWorld\s*=[\s\S]*_grabFrame\.desiredObjectWorldAtGrab[\s\S]*buildFromLocalTriangles\(localFingerPoseTriangles\)[\s\S]*solveGrabFingerPoseFromTriangles\([\s\S]*&_grabFingerTriangleIndex' 'Regular grab commit must solve once against the frozen target relation through the local spatial index.'
