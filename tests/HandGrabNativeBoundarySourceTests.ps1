...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryComputeGrabRawRollPalmPocketPivotAWorld\(world,\s*handWorldTransform,\s*livePivotAWorld\)[\s\S]*_grabAuthorityPivotAProxyLocalGame\s*=\s*\{\}' 'Seated palm-pocket promotion must keep proxy-local pivot A at zero after the proxy origin is moved to the raw-roll pocket point.'
...
Require-Text 'src/physics-interaction/grab/GrabContact.h' 'chooseMeshBackedPatchPivotAuthority[\s\S]*patchBroadSurfaceSupport[\s\S]*patchSurfaceSupportTooSmall[\s\S]*SurfacePatchPositionPivot' 'Grab contact patch pivot policy must require broad deduped surface support before authoring a position-only surface patch pivot.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'dedupeContactPatchSamples[\s\S]*classifyContactPatchQuality[\s\S]*SurfacePatchPositionPivot' 'Runtime grab contact patches must dedupe/classify surface samples before the guarded surface patch pivot can compete.'
Require-Text 'src/physics-interaction/grab/GrabCore.h' 'contactPatchDedupedSampleCount[\s\S]*contactPatchDuplicateRejectedSampleCount[\s\S]*contactPatchQualityKind' 'Grab frame telemetry must preserve contact-patch dedupe and quality metadata after capture.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'positionOnlyPatch\.source\s*=\s*GrabPivotAuthoritySource::ContactPatchPositionOnly' 'Accepted contact-patch pivot authority must use the named SurfacePatchPositionPivot path instead of stale runtime telemetry.'
