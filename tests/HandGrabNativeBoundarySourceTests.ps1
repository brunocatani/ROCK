...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_nativeGrab\.create\(\s*world,\s*objectBodyId,\s*initialNativeTargetBodyWorld,\s*_grabFrame\.pivotBBodyLocalGame' 'Close grab must create the native mouse-spring action from the ROCK body-composed target frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeTargetPointWorldGame\(desiredBodyWorld,\s*_grabFrame\.pivotBBodyLocalGame\)' 'Held-object linear target point must still be derived from the native body-frame target and frozen body-local pivot.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryGetGrabAuthorityBodyWorldTransform[\s\S]*tryGetNativeMouseSpringBodyWorldTransform' 'Native dynamic grab relation authority must use the BODY frame followed by FO4VR mouse-spring and the held visual node.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.bodyLocal\s*=\s*makeIdentityTransform\(\)' 'Native dynamic grab must not double-apply a node-to-body local offset when BODY is the visual object frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'freezePivotBBodyLocal\(grabBodyWorldAtGrab,\s*grabGripPoint\)' 'Grab pivotB must be frozen from the native BODY grab authority frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryGetGrabAuthorityBodyWorldTransform\(world,\s*_savedObjectState\.bodyId' 'Held-object convergence and pivot telemetry must use the native BODY grab authority frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'void Hand::flushPendingHeldNativeGrab' 'Held-object native action updates must be flushed from the Havok step boundary.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'appliedNativeLinearResponseScale\s*=\s*native_mouse_spring_grab::composeTuningScale' 'Ordinary single-hand native grabs must log and apply the same sanitized native linear response scale.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'appliedNativeAngularResponseScale\s*=\s*native_mouse_spring_grab::composeTuningScale' 'Ordinary single-hand native grabs must log and apply the same sanitized native angular response scale.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'appliedNativeAngularClampScale\s*=\s*native_mouse_spring_grab::composeTuningScale' 'Ordinary single-hand native grabs must log and apply the same sanitized native angular clamp scale.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'driveMode=nativeMouseSpring looseWeapon=' 'Native grab creation telemetry must identify the native drive mode and loose-weapon status.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'driveMode=sharedConstraint looseWeapon=' 'Shared constraint grab creation telemetry must identify the shared drive mode and loose-weapon status.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'rockGrabLooseWeaponNativeLinearResponseMultiplier' 'Loose non-equipped weapon native tuning must have an explicit neutral multiplier surface.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'rockGrabLooseWeaponAdaptiveLeadMultiplier' 'Loose non-equipped weapon adaptive lead tuning must have an explicit neutral multiplier surface.'
...
Require-Text 'data/config/ROCK.ini' 'fGrabNativeMouseSpringLinearResponseScale\s*=\s*1\.35' 'Packaged ROCK.ini must expose the native mouse-spring linear response tuning used by single-hand grabs.'
Require-Text 'data/config/ROCK.ini' 'fGrabNativeMouseSpringAngularResponseScale\s*=\s*0\.75' 'Packaged ROCK.ini must expose the native mouse-spring angular response tuning used by single-hand grabs.'
Require-Text 'data/config/ROCK.ini' 'fGrabNativeMouseSpringAngularClampScale\s*=\s*0\.85' 'Packaged ROCK.ini must expose the native mouse-spring angular clamp tuning used by single-hand grabs.'
Require-Text 'data/config/ROCK.ini' 'fGrabLooseWeaponNativeLinearResponseMultiplier\s*=\s*1\.0' 'Packaged ROCK.ini must expose neutral loose non-equipped weapon native linear tuning.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'generated hknp palm anchor[\s\S]*not grab-frame[\s\S]*transform authority' 'Grab pivot authority must document why generated palm-anchor readback is not used for grab capture.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'gamePointToHavokPoint\(transform_math::localPointToWorld\(releaseBodyWorld,\s*_grabFrame\.pivotBBodyLocalGame\)\)' 'Tangential throw velocity must use the held-object grab pivot lever arm when the body frame is readable.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'HeldObjectDriveMode::NativeMouseSpring[\s\S]*_nativeGrab\.create\(' 'Ordinary single-hand close grabs must stay on the native mouse-spring action.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'joiningPeerHeldObject[\s\S]*createConstraintGrabDrive' 'Only shared peer-held loose-object joins may leave native mouse spring for the ROCK shared constraint drive.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'createConstraintGrabDrive[\s\S]*createGrabConstraint\(world' 'Shared loose-object constraint creation must be isolated behind the explicit two-hand drive helper.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.bodyLocal\s*=\s*computeRuntimeBodyLocalTransform\(objectWorldTransform,\s*grabBodyWorldAtGrab\)' 'Native dynamic bodyLocal must stay identity; applying BODY as a local offset double-counts the visual frame.'
