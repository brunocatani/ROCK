...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmPocketPivotAWorld\s*=\s*computeGrabStartupCapturePivotAWorld\(world,\s*handWorldTransform\)' 'Close dynamic grab post-prep authority must use the startup raw-hand palm proxy capture pivot only for the palm-pocket point.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame\(world,\s*handWorldTransform,\s*nullptr,\s*proxyAuthorityWorld,\s*proxyAuthoritySource\)[\s\S]*makeRawHandPalmProxyAuthorityFrame\(proxyAuthorityWorld\)' 'Held-object targets must use the resolved raw-hand hidden proxy frame without rebinding its origin to pivot A.'
