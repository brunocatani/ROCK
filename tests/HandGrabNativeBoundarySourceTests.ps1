...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'finalizeTouchHeldSamePointAuthority[\s\S]*gripEvidencePointWorld\(_grabFrame,\s*currentNodeWorld\)[\s\S]*shiftObjectToAlignGripWithPocket\(currentNodeWorld,\s*activeProxyPivotAWorld,\s*currentGripWorld\)[\s\S]*_grabAuthorityPivotAProxyLocalGame\s*=\s*\{\}' 'TouchHeld final freeze must keep the same evidence point, seat it at the live active proxy pivot, and keep proxy-local pivot A at zero.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveActiveGrabAuthorityPivotAWorld\(world,\s*handWorldTransform,\s*activeProxyPivotAWorld\)[\s\S]*proxyAuthorityWorld\s*=\s*makeProxyFrameWithPivotOrigin\(proxyAuthorityWorld,\s*activeProxyPivotAWorld\)[\s\S]*makeRawRotationPalmTranslationFrame\(handWorldTransform,\s*proxyAuthorityWorld\)' 'Held-object targets must use root-flattened raw hand rotation and the active raw-roll/pinch proxy-origin translation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'threePhaseTouchReachedFinalFreeze' 'Near/far convergence must transition to held by committing the final same-point frozen relation at touch.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'threePhaseTimeoutInsidePocketFinalFreeze' 'Near/far convergence timeout must promote to held by committing the final same-point frozen relation inside the pocket.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'pendingInitialTouchHeldFinalFreeze\s*=\s*fullHeldAuthorityAtCapture[\s\S]*!usingPinchPocket[\s\S]*!useAuthoredGrabFrame[\s\S]*_grabPendingInitialTouchHeldFinalFreeze\s*=\s*pendingInitialTouchHeldFinalFreeze' 'Direct TouchHeld capture must mark generic close grabs for one deferred same-point final freeze on the first held update.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'if \(_grabPendingInitialTouchHeldFinalFreeze\)[\s\S]*DIRECT TOUCHHELD FINAL FREEZE[\s\S]*_grabPendingInitialTouchHeldFinalFreeze\s*=\s*false' 'The first held update must consume the deferred direct-TouchHeld final freeze once the same-point BODY relation is rebuilt.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'rockGrabLooseWeaponSharedConstraintAngularForceMultiplier' 'Live held-object angular force must not keep a separate loose-weapon angular force multiplier.'
...
Reject-Text 'data/config/ROCK.ini' 'fGrabLooseWeaponSharedConstraintAngularForceMultiplier' 'Packaged ROCK.ini must not expose removed loose non-equipped weapon angular force tuning.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'committedTouchHeldAuthority\s*=\s*authoredConvergingRelation\s*\|\|\s*finalizeTouchHeldSamePointAuthority' 'Touch-range promotion must commit the final same-point frozen relation before entering TouchHeld.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'clearGrabExternalHandWorldTransform\(_isLeft\)[\s\S]*_hasGrabVisualHandTransform\s*=\s*false[\s\S]*THREE-PHASE GRAB TOUCHHELD FINAL FREEZE' 'TouchHeld final freeze must clear any previous visual hand relation before publishing again.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'evaluateSeatedPalmPocketPromotion' 'TouchHeld promotion runtime must not keep calling the removed seated palm-pocket authority policy.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'finalFrozenAuthorityFrame\s*=\s*grab_authority_frame_math::freezeGrabAuthorityFrame<RE::NiTransform>[\s\S]*applyFrozenGrabAuthorityFrameToGrabFrame\(_grabFrame,\s*finalFrozenAuthorityFrame\)' 'Accepted TouchHeld promotion must rebuild the frozen authority frame once from the current state, not patch individual grab references.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'seatedRetargetRejectedKeepFrozen' 'TouchHeld promotion must not keep the removed seated-candidate rejection branch.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFingerPosePublished \? "yes" : "no"\);\s*if \(g_rockConfig\.rockGrabMeshFingerPoseEnabled && _hasGrabFingerPose && !_grabFingerPosePublished\)[\s\S]*resolveSurfaceAimObjectLocal\(_grabFingerPose,\s*currentNodeWorld\)' 'TouchHeld final freeze must publish the captured finger pose against the refrozen object frame instead of rebuilding a promoted-seat finger target set.'
