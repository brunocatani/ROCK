...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'createProxyConstraintGrabDrive\(\s*bhkWorld,\s*world,\s*objectBodyId,\s*handWorldTransform,\s*grabPivotAWorld' 'Close dynamic grab must create the hidden proxy plus custom finite-force constraint from the ROCK raw hand frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeTargetPointWorldGame\(desiredBodyWorld,\s*activeGrabDrivePivotBBodyLocalGame\(\)\)' 'Held-object linear target point must derive from the active body-local pivot, including the proxy constraint pivot.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryGetGrabAuthorityBodyWorldTransform[\s\S]*tryGetNativeMouseSpringBodyWorldTransform' 'Dynamic grab object-side authority must use the BODY frame; MOTION remains COM and diagnostics only.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'updateProxyConstraintGrabDriveTarget' 'Proxy constraint dynamic grab must refresh constraint transforms and angular target from the captured raw hand relation.'
