...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame\(handWorldTransform,\s*proxyFrameWorldAtGrab,\s*proxyFrameSourceAtGrab\)' 'Close dynamic grab must resolve the hidden proxy frame directly from root-flattened hand authority before creating the custom finite-force constraint.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'createProxyConstraintGrabDrive\(\s*bhkWorld,\s*world,\s*objectBodyId,\s*proxyFrameWorldAtGrab,\s*grabPivotAWorld' 'Close dynamic grab must create the hidden proxy plus custom finite-force constraint from the resolved hand-bone authority frame.'
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'isFiniteGrabAuthorityTransform[\s\S]*transform\.rotate\.entry\[row\]\[column\]' 'Root-flattened hand authority validation must include rotation matrix entries, not translation only.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*fallbackPalmAnchorWorld' 'Dynamic grab authority must not keep a silent live hand-collider fallback.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'authorityWorld\s*=\s*[\s\S]*computeGrabAuthorityFrameFromHandBasis\(handWorldTransform,\s*_isLeft\)' 'Held-object targets must use the root-flattened hand transform directly.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.pivotBConstraintLocalGame\s*=\s*constraintDrivePivotBBodyLocalGame\(_grabFrame\)' 'Proxy constraint transform-B must stay pinned to the selected contact point in the solver body-B frame after the split frame is known.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'updateProxyConstraintGrabDriveTarget' 'Proxy constraint dynamic grab must refresh constraint transforms and angular target from the captured raw hand-bone relation.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'setHeldAngularVelocity|applyProxyConstraintAngularVelocityDrive|desiredAngularTargetWorld' 'Dynamic grab rotation must not use the removed direct angular velocity writer while the ragdoll atom owns angular authority.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeLongObjectAngularAuthorityScale\([\s\S]*rockGrabLongObjectAngularScalingEnabled' 'Dynamic grab angular atom force must apply the configured long-object lever scale.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabAuthorityFrameFromHandBasis[\s\S]*return handTransform' 'Grab pivot authority must resolve directly from the root-flattened hand frame.'
Reject-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabAuthorityFrameFromHandBasis[\s\S]*computeGrabPivotAPositionFromHandBasis\(handTransform,\s*isLeft\)' 'Grab pivot authority must not consume the old INI-configured handspace PivotA.'
