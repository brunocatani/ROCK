...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmPocketPivotAWorld\s*=\s*computeGrabStartupCapturePivotAWorld\(world,\s*handWorldTransform\)' 'Close dynamic grab post-prep authority must use the startup corrected palm proxy capture pivot only for the palm-pocket point.'
Require-Text 'src/physics-interaction/hand/Hand.h' 'tryComputeGrabRawRollPalmPocketPivotAWorld' 'Held-time palm-pocket support must share an explicit palm-pocket pivot helper.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame\(world,\s*handWorldTransform,\s*nullptr,\s*proxyAuthorityWorld,\s*proxyAuthoritySource\)[\s\S]*makeCorrectedPalmProxyAuthorityFrame\(proxyAuthorityWorld\)' 'Held-object targets must use the resolved corrected hidden proxy frame without rebinding its origin to pivot A.'
