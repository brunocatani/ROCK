...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'solveGrabSeat\(' 'Pulled and close grabs must seat through the grab pose solve (GrabPoseObjective.h), not a fixed hand-back adjustment or the retired correction stack.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'const bool seatSolveEligible =\s*!looseWeaponPrimaryAttachApplied && !usingPinchPocket && pocket\.valid;[\s\S]*seatSolve = solveGrabSeat\(\s*grabLocalMeshTriangles,\s*grabBodyWorldAtGrab,\s*objectToBodyAtGrab,\s*grabGripPoint,\s*grabPivotAWorld\);[\s\S]*if \(seatSolve\.applied\) \{\s*grabPivotAWorld = seatSolve\.pivotAWorld;\s*desiredBodyWorld = seatSolve\.desiredBodyWorld;\s*desiredObjectWorld = seatSolve\.desiredObjectWorld;' 'The capture seat must run the pose solve for every non-attach non-pinch arrival and apply its outcome through pivot A + desired body/object worlds (the two channels the authority freeze preserves). If the solve did not run, the seed pose must be kept whole.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'seatSwingAlignmentWanted|seatRollAlignmentWanted' 'The seat swing/roll alignment corrections are retired: the pose solve owns seat orientation. Reintroducing a per-axis correction beside it recreates the stacked-patch architecture the solve replaced.'
...
Require-Text 'src/physics-interaction/grab/GrabPoseObjective.h' 'model\.shape == ShapeClass::Rod' 'Only rods get axis-preference terms in the pose objective: plates keep their edge/corner holds (31 verified plate holds put the face normal 63 deg off the palm normal) because the objective deliberately has NO plate face-alignment term. Adding one fights the intended grip.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'SEAT DEPTH BACKSTOP|palmPlaneOvershoot' 'The palm-plane penetration backstop is retired: over-penetration is a hard objective wall (overPen) inside the pose solve, not a post-hoc push-out.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outcome\.reason = "nonFiniteSolvedPose";[\s\S]*outcome\.pivotAWorld = pivotAWorld;' 'The pose solve must fail CLOSED: a non-finite solved pose reverts to the untouched seed seat, never a partial correction.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'kMaxSolvedReacquiresPerHold[\s\S]*reacquireSolve = solveGrabSeat\([\s\S]*livePivotAWorld,\s*/\*translationOnly=\*/true\);[\s\S]*reacquireChurnCap' 'Seated pivot reacquire must re-seat through the pose solve in TRANSLATION-ONLY mode (rotating a held object is a visible twitch) under a per-hold churn cap - the unbounded full solve here is what dropped 90fps to 60 mid-hold. Beyond the cap the plain promoted seat commits and the churn WARNs as evidence.'
