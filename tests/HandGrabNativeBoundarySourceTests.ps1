...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'setHeldAngularVelocity|world->SetBodyAngularVelocity\(_savedObjectState\.bodyId|applyProxyConstraintAngularVelocityDrive' 'Dynamic grab must not keep direct held-body angular velocity authority when the ragdoll angular atom is active.'
Require-Text 'src/physics-interaction/grab/GrabConstraint.cpp' 'setGrabMotorAtomsActive\(header,\s*true,\s*true\)' 'Dynamic grab must enable the ragdoll angular atom as production angular authority.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'writeGrabRagdollAngularTarget\(targetBRca,\s*desiredBodyTransformAuthoritySpace,\s*transformAFrameLocal\.rotate\)' 'Dynamic grab must feed raw hand/controller angular authority into the ragdoll atom target.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeLongObjectAngularAuthorityScale\([\s\S]*rockGrabLongObjectAngularScalingEnabled' 'Dynamic grab angular motor authority must apply the configured long-object lever scale.'
