...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame\(world,\s*handWorldTransform,\s*nullptr,\s*proxyAuthorityWorld,\s*proxyAuthoritySource\)[\s\S]*resolveActiveGrabAuthorityPivotAWorld\(world,\s*handWorldTransform,\s*activePivotAWorld\)[\s\S]*rawAuthorityFrameWorld\s*=\s*grab_authority_frame_math::makeRawAuthorityFrame\(handWorldTransform,\s*activePivotAWorld\)[\s\S]*computeDesiredObjectWorld\(rawAuthorityFrameWorld,\s*_grabFrame\.rawAuthorityObjectSpace\)' 'Held-object targets must use root-flattened raw hand rotation and active pivot A as the raw-authority frame while keeping the hidden proxy frame queued separately.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'activePivotBBodyLocalGame\s*=\s*constraintBInBodyBSpace\.translate' 'Held-object updates must refresh the active proxy body-local pivot from transform B before composing target points.'
...
if ($applyFrozenStart -lt 0 -or $applyFrozenEnd -lt 0) {
    ...
    if ($outsideApplyFrozen -match '_grabFrame\.(rawHandSpace|handBodyToRawHandAtGrab|rawAuthorityObjectSpace|rawAuthorityBodySpace|rawRotationProxyHandSpace|rawRotationProxyBodyHandSpace|bodyLocal|rootBodyLocal|ownerBodyLocal|gripPointLocal|gripPointBodyLocalGame|pivotBBodyLocalGame|pivotBConstraintLocalGame|pivotAHandBodyLocalGame|desiredObjectWorldAtGrab|desiredBodyWorldAtGrab|hasFrozenPivotB|hasGripPoint)\s*=') {
