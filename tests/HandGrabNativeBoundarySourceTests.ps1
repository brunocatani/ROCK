...
Require-Text 'src/physics-interaction/debug/DebugBodyOverlay.cpp' 'targetBodyOverlayFrameSource[\s\S]*BodyOverlayRole::Target[\s\S]*BodyOverlayFrameSource::BodyArrayTransform[\s\S]*published\.frameSource\s*=\s*targetBodyOverlayFrameSource\(entry\.role\)[\s\S]*appendSolvedBodyCaptureRequest\([\s\S]*published\.frameSource' 'Target collider visualizer must capture selected/held object colliders after solve from the same hknp BODY frame used by grab body-B authority.'
Require-Text 'src/physics-interaction/debug/DebugBodyOverlay.cpp' 'targetAxisOverlayFrameSource[\s\S]*AxisOverlayRole::TargetBody[\s\S]*BodyOverlayFrameSource::BodyArrayTransform[\s\S]*appendSolvedBodyCaptureRequest\([\s\S]*targetAxisOverlayFrameSource\(published\.entry\.role\)' 'Target body axes in the visualizer must also request the post-solve hknp BODY frame so the drawn orientation matches body-B authority.'
