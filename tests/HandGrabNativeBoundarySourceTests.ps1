...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'freezeNearConvergeRelation[\s\S]*AcquisitionPhase::NearConverging' 'Near-converge capture must freeze the current relation instead of forcing an unseated mesh point to the palm pivot.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'relationMode\s*=\s*useAuthoredGrabFrame\s*\?\s*"authoredGrabNodeFrame"\s*:[\s\S]*"rockPointFrozenRelation"[\s\S]*"rockPointToPalm"' 'Grab commit must distinguish unseated frozen-relation capture from seated ROCK point-to-palm authority.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmSeatWorld\s*=\s*pocket\.palmCenterWorld' 'Three-phase grab capture must keep the palm anchor as the seat witness, not the depth-offset pocket center.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'grabPivotAWorld\s*=\s*freezeNearConvergeRelation\s*\?\s*gripArea\.contactSeedWorld\s*:\s*palmSeatWorld' 'Near-converge capture must freeze pivot A at the current mesh point while seated grabs still use the palm anchor.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'setHeldAngularVelocity\(world,\s*_savedObjectState\.bodyId,\s*_heldBodyIds,\s*appliedAngularVelocity\)' 'Dynamic grab direct angular authority must write the accepted held-body set, not only the primary selected body.'
