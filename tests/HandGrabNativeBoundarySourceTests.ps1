...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryGetGrabDriveObjectWorldTransform[\s\S]*HeldObjectDriveMode::ProxyConstraint[\s\S]*tryResolveLiveBodyWorldTransform[\s\S]*BodyFrameSource::MotionCenterOfMass[\s\S]*return\s+tryGetGrabAuthorityBodyWorldTransform' 'Proxy-constraint object-side reads must use the live hknp MOTION solver frame when available, then fall back to the native BODY helper.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintUsesMotionBodyAtGrab\s*=\s*hasMotionBodyWorldAtGrab\s*&&\s*motionBodySourceAtGrab\s*==\s*body_frame::BodyFrameSource::MotionCenterOfMass' 'Proxy-constraint grab capture must choose the hknp solver body-B frame from live MOTION readback when that is the active source.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintBodyWorldAtGrab\s*=\s*constraintUsesMotionBodyAtGrab\s*\?\s*motionBodyWorldAtGrab\s*:\s*grabBodyWorldAtGrab' 'Proxy-constraint grab capture must encode body-B pivots and desired targets in the same solver frame hknp reports.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constexpr\s+bool\s+constraintUsesMotionBodyAtGrab\s*=\s*false' 'Proxy-constraint grab capture must not hardcode BODY when hknp reports a MOTION solver frame.'
