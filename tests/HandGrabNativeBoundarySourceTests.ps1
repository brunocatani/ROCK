...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame\(\s*world,\s*handWorldTransform,\s*&outPreparation\.handBodyWorldAtGrab,\s*outPreparation\.proxyFrameWorldAtGrab,[\s\S]*GrabAuthorityProxyFramePolicy::LivePalmOnly\)[\s\S]*outPreparation\.grabAuthorityPivotAWorld\s*=\s*outPreparation\.proxyFrameWorldAtGrab\.translate[\s\S]*outPreparation\.palmPocketPivotAWorld\s*=\s*outPreparation\.proxyFrameWorldAtGrab\.translate[\s\S]*outPreparation\.grabPivotAForPrimaryChoice\s*=\s*outPreparation\.palmPocketPivotAWorld' 'Close dynamic grab must resolve the hidden proxy frame before generated/proxy-local palm-pocket mesh/contact acquisition.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'refreshGrabCaptureTransforms\(\s*selection\.rootNode,\s*outSetup\.meshSourceNode,\s*outSetup\.collidableNode\)[\s\S]*outSetup\.objectWorldTransform\s*=\s*outSetup\.collidableNode\s*\?\s*outSetup\.collidableNode->world\s*:\s*handWorldTransform[\s\S]*objectWorldTransform\s*=\s*meshCaptureSetup\.objectWorldTransform[\s\S]*objectToBodyAtGrab\s*=\s*computeRuntimeBodyLocalTransform\(objectWorldTransform,\s*grabBodyWorldAtGrab\)' 'Dynamic grab capture must refresh selected scene-node transforms before sampling objectWorldTransform for BODY relation capture.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolvedOwnerNode\s*=\s*bodyCollisionObject[\s\S]*refreshGrabCaptureNodeTransform\(ownerRefresh,\s*"resolvedOwner",\s*resolvedOwnerNode\)[\s\S]*outCapture\.objectWorldTransform\s*=\s*resolvedOwnerNode->world[\s\S]*cacheTrianglesInLocalSpace\(meshTriangles,\s*outCapture\.objectWorldTransform\)' 'Dynamic grab capture must refresh the final BODY owner node before resampling objectWorldTransform for mesh-local and BODY relation capture.'
...
Require-Text 'src/physics-interaction/hand/Hand.h' 'std::atomic<float>\s+_lastGrabPhysicsHz\{\s*0\.0f\s*\}[\s\S]*std::atomic<float>\s+_lastGrabPhysicsRateForceScale\{\s*1\.0f\s*\}' 'Physics-rate grab telemetry must be synchronized because motor writes and diagnostic reads can cross runtime callbacks.'
...
if ($applyFrozenStart -lt 0 -or $applyFrozenEnd -lt 0) {
    ...
    if ($outsideApplyFrozen -match '_grabFrame\.(rawHandSpace|handBodyToRawHandAtGrab|proxyAuthorityHandSpace|proxyAuthorityBodyHandSpace|rootBodyLocal|ownerBodyLocal)\s*=|_grabFrame\.authority\.(bodyLocal|gripPointBodyLocalGame|pivotBBodyLocalGame|pivotBConstraintLocalGame|pivotAHandBodyLocalGame|desiredObjectWorldAtGrab|desiredBodyWorldAtGrab|hasFrozenPivotB)\s*=|_grabFrame\.gripEvidence\.(gripPointLocal|hasGripPoint)\s*=') {
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'relationMode\s*=\s*usingPinchPocket\s*\?\s*"pinchPocket"\s*:\s*"rockPointToPalm"' 'Grab commit must route generic grabs through the pinch/support authority path.'
...
Reject-Path 'src/physics-interaction/grab/GrabNodeInfoMath.h' 'Removed authored grab-node transform helpers must not return.'
Reject-Path 'src/physics-interaction/grab/GrabNodeNamePolicy.h' 'Removed authored grab-node naming policy must not return.'
Reject-Path 'tests/GrabNodeNamePolicyTests.cpp' 'Removed authored grab-node policy tests must not return.'
Reject-Text 'src/RockConfig.h' 'rockGrabNode|rockPrintGrabNodeInfo' 'ROCK config state must not expose authored grab-node controls.'
Reject-Text 'src/RockConfig.cpp' 'bGrabNode|sGrabNode|bPrintGrabNodeInfo|GrabNodeNamePolicy' 'ROCK config loading must not parse authored grab-node controls.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'ROCK:GrabR|ROCK:GrabL|grabNodeAnchor|authoredGrabNode|GrabNodeInfoMath|GrabNodeNamePolicy' 'Dynamic grab must not retain authored ROCK:GrabR/L marker discovery or authority paths.'
Reject-Text 'src/physics-interaction/grab/MeshGrab.h' 'grabNodeNameBlacklist|blacklistedShapes|GrabNodeNamePolicy' 'Visual mesh extraction must not retain the authored grab-node marker blacklist.'
Reject-Text 'data/config/ROCK_example.ini' 'bGrabNode|sGrabNode|bPrintGrabNodeInfo|ROCK:GrabR|ROCK:GrabL' 'The example INI must not expose authored grab-node controls.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'no object-side contact point[\s\S]*object origin/COM fallback is not valid dynamic grab authority' 'Dynamic grab must fail when no contact object-side pivot exists instead of falling back to object origin or COM.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'support\.normalBodyLocal[\s\S]*support\.axisBodyLocal[\s\S]*support\.binormalBodyLocal' 'Grab support-frame overlay must replay the captured face frame through BODY-local authority.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outResolution\.surfaceOwnerMatchesResolvedBody\s*=[\s\S]*surfaceOwnerRecord->bodyId\s*==\s*outResolution\.primaryChoice\.bodyId[\s\S]*acceptsSelectedMultibodyOwnerlessVisualMesh\(\s*selectedObject,\s*preparedBodySet,\s*outResolution\.primaryChoice\.bodyId' 'Grab commit owner resolution must accept ownerless same-family multipart mesh evidence without accepting concrete mismatched owners.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'ownerGraceSeconds\s*=\s*pull_motion_math::kOwnerGraceSeconds' 'Dynamic pull must keep pulled-object ownership after the velocity drive window.'
...
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'struct ProgrammaticArrivalSeatSafetyInput[\s\S]*programmaticArrival[\s\S]*usingPinchPocket[\s\S]*struct ProgrammaticArrivalSeatSafetyDecision[\s\S]*allowImmediateTouchHeld[\s\S]*requireSettledVisualRelation' 'Programmatic-arrival palm seating safety must be an explicit testable policy contract.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'programmaticTouchHeldAuthorityEvidence[\s\S]*grabbedFromPullCatch \?[\s\S]*pullArrivalTouchHeldAuthorityEvidence[\s\S]*hasStablePocketTouchContact[\s\S]*requireEvidenceForTouchHeld = programmaticArrival' 'A direct forced arrival must not treat mesh/support availability as proof that it has physically reached the hand.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'deferFingerPoseUntilSettledTouch[\s\S]*requiresSettledVisualHandRelation[\s\S]*buildAcquisitionFingerPose\(pendingTarget, 0\.0f\)[\s\S]*final mesh solve deferred until seated TouchHeld' 'An unsettled programmatic arrival must hold the neutral pre-close pose instead of solving and closing against a transient seat.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'programmaticArrival && reachedTouchRange[\s\S]*buildSeatedPalmPocketSupportPatch[\s\S]*\.programmaticArrival = programmaticArrival' 'A verified programmatic arrival must build current palm-pocket support even when the seated surface is far from the stale far-ray hit.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'finalPoseHandWorld =[\s\S]*buildHeldObjectRelativeHandWorld\(currentNodeWorld, _grabFrame\.rawHandSpace\)[\s\S]*rebaseFingerSkeletonSnapshot\(liveFingerSnapshot, handWorldTransform, finalPoseHandWorld\)[\s\S]*solveFrozenMeshFingerPoseBase\([\s\S]*finalPoseHandWorld' 'The deferred final finger probe must use the settled object-relative visual hand frame, not the controller-side hand frame from before visual attachment.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFingerLocalTransformFinalizePending[\s\S]*_grabAcquisitionPhase == grab_three_phase::AcquisitionPhase::TouchHeld[\s\S]*publishLocalTransforms=\*/true[\s\S]*local transforms finalized after settled joint publication' 'Surface-local finger correction must wait one update for the final joint pose to become the live root-flattened skeleton.'
...
Require-Text 'src/physics-interaction/grab/GrabPoseCandidateSelector.h' 'kMaxProxyTriangles = 64[\s\S]*kMaxHandCapsules = 19[\s\S]*std::array<RotationCandidate, 13>' 'The fitted grab-pose refinement must have compile-time geometry, hand-volume, and candidate budgets.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'Fixed-budget fitted-objective refinement[\s\S]*grab_pose_candidate_selector::selectBestCandidate[\s\S]*Seat depth stop' 'The bounded objective selector must run once between analytic orientation and the exact capture-time depth safety.'
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'buildGripPresentationAxisTowardFingertips\([\s\S]*crossPalm \* std::cos\(tiltRadians\) \+ fingerForward \* std::sin\(tiltRadians\)' 'Grip-axis tilt must resolve each hand live signed cross-palm Z first, then add a positive fingers-forward +X component.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'gripAxisTiltRadiansForHand|isLeft \? -g_rockConfig\.rockPullPresentationGripAxisTiltDegrees' 'Grip-axis tilt is an anatomical magnitude toward +X, not a hand-signed angle.'
Require-Text 'src/RockConfig.cpp' 'rockPullPresentationGripAxisTiltDegrees < 0\.0f[\s\S]*rockPullPresentationGripAxisTiltDegrees > 45\.0f' 'Grip-axis tilt configuration must reject the retired negative signed-angle convention.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'requiresSettledVisualHandRelation &&\s*!_grabFrame\.seat\.hasPivotReacquire &&\s*!_grabFrame\.seat\.hasSettledVisualHandRelation\) \{\s*_grabFrame\.seat\.hasSettledVisualHandRelation = true;' 'TouchHeld promotion must never carry an unsatisfiable settled-visual-relation requirement: a rejected seated reacquire settles the visual hand to the frozen commanded relation instead of blocking the publish gate for the whole hold.'
...
Require-Text 'src/physics-interaction/grab/GrabCore.h' 'struct GrabSupportFrameState[\s\S]*GripSupportKind kind[\s\S]*bool authoredPivot' 'Grab frame must store support-model metadata separately from contact patch samples.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.authority\.bodyLocal\s*=\s*makeIdentityTransform\(\)' 'Dynamic grab must not collapse BODY and the visible object into the same frame; logs show that causes instant angular correction.'
