...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'const bool seatPlateShape = seatLongAxis\.valid && !seatRodShape && seatFlatFace;[\s\S]*normalizeOrZero\(crossProduct\(currentAxisWorld, currentSecondAxisWorld\)\);[\s\S]*rotateTransformWorldAboutPoint\(\s*seatBodyWorld, plateAxis, plateAngleRadians, grabGripPoint\)' 'Plate-shaped objects (no dominant long axis but a well-separated minor axis: books, trays, boards) must seat-align by swinging the mesh FACE NORMAL onto the palm normal - the rod twist has no long axis to twist about, so without this branch a flat face keeps its arrival tilt and cuts into the hand.'
