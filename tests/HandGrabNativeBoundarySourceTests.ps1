...
Require-Text 'src/physics-interaction/grab/GrabFinger.h' 'publishWholeMeshSurfaceAimTargets\s*=\s*false' 'Whole-mesh per-finger surface aim must remain opt-in so existing weapon/support callers keep their current behavior.'
Require-Text 'src/physics-interaction/grab/GrabFinger.h' 'publishWholeMeshSurfaceAimTargets[\s\S]*!useTarget[\s\S]*poseTargets\.useWholeMeshForMissingTargets' 'Targetless whole-mesh finger solving must publish visual surface aim only from the actual per-finger mesh hit path.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'solveGrabFingerPoseFromTriangles\([\s\S]*_grabFrame\.fingerPoseAimValid,\s*true' 'Loose-object grab finger pose must opt into per-finger whole-mesh surface aim targets for visual hand correction.'
Reject-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'publishWholeMeshSurfaceAimTargets|surfaceAimTargetCount' 'Two-handed equipped weapon grab must not be changed by the loose-object visual finger aim opt-in.'
