...
Require-Text 'src/physics-interaction/animation/AuthoredWeaponGripCapturePolicy.h' 'shouldCaptureAuthoredSupportHandVisualAnchor[\s\S]*input\.authoredSupportGrip[\s\S]*input\.pumpSemantic[\s\S]*!input\.sourceRootIsWeaponRoot[\s\S]*input\.sourceRootBelongsToWeapon[\s\S]*input\.weaponGenerationMatches' 'The visual articulated anchor must be limited to a current authored pump descendant.'
...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'captureAuthoredSupportHandVisualAnchor\([\s\S]*?pumpSemantic[\s\S]*?shouldCaptureAuthoredSupportHandVisualAnchor[\s\S]*?grip\.hasHandVisualSourceFrame\s*=\s*true;' 'Authored pumps must capture a separate visual-only articulated frame.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'tryRebindPartGripToCurrentGeneration\([\s\S]*?if \(grip\.authoredSupportGrip\)[\s\S]*?grip\.attachmentRoot\s*=\s*_activeWeaponNode;[\s\S]*?captureAuthoredSupportHandVisualAnchor\([\s\S]*?"generation-rebind"' 'Generation rebinding must preserve authored physical weapon-root authority while refreshing the pump visual source.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'resolvePartGripHandWorld\([\s\S]{0,500}resolveCurrentHandVisualSourceRoot[\s\S]{0,900}resolveAuthoredSupportHandVisualWorld' 'The locked support hand must resolve its visual-only pump source before the ordinary authored weapon-root frame.'
Reject-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'resolvePartGripWorld\([^\{]*\{[\s\S]{0,450}handVisualSource' 'The visual-only pump anchor must never feed the physical support pivot.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'resolvePartGripNormalWeaponLocal\([\s\S]*?resolveCurrentSupportAttachmentRoot\([\s\S]*?return grip\.normalLocal;\s*\}\s*RE::NiTransform TwoHandedGrip::resolvePartGripHandWorld' 'The physical support normal must resolve only from the ordinary physical source frame before visual hand resolution begins.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'currentHandVisualSourceRoot\s*=\s*resolveCurrentHandVisualSourceRoot[\s\S]{0,350}outReport\.sourceRoot\s*=\s*reinterpret_cast<std::uintptr_t>\([\s\S]{0,180}useHandVisualSourceFrame[\s\S]{0,700}outReport\.handPartLocalIsSourceLocal\s*=[\s\S]{0,150}useHandVisualSourceFrame' 'The provider report must validate and expose the moving visual source while retaining authored grip provenance.'
