...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'authoredInteractionAcquisitionValid\s*=\s*decision\.acquisitionSource\s*==[\s\S]*PhysicalContact\s*\|\|[\s\S]*ProximityProbe[\s\S]*shouldUseAuthoredSupportGrip\([\s\S]*shouldCaptureDynamicHandoffGrip\([\s\S]*authored_support_grab_policy::select\([\s\S]*Selection::Authored[\s\S]*Selection::Reject[\s\S]*return selectionDecision\.selection;[\s\S]*tryGetSupportGripEvidenceView\(decision\.bodyId, weaponNode, evidenceView\)' 'Authored or explicitly bounded handoff selection must precede the mesh solver, while other pending or missed valid authored acquisition rejects before dynamic fallback.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'Selection::Authored[\s\S]*?grip\.attachmentRoot\s*=\s*weaponNode;[\s\S]*?grip\.hasSourceFrames\s*=\s*false;' 'Authored support grips must retain weapon-root-local physical authority.'
Require-Text 'src/physics-interaction/weapon/AuthoredSupportGrabPolicy.h' 'providerPartAuthorityActive[\s\S]*Selection::ProviderDynamic[\s\S]*dynamicHandoffCaptureEligible[\s\S]*Selection::DynamicHandoff[\s\S]*authoredCaptureEligible[\s\S]*Selection::Authored[\s\S]*!input\.modeEnabled[\s\S]*Selection::DynamicUnrestricted[\s\S]*Capability::Unavailable[\s\S]*Selection::DynamicFallback[\s\S]*Selection::Reject' 'Exact provider targets must override ROCK, followed by bounded firing-grip handoff, authored, mode-off dynamic, qualified fallback, and fail-closed rejection.'
Require-Text 'src/physics-interaction/weapon/WeaponSupport.h' 'shouldCaptureDynamicHandoffGrip[\s\S]{0,900}normalSupportAcquisition[\s\S]{0,260}ambidextrousHandoffEnabled[\s\S]{0,260}firingGripProximityAuthorityEnabled[\s\S]{0,260}providerPartAuthorityActive[\s\S]{0,320}supportPalmToFiringGripDistance[\s\S]{0,320}!input\.authoredCaptureEligible[\s\S]{0,320}authoredSeatToFiringGripDistance' 'The dynamic handoff station must be limited to normal support acquisition inside the firing-grip promotion radius, behind provider authority, while preserving an authored seat already at that grip.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'transitionToGripping\([\s\S]*capturePartGrip\([\s\S]{0,500}providerPartAuthority,[\s\S]{0,120}firingGripProximityAuthorityEnabled,[\s\S]{0,80}true,[\s\S]{0,120}&supportCaptureHandWorld[\s\S]*updatePartCarryGrip\([\s\S]*capturePartGrip\([\s\S]{0,500}providerPartAuthority,[\s\S]{0,120}false,[\s\S]{0,80}false' 'Only normal two-hand support acquisition may enter the dynamic handoff station; PartCarry part grabs must retain their independent strict policy.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'useDynamicHandoffGrip[\s\S]*if \(!useDynamicHandoffGrip\s*&&\s*cachedTrianglesFound\)[\s\S]*if \(useDynamicHandoffGrip\)[\s\S]{0,1000}supportAttachmentRoot\s*=\s*weaponNode[\s\S]{0,300}grip\.attachmentRoot\s*=\s*weaponNode[\s\S]{0,300}grip\.gripLocal\s*=\s*_primaryGripLocal[\s\S]{0,900}gripWorldPoint\s*=\s*useDynamicHandoffGrip\s*\?[\s\S]{0,180}firingGripWorld' 'A dynamic handoff capture must lock the actual weapon-root firing-grip station instead of an incidental receiver triangle, while ordinary dynamic captures retain the mesh solver.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'synchronizeAuthoredSupportCapabilityIdentity\([\s\S]{0,900}getWeaponBodyCount\(\)\s*>\s*0[\s\S]{0,500}!frameInput\.animationBoundaryActive[\s\S]{0,400}advanceAuthoredSupportCapabilityQualification[\s\S]{0,500}shouldCollectAuthoredSupportCapabilityEvidence[\s\S]{0,300}refreshAuthoredSupportGripActivationState[\s\S]{0,300}observeAuthoredSupportCapability' 'Capability qualification must run post-FRIK from current identity and generated geometry, spend only continuously ready elapsed time, then collect bounded pose evidence.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'qualificationReady\s*=[\s\S]{0,320}currentGeometryReady[\s\S]{0,220}capabilityRuntime\.weaponDrawn[\s\S]{0,180}isNodeVisible\(weaponNode\)[\s\S]{0,180}!capabilityRuntime\.localMenuBlocking[\s\S]{0,180}!capabilityRuntime\.compatibilityConfigBlocking[\s\S]{0,220}!frameInput\.animationBoundaryActive[\s\S]{0,220}supportRuntimeState\.supportGripAllowed[\s\S]{0,220}!supportRuntimeState\.providerPartAuthority\.active' 'Missing authored evidence may spend fallback qualification time only while the exact weapon is visible, interactive, generated, and locally owned.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'authoredCapabilityAllowsIndicator\s*=[\s\S]{0,320}!_handlingSettings\.authoredOnlySupportGrabsEnabled\s*\|\|[\s\S]{0,180}Capability::Usable[\s\S]{0,700}activationStateValid\s*=[\s\S]{0,220}authoredCapabilityAllowsIndicator' 'Authored-only mode must not advertise a pending or unavailable authored support point.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'resetAuthoredSupportCapability\("reset"\)[\s\S]*resetAuthoredSupportCapability\("firing-hand-changed"\)' 'World reset and support-topology changes must invalidate the generation-bound capability verdict.'
foreach ($unrelatedGrabPath in @(
    'src/physics-interaction/hand/HandGrab.cpp',
    'src/physics-interaction/hand/DynamicHandCollision.cpp',
    'src/physics-interaction/grab/TouchGrabRuntime.cpp',
    'src/physics-interaction/stash/ShoulderStashDetector.cpp')) {
    Reject-Text $unrelatedGrabPath 'authoredOnlySupportGrabs|rockAuthoredOnlyEquippedWeaponSupportGrabsEnabled' 'Authored-only equipped-weapon support policy must not leak into loose grabs, dynamic hands, touch targets, or shoulder stash.'
}
...
Require-Text 'src/physics-interaction/weapon/AuthoredPrimaryFiringGrip.cpp' 'rightPositionOnlyHandWeaponLocal\s*=[\s\S]{0,300}invertTransform\(solvedWeaponWorld\)[\s\S]{0,160}trackedHandWorld[\s\S]{0,800}cachedPositionOnlyGripWeaponLocal[\s\S]{0,500}authoredGripWeaponLocal[\s\S]{0,900}publishPositionOnlyHold[\s\S]{0,300}currentRevision\(\)' 'The final equipped position-only pose must publish a separately validated, hFRIK-revision-bound physical-hand relation for loose placement.'
Require-Text 'src/physics-interaction/weapon/AuthoredWeaponGripLibrary.cpp' 'rightPositionOnlyHandWeaponLocal[\s\S]{0,500}positionOnlyFrikOffsetRevision[\s\S]{0,500}hasRightPositionOnlyHandWeaponLocal[\s\S]*publishPositionOnlyHold\([\s\S]{0,1200}entry\.captureSequence\s*!=\s*authoredCaptureSequence[\s\S]{0,500}entry\.rightPositionOnlyHandWeaponLocal[\s\S]{0,300}entry\.positionOnlyFrikOffsetRevision\s*=\s*frikOffsetRevision[\s\S]{0,200}entry\.hasRightPositionOnlyHandWeaponLocal\s*=\s*true' 'The loose-placement relation must remain separate from the authored wrist and bind to the exact authored capture and hFRIK offset revision.'
...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'tryBuildMirroredLeftFiringHandWeaponLocalImpl[\s\S]{0,1800}if\s*\(!applyHandlingTrim\)[\s\S]{0,500}mirrorOppositeHandFrame\([\s\S]{0,200}canonicalRightHandWeaponLocal[\s\S]{0,250}return isFiniteTransform' 'Equipped left firing must mirror authored weapon-space hand truth before any legacy loose-hold driver logic.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'tryBuildMirroredRightSupportHandWeaponLocalImpl[\s\S]{0,900}mirrorOppositeHandFrame\(leftHandWeaponLocal\)' 'Opposite authored support must mirror directly in weapon space without controller-driver calibration.'
...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'solveLeftFiringWeaponCarry[\s\S]{0,4200}applyExternalHandWorldTransform[\s\S]{0,1800}applyWeaponVisualAuthority\(weaponNode, weaponTargetWorld\)' 'Left one-hand carry must publish the authored wrist before the final position-only weapon frame.'
...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'for \(const bool isLeft[\s\S]{0,300}hasVisualAuthorityForHand\(isLeft\)[\s\S]{0,500}tryResolvePhysicalHandInputFrame\(isLeft, physicalInput\)[\s\S]{0,400}currentHandWorld\s*=\s*physicalInput\.handWorld' 'Every ROCK-owned hand presentation must keep readback state on the physical damped-driver wrist.'
