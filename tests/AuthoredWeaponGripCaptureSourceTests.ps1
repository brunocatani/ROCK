...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'authoredInteractionAcquisitionValid\s*=\s*decision\.acquisitionSource\s*==[\s\S]*PhysicalContact\s*\|\|[\s\S]*ProximityProbe[\s\S]*shouldUseAuthoredSupportGrip\([\s\S]*authored_support_grab_policy::select\([\s\S]*Selection::Authored[\s\S]*Selection::Reject[\s\S]*return selectionDecision\.selection;[\s\S]*tryGetSupportGripEvidenceView\(decision\.bodyId, weaponNode, evidenceView\)' 'Authored selection must precede the mesh solver, while pending or missed valid authored acquisition rejects before dynamic fallback.'
...
Require-Text 'src/physics-interaction/weapon/AuthoredSupportGrabPolicy.h' 'providerPartAuthorityActive[\s\S]*Selection::ProviderDynamic[\s\S]*authoredCaptureEligible[\s\S]*Selection::Authored[\s\S]*!input\.modeEnabled[\s\S]*Selection::DynamicUnrestricted[\s\S]*Capability::Unavailable[\s\S]*Selection::DynamicFallback[\s\S]*Selection::Reject' 'Exact provider targets must override ROCK, followed by authored, mode-off dynamic, qualified fallback, and fail-closed rejection.'
