...
int main()
{
    ...
    constexpr AuthoredSupportHandVisualAnchorInput pumpVisualAnchorEligible{
        .authoredSupportGrip = true,
        .pumpSemantic = true,
        .sourceRootValid = true,
        .sourceRootBelongsToWeapon = true,
        .weaponGenerationMatches = true,
        .sourceWorldValid = true,
        .authoredHandWorldValid = true,
    };
    static_assert(shouldCaptureAuthoredSupportHandVisualAnchor(
        pumpVisualAnchorEligible));
    static_assert([=] {
        auto input = pumpVisualAnchorEligible;
        input.pumpSemantic = false;
        return !shouldCaptureAuthoredSupportHandVisualAnchor(input);
    }());
    static_assert([=] {
        auto input = pumpVisualAnchorEligible;
        input.sourceRootIsWeaponRoot = true;
        return !shouldCaptureAuthoredSupportHandVisualAnchor(input);
    }());
    static_assert([=] {
        auto input = pumpVisualAnchorEligible;
        input.sourceRootBelongsToWeapon = false;
        return !shouldCaptureAuthoredSupportHandVisualAnchor(input);
    }());
    static_assert([=] {
        auto input = pumpVisualAnchorEligible;
        input.weaponGenerationMatches = false;
        return !shouldCaptureAuthoredSupportHandVisualAnchor(input);
    }());

    constexpr AffineTransform weaponWorldAtCapture{ 2.0f, 100.0f };
    constexpr AffineTransform pumpWorldAtCapture{ 2.0f, 120.0f };
    constexpr AffineTransform authoredSupportHandWorldAtCapture{ 2.0f, 130.0f };
    constexpr auto handVisualPumpLocal =
        captureAuthoredSupportHandVisualSourceLocal(
            pumpWorldAtCapture,
            authoredSupportHandWorldAtCapture,
            compose,
            invert);
    static_assert(handVisualPumpLocal.scale == 1.0f);
    static_assert(handVisualPumpLocal.translate == 5.0f);
    constexpr AffineTransform animatedPumpWorld{ 2.0f, 140.0f };
    constexpr auto animatedSupportHandWorld =
        resolveAuthoredSupportHandVisualWorld(
            animatedPumpWorld,
            handVisualPumpLocal,
            compose);
    static_assert(animatedSupportHandWorld.scale == 2.0f);
    static_assert(animatedSupportHandWorld.translate == 150.0f);

    // Pump-only visual motion must not alter the weapon-rooted support pivot.
    constexpr AffineTransform authoredPhysicalGripInWeapon{ 1.0f, 15.0f };
    constexpr auto physicalGripWorldBeforePumpMotion = compose(
        weaponWorldAtCapture,
        authoredPhysicalGripInWeapon);
    constexpr auto physicalGripWorldAfterPumpMotion = compose(
        weaponWorldAtCapture,
        authoredPhysicalGripInWeapon);
    static_assert(
        physicalGripWorldBeforePumpMotion.translate ==
        physicalGripWorldAfterPumpMotion.translate);
    static_assert(
        animatedSupportHandWorld.translate !=
        authoredSupportHandWorldAtCapture.translate);
