...
int main()
{
    ...
    {
        // Skeleton hand-seat mirror: weapon side-plane reflection outside,
        // hand-basis Z reflection inside. An identity right seat maps to the
        // 180-degree Y turn of the mirrored left bone basis.
        TestTransform rightSeat =
            rock::transform_math::makeIdentityTransform<TestTransform>();
        rightSeat.translate = { 2.8f, -6.6f, 0.9f };
        const TestTransform identityLeftSeat =
            rock::hand_seat_mirror_math::
                mirrorHandWeaponLocalAcrossWeaponSidePlane(rightSeat);
        TestTransform expectedIdentityLeftSeat =
            rock::transform_math::makeIdentityTransform<TestTransform>();
        expectedIdentityLeftSeat.rotate.entry[0][0] = -1.0f;
        expectedIdentityLeftSeat.rotate.entry[2][2] = -1.0f;
        expectedIdentityLeftSeat.translate = { -2.8f, -6.6f, 0.9f };
        ok &= expectTransformNear(
            "skeleton seat mirror of identity seat",
            identityLeftSeat,
            expectedIdentityLeftSeat);

        rightSeat.rotate = makeAxisAngleRotation(
            rock::weaponSolverNormalize(
                TestVector3{ 0.3f, -0.7f, 0.5f }),
            47.0f);
        rightSeat.translate = { 1.2f, -17.2f, 5.1f };
        const TestTransform leftSeat =
            rock::hand_seat_mirror_math::
                mirrorHandWeaponLocalAcrossWeaponSidePlane(rightSeat);
        const auto& seatRotation = leftSeat.rotate.entry;
        const float seatDeterminant =
            seatRotation[0][0] *
                (seatRotation[1][1] * seatRotation[2][2] -
                    seatRotation[1][2] * seatRotation[2][1]) -
            seatRotation[0][1] *
                (seatRotation[1][0] * seatRotation[2][2] -
                    seatRotation[1][2] * seatRotation[2][0]) +
            seatRotation[0][2] *
                (seatRotation[1][0] * seatRotation[2][1] -
                    seatRotation[1][1] * seatRotation[2][0]);
        ok &= expectNear(
            "skeleton seat mirror remains a proper rotation",
            seatDeterminant,
            1.0f);
        ok &= expectVectorNear(
            "skeleton seat mirror reflects translation across weapon X",
            leftSeat.translate,
            TestVector3{ -1.2f, -17.2f, 5.1f });
        ok &= expectNear(
            "skeleton seat mirror keeps scale",
            leftSeat.scale,
            1.0f);
        ok &= expectTransformNear(
            "skeleton seat mirror is an involution",
            rock::hand_seat_mirror_math::
                mirrorHandWeaponLocalAcrossWeaponSidePlane(leftSeat),
            rightSeat);

        // Hand bases correspond with only Z flipped: the left hand's +X and
        // +Y land on the weapon-X reflection of the right hand's, its +Z on
        // the reflection of the right hand's -Z.
        const TestVector3 rightX = rock::transform_math::localVectorToWorld(
            rightSeat, TestVector3{ 1.0f, 0.0f, 0.0f });
        const TestVector3 rightY = rock::transform_math::localVectorToWorld(
            rightSeat, TestVector3{ 0.0f, 1.0f, 0.0f });
        const TestVector3 rightZ = rock::transform_math::localVectorToWorld(
            rightSeat, TestVector3{ 0.0f, 0.0f, 1.0f });
        ok &= expectVectorNear(
            "skeleton seat mirror maps hand +X",
            rock::transform_math::localVectorToWorld(
                leftSeat, TestVector3{ 1.0f, 0.0f, 0.0f }),
            TestVector3{ -rightX.x, rightX.y, rightX.z });
        ok &= expectVectorNear(
            "skeleton seat mirror maps hand +Y",
            rock::transform_math::localVectorToWorld(
                leftSeat, TestVector3{ 0.0f, 1.0f, 0.0f }),
            TestVector3{ -rightY.x, rightY.y, rightY.z });
        ok &= expectVectorNear(
            "skeleton seat mirror maps hand +Z to reflected -Z",
            rock::transform_math::localVectorToWorld(
                leftSeat, TestVector3{ 0.0f, 0.0f, 1.0f }),
            TestVector3{ rightZ.x, -rightZ.y, -rightZ.z });

        // The user-tuned palm pivots R(6,-2,+0.2) / L(6,-2,-0.2) therefore
        // land on mirrored weapon points without any position anchoring.
        const TestVector3 rightPalm = rock::transform_math::localPointToWorld(
            rightSeat, TestVector3{ 6.0f, -2.0f, 0.2f });
        ok &= expectVectorNear(
            "skeleton seat mirror places the left palm pivot on the reflected right palm point",
            rock::transform_math::localPointToWorld(
                leftSeat, TestVector3{ 6.0f, -2.0f, -0.2f }),
            TestVector3{ -rightPalm.x, rightPalm.y, rightPalm.z });
    }
