...
int main()
{
...

    {
        ...
        TestTransform rightAuthoredHandInWeapon =
            rock::transform_math::makeIdentityTransform<TestTransform>();
        rightAuthoredHandInWeapon.rotate = makeAxisAngleRotation(
            rock::weaponSolverNormalize(
                TestVector3{ 0.6f, -0.2f, 0.7f }),
            37.0f);
        rightAuthoredHandInWeapon.translate = { 2.9f, -6.8f, -2.3f };
        rightAuthoredHandInWeapon.scale = 1.15f;
        const TestTransform leftAuthoredHandInWeapon =
            rock::left_firing_position_only_math::
                mirrorOppositeHandFrame(
                    rightAuthoredHandInWeapon);
        ok &= expectVectorNear(
            "authored hand mirror changes only weapon lateral position",
            leftAuthoredHandInWeapon.translate,
            TestVector3{ -2.9f, -6.8f, -2.3f });
        ok &= expectTransformNear(
            "authored hand mirror is an exact bilateral involution",
            rock::left_firing_position_only_math::
                mirrorOppositeHandFrame(
                    leftAuthoredHandInWeapon),
            rightAuthoredHandInWeapon);
    ...
    {
        TestTransform primaryWandWorld =
            rock::transform_math::makeIdentityTransform<TestTransform>();
        primaryWandWorld.translate = { -32320.0f, 31100.0f, 4330.0f };
        primaryWandWorld.rotate = makeAxisAngleRotation(
            TestVector3{ 0.2f, 0.7f, 0.4f },
            41.0f);

        TestTransform kickParentInPrimaryWand =
            rock::transform_math::makeIdentityTransform<TestTransform>();
        kickParentInPrimaryWand.translate = { 0.0f, -4.0f, 1.0f };
        kickParentInPrimaryWand.rotate = makeAxisAngleRotation(
            TestVector3{ 0.0f, 0.0f, 1.0f },
            7.0f);
        const TestTransform nativeKickParentWorld =
            rock::transform_math::composeTransforms(
                primaryWandWorld,
                kickParentInPrimaryWand);

        TestTransform offhandWandWorld = primaryWandWorld;
        offhandWandWorld.translate = { -32315.0f, 31096.0f, 4332.0f };
        offhandWandWorld.rotate = makeAxisAngleRotation(
            TestVector3{ -0.2f, 0.7f, 0.4f },
            -41.0f);

        TestTransform nativeKickLocal =
            rock::transform_math::makeIdentityTransform<TestTransform>();
        nativeKickLocal.translate = { 0.0f, -2.7248f, 0.0f };

        TestTransform resolvedParentWorld{};
        TestTransform resolvedKickLocal{};
        ok &= expectTrue(
            "native-primary recoil frame resolves without a world delta",
            rock::weapon_recoil_authority_math::tryResolveKickFrame(
                nativeKickLocal,
                ...
                resolvedKickLocal));
        ok &= expectTransformNear(
            "native-primary recoil keeps the native kick parent",
            resolvedParentWorld,
            nativeKickParentWorld);
        ok &= expectTransformNear(
            "native-primary recoil keeps the native local kick",
            resolvedKickLocal,
            nativeKickLocal);

        TestTransform targetInKickParent =
            rock::transform_math::makeIdentityTransform<TestTransform>();
        targetInKickParent.translate = { 6.0f, 2.0f, -3.0f };
        targetInKickParent.rotate = makeAxisAngleRotation(
            TestVector3{ 1.0f, 0.0f, 0.0f },
            13.0f);
        const TestTransform targetWorld =
            rock::transform_math::composeTransforms(
                resolvedParentWorld,
                targetInKickParent);
        TestTransform kickedTargetWorld{};
        ok &= expectTrue(
            "large-world recoil applies through the compact parent frame",
            rock::weapon_recoil_authority_math::
                tryApplyKickToWorldTarget(
                    ...
                    targetWorld,
                    kickedTargetWorld));
        TestTransform kickedTargetInParent{};
        ok &= expectTrue(
            "large-world recoil result resolves back into its parent",
            rock::weapon_recoil_authority_math::
                tryResolveFrameLocalTransform(
                    resolvedParentWorld,
                    kickedTargetWorld,
                    kickedTargetInParent));
        const TestTransform expectedKickedTargetInParent =
            rock::transform_math::composeTransforms(
                resolvedKickLocal,
                targetInKickParent);
        ok &= expectVectorNear(
            "large-world recoil preserves the authored local axis",
            kickedTargetInParent.translate,
            expectedKickedTargetInParent.translate,
            0.005f);

        TestTransform offhandParentWorld{};
        TestTransform offhandKickLocal{};
        ok &= expectTrue(
            "native-offhand recoil frame resolves from the wand pair",
            rock::weapon_recoil_authority_math::tryResolveKickFrame(
                nativeKickLocal,
                nativeKickParentWorld,
                primaryWandWorld,
                ...
                offhandParentWorld,
                offhandKickLocal));
        const TestTransform expectedOffhandParentWorld =
            rock::transform_math::composeTransforms(
                offhandWandWorld,
                rock::weapon_recoil_authority_math::
                    mirrorLocalAcrossSagittal(
                        kickParentInPrimaryWand));
        ok &= expectTransformNear(
            "native-offhand recoil mirrors only the compact parent relation",
            offhandParentWorld,
            expectedOffhandParentWorld);
        ok &= expectTransformNear(
            "native-offhand recoil mirrors the local kick",
            offhandKickLocal,
            rock::weapon_recoil_authority_math::
                mirrorLocalAcrossSagittal(nativeKickLocal));
    }
