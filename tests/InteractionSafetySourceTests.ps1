...
Require-Text $grabInput `
    'if\s*\(_pendingForceGrabCommits\[handIndex\]\.active\)\s*\{[\s\S]*?grab_input_intent_policy::reset\(inputIntentState\);[\s\S]*?cancelPeerHeldJoinRetry[\s\S]*?autoEquipState\s*=\s*\{\};[\s\S]*?clearGameplayCandidatesForHand[\s\S]*?_softContactRuntime\.clearHandForStrongerOwner[\s\S]*?clearSelectionState\(false\);[\s\S]*?return;' `
    'A pending force-grab must reserve normal input, retry, auto-equip, gameplay-candidate, soft-contact, and selection ownership for its hand.'
Require-Text $grabInput `
