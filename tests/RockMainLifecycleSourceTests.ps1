...
Require-Pattern 'onFrameUpdate\(\)[\s\S]*processPendingConfigReload\(\)[\s\S]*setGameplayInputAllowed\(gameplayInputAllowed\)[\s\S]*ensurePhysicsInteractionForReadySkeleton\(\)' 'Frame update must call lazy lifecycle recovery after config/input gating.'
Require-Pattern 'if \(s_physicsInteraction \|\| !g_rockConfig\.rockEnabled\)[\s\S]{0,120}return;[\s\S]*createPhysicsInteraction\(\)' 'Lifecycle recovery must create PhysicsInteraction when no instance exists.'
