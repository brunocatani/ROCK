...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'tryResolveDescendantWorldTransform\(\s*packageDriveNode,\s*packageWorld,\s*instance\.sourceNode,\s*sourceWorld,\s*instance\.generatedSourceFrameCorrectionValid' 'Generated weapon bodies must follow descendant locals composed from the current weapon root and the validated post-undraw correction.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.h' 'struct GeneratedRecaptureDiagnosticSource[\s\S]{0,900}sourceInWeapon[\s\S]{0,500}weaponLocalPoints[\s\S]{0,500}weaponLocalTriangles[\s\S]{0,500}sourceLocalPoints[\s\S]{0,300}sourceLocalTriangles' 'The pre-sheath authority must own value snapshots of the correct weapon-local and source-local geometry.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'frameAuthorityEligible[\s\S]{0,900}sourcePointerStable[\s\S]{0,300}rootPointerStable[\s\S]{0,300}sourceGeometryStable[\s\S]{0,300}sourceScaleStable[\s\S]{0,300}frameCorrectionAvailable' 'Post-undraw frame restoration must fail closed unless the exact live source tree and raw source-local geometry remain stable.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'makePostUndrawFrameCorrection\(\s*row\.baseline->sourceInWeapon,\s*row\.current->sourceInWeapon\)' 'Shoulder frame restoration must remove the current presentation offset relative to the captured correct source frame.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'current\.postUndrawFrameCorrectionValid\s*=\s*true[\s\S]{0,500}current\.localPointsGame\s*=\s*baseline\.weaponLocalPoints[\s\S]{0,500}current\.localTrianglesGame\s*=\s*baseline\.weaponLocalTriangles' 'A validated shoulder recapture must repair both live source driving and weapon-local compound geometry.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'current\.sourceLocalPointsGame\s*=\s*baseline\.sourceLocalPoints[\s\S]{0,500}current\.sourceLocalTrianglesGame\s*=\s*baseline\.sourceLocalTriangles[\s\S]{0,500}current\.sourceNodeScale\s*=\s*baseline\.sourceNodeScale' 'A validated shoulder recapture must restore the exact pre-sheath source-local hull data.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'tryBuildSupportGripEvidenceView[\s\S]{0,1200}instance\.generatedSourceFrameCorrectionValid' 'Support-grip surface evidence must use the same corrected source frame as the physical bodies.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'findCurrentWeaponSurfaceNearPoints[\s\S]{0,4200}instance\.generatedSourceFrameCorrectionValid' 'Weapon surface queries must use the same corrected source frame as the physical bodies.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'tryFindInteractionContactNearPoint[\s\S]{0,1500}instance\.generatedSourceFrameCorrectionValid' 'Weapon-part probes must use the same corrected source frame as the physical bodies.'
