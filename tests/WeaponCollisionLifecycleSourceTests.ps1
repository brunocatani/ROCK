...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.h' 'struct GeneratedRecaptureDiagnosticSource[\s\S]{0,900}sourceInWeapon[\s\S]{0,500}weaponLocalPoints[\s\S]{0,500}weaponLocalTriangles' 'The pre-sheath authority must own value snapshots of the correct source frame and weapon-local geometry.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'frameAuthorityEligible[\s\S]{0,900}sourcePointerStable[\s\S]{0,300}rootPointerStable[\s\S]{0,300}sourceScaleStable[\s\S]{0,300}frameCorrectionAvailable' 'Post-undraw frame restoration must fail closed unless the exact live source tree and source-local geometry remain stable.'
