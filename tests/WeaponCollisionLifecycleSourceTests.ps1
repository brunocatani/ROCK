...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'getEquippedWeaponIdentityKey\(&observedIdentityKey,[\s\S]{0,7000}if \(rebuildRequired\)[\s\S]{0,420}getWeaponVisualCompositionKey\(weaponNode,\s*visualKeyStats\)' 'Weapon visual traversal must stay behind the identity and drive rebuild gate.'
...
Reject-Text 'data/config/ROCK_example.ini' 'WeaponCollisionMaxSourceDistance' 'The superseded origin-distance collider template settings must stay removed.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'generationDrivenRebuild[\s\S]{0,1200}maybeRunWeaponOmodReconciliation\(weaponNode,\s*observedKey,\s*true\)[\s\S]{0,900}reconciliation\.sceneEnriched[\s\S]{0,900}clearPendingWeaponVisualRebuild\(\);[\s\S]{0,300}return;' 'Initial weapon generation must complete OMOD scene enrichment for the observed equipped identity before beginning collider source stabilization.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'reconciliation\.sceneEnriched[\s\S]{0,900}return;[\s\S]{0,300}reconciliation\.ran[\s\S]{0,300}_omodPrebuildReconciliationEquippedKey\s*=\s*observedKey' 'A mutating OMOD pre-build pass must repeat until a later non-mutating pass confirms convergence.'
foreach ($path in @('src/RockConfig.h', 'src/RockConfig.cpp', 'data/config/ROCK_example.ini')) {
    Reject-Text $path 'rockWeaponOmodSelfHealEnabled|bDebugWeaponOmodSelfHeal' "$path must not expose mandatory OMOD self-heal as configuration."
}
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'generationDrivenRebuild\s*&&[\s\S]{0,100}!omodPrebuildReconciliationCurrent[\s\S]{0,180}maybeRunWeaponOmodReconciliation\(weaponNode,\s*observedKey,\s*true\)' 'Every initial weapon generation must run mandatory pre-build OMOD reconciliation.'
Reject-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'selfHealEnabled|rockWeaponOmodSelfHealEnabled|bDebugWeaponOmodSelfHeal' 'Mandatory OMOD self-heal must not retain an enable branch or configuration lookup.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'findStaleRockOmodEnrichmentContainers\(weaponNode,\s*activeOmodFormIds\)[\s\S]*removeStaleRockOmodEnrichmentContainers\(staleEnrichmentContainers\)[\s\S]*if \(!selfHealCandidates\.empty\(\)\)' 'Stale enrichment cleanup and bounded candidate recovery must remain unconditional functional paths.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'visitGeneratedWeaponMeshRootCandidates\([\s\S]{0,500}WEAPON_ANIM_NODE_DUMP_MAX_VISITED_NODES[\s\S]{0,900}if \(emitCoverageDiagnostics\) \{[\s\S]{0,1200}absoluteSceneRoot' 'Functional OMOD repair must scan only collider-consumable weapon roots; world-wide roots belong only to explicit diagnostics.'
Require-Text 'src/physics-interaction/weapon/WeaponOmodSceneScan.cpp' 'visitedNodeAddresses\.insert\(address\)\.second[\s\S]*scanTreeRecursive' 'Overlapping OMOD roots must share one visited-node authority instead of rescanning nested scene subtrees.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'std::unordered_set<std::uintptr_t> visitedNodeAddresses[\s\S]{0,500}weapon_omod_scene_scan::scanTree\([\s\S]{0,300}visitedNodeAddresses' 'The reconciliation caller must reuse one visited-node set across every selected root.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'healTargetNode\s*=\s*weaponNode\s*\?\s*weaponNode->IsNode\(\)' 'Functional OMOD recovery must target the exact visual root used for collider capture before considering diagnostic token fallbacks.'
Reject-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'scanOmodAuditTreeRecursive|omodAuditNameContainsToken|makeOmodAuditModelToken' 'OMOD tree scanning and token matching must stay extracted from the weapon collision monolith.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'kPostBuildSelfHealCheckFrames\s*=\s*90[\s\S]{0,500}!emitCoverageDiagnostics\s*&&[\s\S]{0,100}_omodReconciliationRunIndex\s*!=\s*0' 'Self-heal must own one fixed post-build reconciliation pass independent of the diagnostic interval.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'if \(emitCoverageDiagnostics\) \{[\s\S]{0,800}OMOD-AUDIT begin' 'Coverage audit logging must remain behind its own diagnostic gate.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'retireWeaponBodyPayload[\s\S]*RETIRED_GENERATED_WEAPON_BODY_GRACE_STEPS' 'Retired generated weapon bodies must wait a bounded number of physics steps before native neutralization.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'serviceRetiredWeaponBodies\([\s\S]{0,120}currentWorld[\s\S]*retired\.bodyPayload\.retiredHknpWorld\s*!=\s*currentWorld[\s\S]*BethesdaPhysicsBody::quarantineRetiredPayload' 'Retired generated weapon bodies must enter quarantine only from the exact world that retired them.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'observeCustomGrabAuthorityAfterSolve[\s\S]*_weaponCollision\.serviceRetiredWeaponBodies\(world\);' 'The physics after-solve callback must service retired generated weapon bodies through the current native world.'
...
# must use grace-window retirement followed by process-lifetime wrapper
# quarantine, never immediate destroy().
...
Require-Text 'src/physics-interaction/native/BethesdaPhysicsBody.cpp' 'retireDeferred\(void\* bhkWorld\)[\s\S]{0,600}retireFromWorld\(bhkWorld, payload\)[\s\S]{0,600}remainingPhysicsSteps = kRetiredDeferredBodyGraceSteps' 'Deferred body retirement must remove from world then hold a bounded physics-step grace before native neutralization.'
Require-Text 'src/physics-interaction/native/BethesdaPhysicsBody.cpp' 'serviceRetiredDeferredPayloads\([\s\S]{0,120}currentWorld[\s\S]*retired\.payload\.retiredHknpWorld\s*!=\s*currentWorld[\s\S]*quarantineRetiredPayload\(retired\.payload\)' 'Deferred body retirement must enter quarantine only from the exact world that retired it.'
Require-Text 'src/physics-interaction/native/BethesdaPhysicsBody.cpp' 'kMaxProcessLifetimeCollisionObjectTombstones[\s\S]{0,600}std::array<void\*,\s*kMaxProcessLifetimeCollisionObjectTombstones>[\s\S]*s_retirementQuarantineExhausted' 'Retired collision-object addresses must have a fixed process-lifetime owner with fail-closed capacity state.'
Require-Text 'src/physics-interaction/native/BethesdaPhysicsBody.cpp' 'quarantineRetiredPayload[\s\S]{0,2600}detachAndReleaseNiNode[\s\S]{0,2000}kCollisionObject_PhysSystemPtr[\s\S]{0,2400}InterlockedExchangePointer[\s\S]{0,500}releaseRefCounted\(physicsSystem\)[\s\S]{0,800}s_collisionObjectTombstones' 'Quarantine must neutralize node and physics-system ownership while retaining the collision-object address.'
Require-Text 'src/physics-interaction/native/BethesdaPhysicsBody.cpp' 's_retirementQuarantineExhausted\.load[\s\S]{0,500}Generated-body creation rejected' 'Quarantine capacity exhaustion must fail closed before allocating another generated body.'
Reject-Text 'src/physics-interaction/native/BethesdaPhysicsBody.cpp' 'releaseRetiredPayload' 'Live-world retirement must not recover collision-object addresses that native readers can retain.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'observeCustomGrabAuthorityAfterSolve[\s\S]*BethesdaPhysicsBody::serviceRetiredDeferredPayloads\(world\);' 'The physics after-solve callback must service deferred collider retirements through the current native world.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'tryFindCurrentWeaponSurfaceNearPoint\([\s\S]*findCurrentWeaponSurfaceNearPoints\([\s\S]*currentGeneration = getCurrentWeaponGenerationKey\(\)[\s\S]*activeWeaponBodies\(\)[\s\S]*generatedSourceLocalTrianglesGame[\s\S]*generatedLocalTrianglesGame[\s\S]*pointAabbDistanceSquared\([\s\S]*closestPointOnTriangleToPoint\([\s\S]*witness\.weaponGenerationKey = currentGeneration' 'Authored-seat validation must broadphase against the current body bank, confirm exact cached-triangle proximity, and publish the matching weapon generation.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'pendingGeneratedWeaponBuildMatches\(\s*observedKey,\s*observedOwnershipKey,\s*weaponNode,\s*observedFormID\)' 'Pending generated weapon creation must match equipped identity, ownership, scene root, and form, not reload visual witnesses.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'rebuildRequired\s*=\s*driveRequestedRebuild\s*\|\|\s*workbenchExitRequested\s*\|\|\s*keyChanged\s*\|\|[\s\S]{0,120}ownershipKeyChanged[\s\S]{0,120}activeRootChanged[\s\S]{0,120}missingBodies' 'Workbench exit must be an explicit rebuild gate alongside equip identity, ownership, root, drive, and missing-body gates.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'pendingInvalidated\s*=\s*driveRequestedRebuild\s*\|\|\s*workbenchExitRequested\s*\|\|[\s\S]{0,180}!pendingGeneratedWeaponBuildMatches\(\s*observedKey,\s*observedOwnershipKey,\s*weaponNode,\s*observedFormID\)' 'A consumed workbench-exit request must restart a matching staged create rather than silently keeping old sources.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'updateHandCollisions\(frame\);[\s\S]{0,500}if \(rightHandWeaponAuthorityActive\)[\s\S]{0,160}suppressRightHandCollisionForDominantWeapon\(hknp\)' 'New keyframed hand bodies must receive active dominant-weapon suppression before the next physics step.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'releaseStaleGeneratedHandSuppressionLeases\([\s\S]*handContainsBody[\s\S]*globalCollisionSuppressionRegistry\(\)[\s\S]*\.release\(' 'Hand-body rebuilds must release stale body-ID leases before filling the fixed-capacity suppression set with current bodies.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'tryResolveDescendantLocalTransform[\s\S]{0,2600}reversePath\[--pathLength\]->local' 'Weapon collider transforms must compose the bounded local parent path instead of trusting stale descendant world transforms.'
...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' '_state\s*!=\s*TwoHandedState::PrimaryOnly[\s\S]{0,320}if\s*\(currentWeaponGenerationKey\s*==\s*0\)[\s\S]{0,1100}return\s+true;' 'Only PrimaryOnly ownership may survive the pre-publication collider window.'
