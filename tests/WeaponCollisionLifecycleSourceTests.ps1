...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'generationDrivenRebuild[\s\S]{0,1200}maybeRunWeaponOmodCoverageAudit\(weaponNode,\s*true\)[\s\S]{0,900}auditResult\.sceneEnriched[\s\S]{0,900}clearPendingWeaponVisualRebuild\(\);[\s\S]{0,300}return;' 'Initial weapon generation must complete OMOD scene enrichment before beginning collider source stabilization.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.h' 'getCurrentEquippedWeaponIdentityKey\(\)\s+const\s*\{\s*return\s+_observedEquippedWeaponIdentityKey' 'Manual weapon ownership must be able to use current equipped-item identity before replaceable collision generations publish.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'preserving manual ownership across collision rebuild[\s\S]*?tryRebindPartGripToCurrentGeneration' 'Two-handed ownership must survive same-equipped-weapon collider rebuilds and rebind captured part evidence.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'primaryOwnershipGenerationKey\s*=\s*currentWeaponGenerationKey\s*!=\s*0\s*\?\s*currentWeaponGenerationKey\s*:\s*currentEquippedWeaponIdentityKey' 'Primary-only ownership must use equipped identity as a provisional key while full weapon colliders build.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' '_state\s*!=\s*TwoHandedState::PrimaryOnly[\s\S]{0,180}if\s*\(currentWeaponGenerationKey\s*==\s*0\)[\s\S]{0,260}return\s+true;' 'Only PrimaryOnly ownership may survive the pre-publication collider window.'
