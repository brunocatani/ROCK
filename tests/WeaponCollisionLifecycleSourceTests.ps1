...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'selectMissingWeaponVisualDriveRootCandidate' 'Missing first-person Weapon roots must still allow equip-slot visual package roots to drive generated collision.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'First-person weapon visual node absent - using equip-slot visual package root' 'Missing-root diagnostics must state when ROCK is using an equip-slot package root instead of the Weapon label.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'selectGeneratedWeaponPackageDriveRoot\(candidates,\s*weaponNode\)' 'Generated source scans must select a package drive root from candidates instead of hard-requiring the Weapon label.'
Reject-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'findGeneratedWeaponShapeSources[\s\S]{0,180}if \(!weaponNode\)[\s\S]{0,120}return 0;' 'Generated weapon source extraction must not stop before equip-slot candidates when the first-person Weapon label is absent.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'weaponNode \|\| _weaponCollision\.hasWeaponBody\(\)[\s\S]{0,180}updateBodiesFromCurrentSourceTransforms' 'Generated weapon bodies driven by an equip-slot root must keep moving even when the Weapon label is absent.'
