...
Require-Text 'src/physics-interaction/weapon/WeaponOmodAuditPolicy.h' 'shouldAttemptWholeModelAttach[\s\S]*NativeAttachmentIdentityEvidence::Absent[\s\S]*!templateSignatureIsPresent\(matchedDistinctMeshCount,\s*distinctTemplateMeshCount\)' 'Native whole-model recovery requires exact engine identity absence in addition to an incoherent physical signature.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'queryNativeOmodAttachmentIdentity[\s\S]*BSFadeNode[\s\S]*0x3D930[\s\S]*hasAttachedOmodIdentity' 'OMOD recovery must use the FO4VR engine caller''s exact form-and-rank identity guard before native attachment.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'resolveOmodPhysicalCoverage[\s\S]*canonicalConnectPointForAttachPoint[\s\S]*evidenceRoots[\s\S]*tokenSlot[\s\S]*tryAttach3DRecurse[\s\S]*resolveOmodPhysicalCoverage' 'OMOD presence must cover every canonical slot plus exact in-instance token branches and must be recomputed after native mutation.'
