...
Require-Text 'src/physics-interaction/weapon/WeaponOmodAuditPolicy.h' 'requiresDurableAnchorRecovery[\s\S]*shouldAttemptWholeModelAttach[\s\S]*matchedDistinctMeshCount\s*==\s*0' 'Only a wholly absent physical branch may use native whole-model attachment; partial branches must recover only the missing housing.'
