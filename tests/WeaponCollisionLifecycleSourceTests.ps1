...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'removeStaleRockOmodEnrichmentContainers\(weaponNode,\s*activeOmodFormIds\)[\s\S]*if \(!selfHealCandidates\.empty\(\)\)' 'Stale enrichment cleanup and bounded candidate recovery must remain unconditional functional paths.'
