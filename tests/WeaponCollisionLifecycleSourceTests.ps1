...
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'generatedWeaponCollisionActive[\s\S]{0,140}_weaponCollision\.hasWeaponBody\(\)[\s\S]{0,140}_weaponCollision\.getCurrentWeaponGenerationKey\(\)\s*!=\s*0' 'Dominant weapon authority must include retained generated weapon bodies, not only a live weapon visual node.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'rightHandWeaponAuthorityActive\s*=\s*weapon_two_handed_grip_math::rightHandDominantWeaponCollisionOwnsHand\([\s\S]{0,240}generatedWeaponCollisionActive' 'Right-hand weapon suppression must stay active across reload-null visual frames while allowing free detached-primary hands.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' '_twoHandedGrip\.isPrimaryDetached\(\)[\s\S]{0,180}resolveCachedEquippedWeaponFiringGripReference\(' 'FRIK firing-grip lookup must only run while primary-detached authority can consume it.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' '_equippedWeaponFiringGripReferenceCache\s*=\s*\{\};[\s\S]{0,120}primary-attached' 'Ordinary attached/equip frames must clear cached FRIK firing-grip roots instead of retaining stale visual pointers.'
