...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'tryBuildSupportGripEvidenceTriangles\(' 'Two-hand support grip must reuse cached weapon collision geometry evidence.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'sameEquippedIdentity[\s\S]{0,260}visualKeyChanged[\s\S]{0,260}WeaponRebuildVisualSourceUnavailableRetained' 'Same-identity visual-only source misses must retain live generated weapon bodies instead of forcing a missing-body rebuild loop.'
