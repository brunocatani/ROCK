...
Require-Text 'src/physics-interaction/weapon/WeaponInstanceWitnessRuntime.cpp' 'REL::RelocationID\(1191757,\s*2189025\)' 'Direct BGSObjectInstanceExtra AddMod mutations must be witnessed with the verified FO4VR relocation.'
Require-Text 'src/physics-interaction/weapon/WeaponInstanceWitnessRuntime.cpp' 'REL::RelocationID\(1136607,\s*2189027\)' 'Direct BGSObjectInstanceExtra RemoveMod mutations must be witnessed with the verified FO4VR relocation.'
Require-Text 'src/physics-interaction/weapon/WeaponInstanceWitnessRuntime.cpp' 'BGSObjectInstanceExtraAddModPostCall' 'Direct object-instance AddMod witness logs must identify the post-call mutation boundary.'
Require-Text 'src/physics-interaction/weapon/WeaponInstanceWitnessRuntime.cpp' 'BGSObjectInstanceExtraRemoveModPostCall' 'Direct object-instance RemoveMod witness logs must identify the post-call mutation boundary.'
Require-Text 'src/physics-interaction/weapon/WeaponInstanceWitnessRuntime.cpp' 'objectInstanceExtraAddress' 'Direct object-instance mutation signals must bind to the exact equipped BGSObjectInstanceExtra address.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'appendEquippedWeaponSlotNodeCandidates' 'Generated weapon source candidates must include read-only equip-slot visual package roots.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'readEquippedWeaponSlotNodeMatchInput[\s\S]{0,700}tryGetAuthoritativeEquippedWeaponIdentity' 'Equip-slot visual root matching must prefer the bounded authoritative equipped-instance witness before stale live equip data.'
Reject-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'selectMissingWeaponVisualDriveRootCandidate|First-person weapon visual node absent - using equip-slot visual package root' 'Equip-slot visual package roots must never replace the first-person weapon drive root.'
Reject-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'selectGeneratedWeaponPackageDriveRoot' 'Generated weapon collision must not select an equip-slot node as package drive root when Weapon is absent.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'findGeneratedWeaponShapeSources[\s\S]{0,220}if \(!weaponNode\)[\s\S]{0,160}return 0;' 'Generated weapon source extraction must require a real first-person weapon drive root.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'const bool rightHandWeaponEquipped = weaponNode != nullptr;' 'Existing weapon bodies must not keep the dominant-hand weapon state alive after unequip.'
Reject-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'weaponNode \|\| _weaponCollision\.hasWeaponBody\(\)' 'Existing weapon bodies must not substitute for an equipped first-person weapon node.'
Require-Text 'src/physics-interaction/contact/SoftContactRuntime.cpp' '!weaponNode \|\| !movableShape\.capsule\.valid \|\| !weaponCollision\.hasWeaponBody\(\)' 'Weapon soft contact must require the first-person weapon drive root.'
Require-Text 'src/physics-interaction/core/PhysicsInteractionProvider.inl' 'if \(!weaponNode\) \{' 'Provider weapon-contact queries must not treat stale generated bodies as an equipped weapon.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'PlayerEquipData\.slot\[' 'Generated weapon source diagnostics must identify first-person equip-data slot node candidates.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'ActorEquipData\.slot\[' 'Generated weapon source diagnostics must identify actor equip-data slot node candidates.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'slot\.node' 'Generated weapon source candidates must read native equip-slot node roots, not only the first-person Weapon label.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'parentAddr=\{:x\}' 'Generated weapon candidate diagnostics must expose parent pointers for stale attach-tree analysis.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'directChildren=\{\}' 'Generated weapon candidate diagnostics must expose direct child counts for stale attach-tree analysis.'
