...
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'rightWeaponContactProbeAllowed\s*=\s*weaponNode\s*!=\s*nullptr\s*&&\s*_twoHandedGrip\.primaryHandIsDetachedFromFiringGrip\(\)[\s\S]{0,160}consumeWeaponContactForHand\(false,\s*frame\.right,\s*rightWeaponContactProbeAllowed,\s*rightWeaponContact\)' 'Detached primary must sample right-hand equipped-weapon contact whenever it is detached from the firing grip.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' '_twoHandedGrip\.update\([\s\S]{0,140}leftWeaponContact,[\s\S]{0,80}rightWeaponContact,[\s\S]{0,260}providerInteractionState,[\s\S]{0,80}rightProviderInteractionState' 'TwoHandedGrip update must consume both left and right weapon contacts with separate provider whitelist state.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.h' 'PrimaryDetachedPartGrip[\s\S]*PrimaryDetachedPartOnly[\s\S]*primaryHandIsDetachedFromFiringGrip[\s\S]*primaryHandOwnsDetachedWeaponPartGrip' 'TwoHandedGrip must model detached primary weapon-part ownership separately from firing-grip ownership.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'tryStartDetachedPrimaryWeaponPartGrip[\s\S]*routeWeaponInteraction\(rightWeaponContact,\s*rightRuntimeState\)[\s\S]*tryBuildSupportGripEvidenceTriangles\(decision\.bodyId,\s*weaponNode,\s*triangles\)' 'Detached primary part grip must route the sampled right-hand weapon contact and reuse cached collision evidence.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'resolveDetachedPrimaryGripRoute[\s\S]*weaponLocalToWorld\(_primaryFiringGripWeaponLocal\.translate,\s*routeWeaponNode\)' 'Detached primary reattach distance must remain anchored to the firing grip, not the arbitrary grabbed weapon part.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'tryReattachPrimaryGrip[\s\S]*weaponLocalToWorld\(_primaryFiringGripWeaponLocal\.translate,\s*weaponNode\)[\s\S]*clearDetachedPrimaryWeaponPartGrip\(true\)' 'Reattaching the primary must clear arbitrary part-grip authority and restore the firing-grip local frame.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'setEquippedWeaponPrimaryDetached\(_twoHandedGrip\.primaryHandIsDetachedFromFiringGrip\(\)\)' 'Native trigger suppression must remain active while the primary hand grips a non-firing weapon part.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'canProcessNormalGrabInput\([\s\S]{0,180}_twoHandedGrip\.primaryHandIsDetachedFree\(\)' 'Normal right-hand grabbing must only be restored for the free detached primary state, not while it owns a weapon part.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'rightHandWeaponEquipped\s*&&\s*_twoHandedGrip\.primaryHandIsDetachedFree\(\)[\s\S]{0,140}rightHandWeaponAuthorityActive\s*=\s*false' 'Visible detached primary must restore right-hand generated colliders even while equipped weapon bodies remain live.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'rightHandWeaponAuthorityActiveAfterGrip\s*=\s*rightHandWeaponEquipped\s*\|\|\s*retainedWeaponCollisionActive[\s\S]{0,220}rightHandWeaponEquipped\s*&&\s*_twoHandedGrip\.primaryHandIsDetachedFree\(\)[\s\S]{0,140}rightHandWeaponAuthorityActiveAfterGrip\s*=\s*false' 'Post-update detached primary authority must keep right-hand collider restore in sync with support grip transitions.'
