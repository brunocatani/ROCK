...
Require-Text 'src/RockConfig.h' 'rockWeaponOmodSelfHealEnabled\s*=\s*true' 'OMOD self-heal must be enabled by the compiled production defaults.'
Require-Text 'data/config/ROCK_example.ini' 'bDebugWeaponOmodCoverageAudit\s*=\s*false[\s\S]{0,600}bDebugWeaponOmodSelfHeal\s*=\s*true' 'The example must keep coverage diagnostics disabled while functional OMOD self-heal remains enabled.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'generationDrivenRebuild\s*&&\s*!omodPrebuildReconciliationCurrent\s*&&[\s\S]{0,100}rockWeaponOmodSelfHealEnabled' 'Pre-build OMOD reconciliation must depend only on the functional self-heal setting.'
Reject-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'rockDebugWeaponOmodCoverageAudit\s*&&\s*g_rockConfig\.rockWeaponOmodSelfHealEnabled' 'Coverage diagnostics must not gate functional OMOD self-heal.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'emitCoverageDiagnostics\s*=\s*g_rockConfig\.rockDebugWeaponOmodCoverageAudit[\s\S]{0,180}selfHealEnabled\s*=\s*g_rockConfig\.rockWeaponOmodSelfHealEnabled' 'Reconciliation must read diagnostic and functional enablement as independent inputs.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'rebuildRequired\s*=\s*driveRequestedRebuild\s*\|\|\s*workbenchExitRequested\s*\|\|\s*keyChanged\s*\|\|\s*missingBodies' 'Workbench exit must be an explicit rebuild gate alongside equip, drive, and missing-body gates.'
