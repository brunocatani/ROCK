...
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'rightHandWeaponAuthorityActive\s*=\s*weapon_two_handed_grip_math::shouldSuppressPrimaryHandCollidersForWeapon\(\s*rightHandWeaponEquipped,\s*retainedWeaponCollisionActive,\s*_twoHandedGrip\.getPrimaryWeaponControlState\(\)' 'Right-hand weapon suppression must route through primary weapon-control ownership policy.'
Require-Text 'src/physics-interaction/weapon/WeaponSupport.h' 'shouldSuppressPrimaryHandCollidersForWeapon\([\s\S]{0,260}if \(!weaponNodeAvailable\) \{[\s\S]{0,120}return retainedWeaponCollisionActive;' 'Primary weapon-control ownership policy must keep suppression active across reload-null visual frames with retained weapon bodies.'
