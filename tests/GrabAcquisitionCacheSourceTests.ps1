...
Require-Text 'src/physics-interaction/object/ObjectPhysicsBodySet.h' 'struct ObjectPhysicsBodyScanCursor' 'Object body acquisition must expose a bounded scan cursor for selection-time prewarm.'
Require-Text 'src/physics-interaction/object/ObjectPhysicsBodySet.h' 'ObjectPhysicsBodyScanBudget' 'Object body acquisition must be budgeted when prewarmed from selection update.'
...
Require-Text 'src/physics-interaction/object/ObjectPhysicsBodySet.h' 'advanceObjectPhysicsBodyScanCache' 'Object body acquisition must be able to advance cache discovery in slices.'
...
Require-Text 'src/physics-interaction/object/ObjectPhysicsBodySet.h' 'staleCacheEntrySkips' 'Object-body diagnostics must expose stale cached scene pointers.'
...
Require-Text 'src/physics-interaction/object/ObjectPhysicsBodySet.cpp' 'cacheEntryStillReachable' 'Cached body-id replay must validate scene/collision reachability before use.'
...
Require-Text 'src/physics-interaction/hand/Hand.h' 'PreScanRunning' 'Grab acquisition cache must track bounded pre-scan progress.'
Require-Text 'src/physics-interaction/hand/Hand.h' 'postPrepComplete' 'Grab acquisition cache must distinguish pre-prep evidence from post-prep proof.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'advanceObjectPhysicsBodyScanCache' 'Selection update must advance grab acquisition cache in bounded slices.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' '_grabAcquisitionCache\.rootNode\.get\(\)\s*==\s*rootNode' 'Cache identity must include the selected root node pointer.'
