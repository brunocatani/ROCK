...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'bool Hand::tryBuildSavedGrabCapture[\s\S]*mesh\.nodeInObjectRoot = _grabFrame\.hasMeshCacheNodeInObjectRoot\s*\?\s*storeFrame\(_grabFrame\.meshCacheNodeInObjectRoot\)' 'The capture must emit the mesh-cache-to-root relation FROZEN at cache-build time: the held node''s world drifts against the root during the hold (visual lock), so recomputing the relation from live node worlds at save time double-counts that drift (measured 2-45gu misalignment on real captures).'
Reject-Text 'src/physics-interaction/hand/Hand.cpp' 'bool Hand::tryBuildSavedGrabCapture[\s\S]*objectInGeneratedProxyLocalSpace\(rootNode->world, meshNode->world\)' 'The capture must not derive the mesh-space relation from save-time node worlds; that is the drift-contaminated form the frozen grab-frame relation replaced.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'meshCacheNodeInObjectRoot =\s*\n?\s*grab_frame_math::objectInGeneratedProxyLocalSpace\(rootNode->world, objectWorldTransform\)' 'The mesh-cache-to-root relation must be frozen where the triangle cache is built, from the same objectWorldTransform the triangles were made local to.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.seatDiagnostics = GrabSeatDiagnostics\{[\s\S]*\.acquisitionMode =[\s\S]*\.shapeClass =[\s\S]*\.solveReason = seatSolve\.reason,[\s\S]*\.solveTermGirth = seatSolve\.solve\.terms\.girth,' 'Grab capture must record the acquisition mode, shape class and the full pose-solve outcome (reason, correction magnitude, term values), so a ground-truth capture explains which decisions produced the seat it is paired with.'
