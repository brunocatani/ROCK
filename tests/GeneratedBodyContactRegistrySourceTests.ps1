...
Require-Text 'src/physics-interaction/native/GeneratedKeyframedBodyDrive.cpp' 'tryComputeSampledAngularVelocityRadians\([\s\S]{0,240}referenceTarget\.rotate[\s\S]{0,160}target\.rotate' 'Generated collider target queueing must preserve target-to-target angular motion.'
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' '_weaponBodySampledAngularVelocityRadiansXAtomic\[publicationIndex\]\.store\(queueResult\.sampledAngularVelocityRadians\.x' 'Weapon sampled angular motion must be published with the cached body slot.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'tryGetWeaponBodySampledAngularVelocityAtomic\(contact\.bodyId[\s\S]{0,500}kFlagSampledAngularVelocity' 'Main-thread registry publication must snapshot weapon angular motion before physics callbacks.'
Require-Text 'src/physics-interaction/core/PhysicsInteractionContacts.inl' 'hasFiniteSampledAngularVelocity\(classification\)[\s\S]{0,500}sampledAngularVelocityRadians' 'Contact classification must carry sampled weapon angular motion from the fixed registry.'
Require-Text 'src/physics-interaction/core/PhysicsInteractionContacts.inl' 'weaponSource->hasSampledAngularVelocity[\s\S]{0,500}SourceAngularVelocityValid[\s\S]{0,1200}sourceMotion->angularVelocity' 'Weapon contact enrichment must prefer sampled target rotation before native body-motion angular fallback.'
