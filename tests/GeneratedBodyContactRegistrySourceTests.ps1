...
Require-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'const bool useSourceNode = instance\.sourceNode &&[\s\S]{0,180}tryResolveDescendantWorldTransform\([\s\S]{0,180}packageDriveNode[\s\S]{0,180}sourceWorld' 'Weapon body follow-updates must validate and use current source-node transforms when available.'
