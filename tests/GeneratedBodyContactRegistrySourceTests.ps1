...
Reject-Text 'src/physics-interaction/weapon/WeaponCollisionBodies.cpp' 'snapshotGeneratedKeyframedBodyDriveSampledVelocity' 'Weapon generated body queue path must publish queue-result sampled velocity without immediately snapshotting drive state.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollisionBodies.cpp' 'publishSampledVelocityAtomic\(instance\.publicationIndex, queueResult\)' 'Weapon generated body queue path must publish sampled velocity by cached publication slot.'
...
Require-Text 'src/physics-interaction/weapon/WeaponCollisionSources.cpp' 'source\.sourceLocalPointsGame' 'Generated weapon source extraction must build source-local hull point clouds.'
Require-Text 'src/physics-interaction/weapon/WeaponCollisionBodies.cpp' 'const bool useSourceNode = instance\.sourceNode &&[\s\S]{0,180}tryResolveDescendantWorldTransform\([\s\S]{0,180}packageDriveNode[\s\S]{0,180}sourceWorld' 'Weapon body follow-updates must validate and use current source-node transforms when available.'
