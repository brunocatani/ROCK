...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'getNativeScopeCameraTargetPreviewSnapshot[\s\S]*_nativeScopeRigidFrame\.valid[\s\S]*_nativeScopeAnchorValid[\s\S]*tryResolveNativeScopeCameraWeaponLocal\([\s\S]*_gunstockAlignmentAppliedThisFrame[\s\S]*cameraWeaponLocal[\s\S]*\.valid\s*=\s*targetValid' `
    'The exact preview must fail closed unless the retained target and resolved anchor still describe one weapon generation, and must expose the final gunstock-specific scope frame when applied.'
...
Require-Text 'src/physics-interaction/weapon/WeaponAuthority.h' 'alignOpticalAxesToBore[\s\S]*boreFrameWeaponLocal\.rotate\.entry\[1\]\[column\][\s\S]*-boreFrameWeaponLocal\.rotate\.entry\[0\]\[column\][\s\S]*boreFrameWeaponLocal\.rotate\.entry\[2\]\[column\]' `
    'Gunstock scope presentation must map optical +X to the latched bore +Y and retain bore +Z as camera up.'
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'tryResolveNativeScopeCameraWeaponLocal[\s\S]*hasBoreFrameWeaponLocal[\s\S]*alignOpticalAxesToBore[\s\S]*FiringGripFallback[\s\S]*applyWeaponLocalRotationOffset[\s\S]*applyGunstockAlignment[\s\S]*boreFrameWeaponLocal[\s\S]*applyWeaponVisualAuthority\([\s\S]*currentWeaponGenerationKey,[\s\S]*true\)[\s\S]*scopeTargetReady[\s\S]*resolveRigidAnchorFrameWorld\([\s\S]*cameraWeaponLocal' `
    'Only final gunstock weapon authority may replace the authored scope rotation with the bore-aligned optical frame, while retaining fallback tuning and the common anchor path.'
