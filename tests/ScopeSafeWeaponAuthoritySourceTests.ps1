...
Require-Text 'src/physics-interaction/weapon/WeaponCollisionBodies.cpp' 'accumulatePartKind\(WeaponPartKind::Scope\)[\s\S]*accumulatePartKind\(WeaponPartKind::Sight\)[\s\S]*rearPlaneCenterFromSightBounds[\s\S]*_nativeScopeSightAnchorSnapshot\s*=\s*nativeScopeSightAnchorSnapshot' `
...
Require-Text 'src/physics-interaction/weapon/TwoHandedGrip.cpp' 'updateHandVisualReturns\(dt\);[\s\S]*reconcileDeferredScopeHandAuthority\(weaponNode\);[\s\S]*traceNativeScopeTransitionFinalState\(weaponNode\);' `
    'Scope transition diagnostics must sample after ROCK finishes weapon, hand, visual-return, and deferred-authority publication.'
