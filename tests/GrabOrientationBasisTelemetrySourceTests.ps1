...
Reject-Text 'src/physics-interaction/hand/HandColliderTypes.h' 'generatedColliderFrameToGrabAuthorityFrame[\s\S]*transposeStoredRotation\(colliderFrame\.rotate\)' 'Grab telemetry must not preserve the bad transpose adapter; angular authority is supplied by the raw-hand hybrid boundary.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmAnchorGrabAuthorityBase\s*=[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(out\.handBodyWorld\)[\s\S]*rawRotationPalmAuthorityBase\s*=[\s\S]*makeGrabAuthorityProxyBaseFrame\(out\.rawHandWorld,\s*palmAnchorGrabAuthorityBase\)[\s\S]*palmAnchorGrabAuthorityWorld\s*=\s*applyGrabAuthorityProxyLocalOffsetToFrame\(rawRotationPalmAuthorityBase,\s*_isLeft\)' 'Telemetry must include the offset hidden-proxy grab-authority interpretation using live palm translation and raw hand rotation.'
