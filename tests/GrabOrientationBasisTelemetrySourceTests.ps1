...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'tryComputeGrabAuthorityProxyFrameWorld[\s\S]*tryResolveLivePalmAnchorReference\(world,\s*palmReference\)[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyBaseWorld,\s*_isLeft\)' 'Runtime dynamic grab pivot A must be sourced from the offset hidden grab authority proxy derived from the live palm-anchor body frame.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*tryComputeGrabAuthorityProxyFrameWorld\(world,\s*proxyWorld\)[\s\S]*return proxyWorld\.translate' 'Runtime dynamic grab pivot A must consume the same proxy frame as palm-pocket construction.'
...
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'buildGrabPocketFrameFromAuthorityFrame[\s\S]*computeGrabAuthorityPalmFaceWorld' 'Dynamic grab pocket construction must consume the generated proxy basis, not legacy authored handspace.'
