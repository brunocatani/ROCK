...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabPivotAHandspacePosition[\s\S]*g_rockConfig\.rockLeftGrabPivotAHandspace[\s\S]*g_rockConfig\.rockRightGrabPivotAHandspace' 'Source proof must show the current runtime pivot A is still backed by the old INI-configured handspace point.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*computeGrabPivotAPositionFromHandBasis\(fallbackHandWorldTransform,\s*_isLeft\)' 'Source proof must show runtime computeGrabPivotAWorld still resolves through the old configured pivot path today.'
