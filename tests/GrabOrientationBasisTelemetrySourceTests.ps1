...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabLegacyPalmPivotAHandspacePosition[\s\S]*g_rockConfig\.rockLeftGrabLegacyPalmPivotAHandspace[\s\S]*g_rockConfig\.rockRightGrabLegacyPalmPivotAHandspace' 'Source proof must keep the configured handspace palm point available for telemetry and raw-hand dynamic grab authority.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*return\s+makeRawHandGrabAuthorityFrame\(fallbackHandWorldTransform,\s*_isLeft\)\.translate' 'Runtime dynamic grab pivot A must use the raw hand authority palm pivot.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'makeRawHandGrabAuthorityFrame[\s\S]*computeGrabLegacyPalmPivotAWorldFromHandBasis\(handTransform,\s*isLeft\)' 'Runtime dynamic grab pivot fallback must use the raw hand configured palm point instead of generated/proxy readback.'
...
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'rejectFromAxis\(basisFingerForwardWorld,\s*frame\.palmNormalWorld\)[\s\S]*rejectFromAxis\(crossPalmCandidate,\s*frame\.fingerForwardWorld\)' 'Dynamic grab pocket roll must project authority axes around the unchanged palm normal.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'proxyAuthorityFrameWorldAtGrab\s*=\s*grabAuthorityFrameWorldAtGrab[\s\S]*buildGrabPocketFrameWithPalmCenter\(\s*proxyAuthorityFrameWorldAtGrab[\s\S]*grabPivotAWorld' 'Dynamic grab capture must pass raw hand authority basis and the selected palm-pocket pivot into three-phase pocket construction.'
