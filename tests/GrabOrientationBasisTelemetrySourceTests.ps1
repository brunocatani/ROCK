...
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'rejectFromAxis\(basisFingerForwardWorld,\s*frame\.palmNormalWorld\)[\s\S]*rejectFromAxis\(thumbSideCandidate,\s*frame\.fingerForwardWorld\)' 'Dynamic grab pocket roll must project generated/proxy authority axes around the unchanged palm normal.'
