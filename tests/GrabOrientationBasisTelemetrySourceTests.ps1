...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*_boneColliders\.tryGetPalmAnchorTarget\(generatedPalmWorld\)[\s\S]*return generatedPalmWorld\.translate' 'Runtime dynamic grab pivot A must be sourced from the root-flattened generated palm target.'
