...
Reject-Text 'src/physics-interaction/hand/HandColliderTypes.h' 'generatedColliderFrameToGrabAuthorityFrame[\s\S]*transposeStoredRotation\(colliderFrame\.rotate\)' 'Grab telemetry must not preserve the bad transpose adapter; generated palm axes remain separate from raw-rotation grab authority.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmAnchorGrabAuthorityBase\s*=[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(out\.handBodyWorld\)[\s\S]*palmAnchorGrabAuthoritySeatWorld\s*=[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(palmAnchorGrabAuthorityBase,\s*out\.rawHandWorld,\s*_isLeft\)[\s\S]*palmAnchorGrabAuthorityWorld\s*=[\s\S]*makeRawRotationPalmTranslationFrame\(out\.rawHandWorld,\s*palmAnchorGrabAuthoritySeatWorld\)' 'Telemetry must include the live left-hand raw-basis offset on the palm anchor and expose raw-rotation grab authority.'
