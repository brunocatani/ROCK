...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmAnchorGrabAuthorityWorld\s*=\s*computeGrabAuthorityFrameFromHandBasis\(out\.nativeFlattenedHandWorld,\s*_isLeft\)' 'Telemetry must include the hand-bone-derived grab-authority frame used by runtime grab math.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'proxyReadbackWorld\s*=\s*proxyWorld' 'Telemetry must include live proxy/body-A readback for visual comparison against the hand-bone authority frame.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteractionDebugOverlay.inl' 'GRAB BASIS LEGACY_PIVOT' 'Logs must expose old INI pivot contamination separately from raw hand-bone authority.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabAuthorityFrameFromHandBasis[\s\S]*return handTransform' 'Source proof must derive dynamic grab authority directly from the root-flattened hand frame.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*computeGrabAuthorityFrameFromHandBasis\(fallbackHandWorldTransform,\s*_isLeft\)\.translate' 'Runtime dynamic grab pivot A must be sourced from the root-flattened hand authority frame.'
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'buildGrabPocketFrameWithPalmCenter' 'Dynamic grab pocket construction must accept the already-resolved grab authority center.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'buildGrabPocketFrameWithPalmCenter\([\s\S]*grabPivotAWorld' 'Dynamic grab capture must pass the raw hand-bone pivot into three-phase pocket construction.'
