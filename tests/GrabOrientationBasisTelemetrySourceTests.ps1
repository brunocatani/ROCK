...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmAnchorGrabAuthorityBase\s*=[\s\S]*makeGrabAuthoritySemanticPalmBaseFrame\(out\.rawHandWorld,\s*out\.handBodyWorld\.translate,\s*_isLeft\)[\s\S]*palmAnchorGrabAuthorityWorld\s*=[\s\S]*makeGrabAuthorityProxyFrameFromSemanticPalmPocket\(out\.rawHandWorld,\s*palmAnchorGrabAuthorityBase\.translate,\s*_isLeft\)' 'Telemetry must include the raw-rotation semantic hidden-proxy interpretation of the actual live palm-anchor translation.'
