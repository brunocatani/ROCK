...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmAnchorGrabAuthorityWorld\s*=[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(out\.handBodyWorld\)' 'Telemetry must include the grab-authority interpretation of the actual live palm-anchor body frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'out\.desiredTransformBLocalGame\s*=\s*activeProxyConstraintPivotBLocalGame\(\)' 'Constraint telemetry must compare transform-B against the frozen active pivot used by production grab.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'composeRagdollAngularTargetRotation\([\s\S]*desiredBodyToAuthoritySpace\.rotate[\s\S]*transformAAsHkColumns' 'Constraint telemetry must compare target_bRca against the raw-hand transform-A authority equation.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyTransformHandSpace\s*=\s*_grabFrame\.constraintBodyHandSpace[\s\S]*computeDynamicTransformBTranslationGame\(desiredBodyTransformHandSpace' 'Constraint telemetry must not use the retired computed transform-B convention.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*tryResolveLivePalmAnchorReference\(world,\s*palmReference\)[\s\S]*return palmReference\.world\.translate' 'Runtime dynamic grab pivot A must be sourced from the actual live palm-anchor body frame.'
