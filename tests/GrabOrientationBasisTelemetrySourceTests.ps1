...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'legacyHandBasisPivotAWorld\s*=\s*computeGrabPivotAPositionFromHandBasis' 'Telemetry must label the old INI pivot separately instead of treating it as the generated palm.'
Require-Text 'src/physics-interaction/grab/GrabTelemetry.h' 'rawHandTelemetryAliasToPalmAnchorTarget' 'Telemetry must distinguish the flattened hand frame from the generated palm frame.'
Require-Text 'src/physics-interaction/grab/GrabTelemetry.h' 'legacyHandBasisPivotAToPalmAnchor' 'Telemetry must measure old configured pivot distance to the bone-derived palm without making it an authority reference.'
Require-Text 'src/physics-interaction/grab/GrabTelemetry.h' 'livePalmAnchorFingerBaseLineWorld' 'Telemetry must keep a finger-base direction anchored from the live hand body frame.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeRuntimeAuthorityPivotAWorld[\s\S]*tryResolveLivePalmAnchorReference\(world,\s*palmReference\)[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(proxyBaseWorld,\s*fallbackHandWorldTransform,\s*_isLeft\)\.translate' 'Runtime dynamic grab pivot A must preserve the live palm-anchor origin while using the raw left-hand basis for the active offset.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeRuntimeAuthorityPivotAWorld[\s\S]*return fallbackHandWorldTransform\.translate' 'Runtime dynamic grab pivot fallback must use raw hand origin instead of the retired INI-configured offset.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'livePalmAnchorFingerBaseLineWorld\s*=' 'Telemetry must compute a finger-base direction from the live hand body origin.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteractionDebugOverlay.inl' 'livePalmAnchorLine=\{\} generatedPalmLine=\{\}' 'Per-frame telemetry must expose hand-body and generated-palm finger-base lines separately from the raw-hand line.'
