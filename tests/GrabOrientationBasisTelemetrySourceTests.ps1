...
Reject-Text 'src/physics-interaction/hand/HandColliderTypes.h' 'generatedColliderFrameToGrabAuthorityFrame[\s\S]*transposeStoredRotation\(colliderFrame\.rotate\)' 'Grab telemetry must not preserve the bad transpose adapter; generated palm remains diagnostic comparison only.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmAnchorGrabAuthorityBase\s*=[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(out\.handBodyWorld\)[\s\S]*makeRawRotationPalmTranslationFrame\(out\.nativeFlattenedHandWorld,\s*palmAnchorGrabAuthorityBase\)[\s\S]*palmAnchorGrabAuthorityWorld\s*=\s*applyGrabAuthorityProxyLocalOffsetToFrame\(palmAnchorGrabAuthorityFrame,\s*_isLeft\)' 'Telemetry must include the offset hidden-proxy authority frame built from live palm translation and raw hand rotation.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'tryComputeGrabAuthorityProxyFrameWorld\(RE::hknpWorld\* world,\s*const RE::NiTransform& rawHandWorld[\s\S]*tryResolveLivePalmAnchorReference\(world,\s*palmReference\)[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*makeRawRotationPalmTranslationFrame\(rawHandWorld,\s*proxyBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(authorityWorld,\s*_isLeft\)' 'Runtime dynamic grab pivot A must be sourced from live palm translation plus raw hand rotation.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*tryComputeGrabAuthorityProxyFrameWorld\(world,\s*fallbackHandWorldTransform,\s*proxyWorld\)[\s\S]*return proxyWorld\.translate' 'Runtime dynamic grab pivot A must consume the same proxy frame as palm-pocket construction.'
...
Require-Text 'src/physics-interaction/grab/GrabThreePhase.h' 'buildGrabPocketFrameFromAuthorityFrame[\s\S]*computeGrabAuthorityPalmFaceWorld' 'Dynamic grab pocket construction must consume the raw-rotation grab authority basis, not legacy authored handspace.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'buildGrabPocketFrameFromAuthorityFrame\([\s\S]*proxyFrameWorldAtGrab[\s\S]*grabPivotAWorld' 'Dynamic grab capture must pass the generated proxy frame and pivot into three-phase pocket construction.'
