...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'writeGrabRagdollAngularTarget\(targetBRca,\s*desiredBodyTransformAuthoritySpace,\s*transformAFrameLocal\.rotate\)' 'Proxy target updates must refresh target_bRca through the FO4VR ragdoll angular atom composition.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationColumns\(transformBRotation,\s*bodyToAuthorityRotation\)' 'FO4VR custom grab angular setup must write transform-B as Havok column blocks from the body-to-authority relation.'
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationColumns\(targetBRca,\s*targetRotation\)' 'FO4VR custom grab angular setup must write target_bRca as Havok column blocks after composing body-to-authority with transform-A.'
