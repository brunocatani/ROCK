...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'PROXY GRAB FRAME MISMATCH[\s\S]*rawToProxyTarget[\s\S]*targetAxisDeg[\s\S]*liveAxisDeg[\s\S]*determinantTarget[\s\S]*targetRowsRest[\s\S]*transformBInv' 'Proxy after-solve telemetry must expose the raw/proxy frame mismatch, palm-basis axes, and constraint-frame invariants without relying on verbose grab logging.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'RAGDOLL ANGULAR PROBE[\s\S]*beforeErr[\s\S]*afterErr[\s\S]*reduce[\s\S]*axisDot[\s\S]*forceA[\s\S]*targetRowsRest[\s\S]*gripBefore[\s\S]*pivotLever' 'Proxy after-solve telemetry must prove whether the ragdoll angular atom reduces body rotation error independently from the linear pivot.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'targetConstraintFrameRotation\s*=\s*transform_math::makeIdentityRotation[\s\S]*writeHavokRotationRows\(targetBRca,\s*targetConstraintFrameRotation\)' 'FO4VR custom grab angular setup must write target_bRca as the rest relation between transformed constraint frames.'
Reject-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationRows\(targetBRca,\s*bodyToHandRotation\)' 'Proxy angular setup must not write the captured body-to-hand relation into both transform-B and target_bRca.'
