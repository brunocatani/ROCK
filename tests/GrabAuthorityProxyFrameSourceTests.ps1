...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.rawAuthorityObjectSpace\s*=\s*frozen\.rawAuthorityObjectSpace' 'Proxy authority must capture the desired object relation in the raw-authority pivot frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.rawAuthorityBodySpace\s*=\s*frozen\.rawAuthorityBodySpace' 'Proxy authority must capture the desired BODY relation in the raw-authority pivot frame used by live constraint updates.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'kGrabObjectRotationReferenceName\s*=\s*"rawAuthorityPivotFrame"' 'The active grab angular reference must be the raw-authority pivot frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintPivotAWorld\s*=\s*grabPivotAWorld[\s\S]*rawAuthorityFrameWorld\s*=\s*grab_authority_frame_math::makeRawAuthorityFrame\(rawHandWorldTransform,\s*constraintPivotAWorld\)[\s\S]*constraintAInBodyASpace\s*=\s*grab_constraint_math::computeConstraintFrameInBodySpace\(proxyWorldTransform,\s*rawAuthorityFrameWorld\)[\s\S]*constraintBInBodyBSpace\s*=\s*grab_constraint_math::computeConstraintFrameInBodySpace\(desiredBodyWorld,\s*rawAuthorityFrameWorld\)[\s\S]*createGrabConstraint\(world,[\s\S]*constraintAInBodyASpace,[\s\S]*constraintBInBodyBSpace' 'Proxy constraint creation must encode the raw-authority pivot frame into both body-local constraint frames.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyWorld\s*=\s*grab_constraint_math::computeDesiredBodyWorld\(rawAuthorityFrameWorld,\s*_grabFrame\.rawAuthorityBodySpace\)' 'Proxy constraint creation must derive the desired BODY frame from the frozen raw-authority BODY relation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outActivePivotBBodyLocalGame\s*=\s*constraintBInBodyBSpace\.translate' 'Proxy constraint target math must refresh the active body-local pivot from the raw-authority transform-B frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'writeConstraintLocalTransform\(transformBRotation,\s*transformBTranslation,\s*constraintBInBodyBSpace,\s*gameToHkScale\)' 'Transform-B must be written from the raw-authority body-local frame, not a stale visual/object local point.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outDesiredObjectWorld\s*=\s*grab_constraint_math::computeDesiredObjectWorld\(rawAuthorityFrameWorld,\s*_grabFrame\.rawAuthorityObjectSpace\)' 'Proxy desired object frame must always compose from the frozen raw-authority relation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'PROXY GRAB FRAME MISMATCH[\s\S]*rawToProxyTarget[\s\S]*targetAxisDeg[\s\S]*liveAxisDeg[\s\S]*determinantTarget[\s\S]*targetRowsTransformB[\s\S]*targetColsTransformB' 'Proxy after-solve telemetry must expose the raw/proxy frame mismatch, palm-basis axes, and target/transform-B motor feed invariants without relying on verbose grab logging.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'RAGDOLL ANGULAR PROBE[\s\S]*beforeErr[\s\S]*afterErr[\s\S]*reduce[\s\S]*axisDot[\s\S]*bodyErrProxyAngle[\s\S]*bodyErrProxyAxis[\s\S]*bRcaErrProxyAngle[\s\S]*bRcaErrProxyAxis[\s\S]*feedErrProxyAngle[\s\S]*feedErrProxyAxis[\s\S]*bodyToBRcaAxisDot[\s\S]*feedToBRcaAxisDot[\s\S]*forceA[\s\S]*targetRowsTransformB[\s\S]*gripBefore[\s\S]*pivotLever' 'Proxy after-solve telemetry must decompose intended body correction, live ragdoll correction, and motor-feed mismatch in the same proxy-local axis basis.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'RAGDOLL FRAME ERROR[\s\S]*bRcaRowsErr[\s\S]*reqAxisProxy[\s\S]*bodyErrProxyAngle[\s\S]*bodyErrProxyAxis[\s\S]*bRcaErrProxyAngle[\s\S]*bRcaErrProxyAxis[\s\S]*feedErrProxyAngle[\s\S]*feedErrProxyAxis[\s\S]*bodyToBRcaAxisDot[\s\S]*feedToBRcaAxisDot' 'Proxy frame-error telemetry must expose the same proxy-local motor decomposition fields as the main ragdoll probe.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeConstraintLocalTransform[\s\S]*writeHavokRotationColumns\(transformRotation,\s*frameInBodySpace\.rotate\)' 'FO4VR custom grab angular setup must write constraint local rotations as Havok column blocks.'
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeRagdollTargetForConstraintBFrame[\s\S]*writeHavokRotationRows\(targetBRca,\s*constraintBInBodyBSpace\.rotate\)' 'FO4VR custom grab angular setup must feed target_bRca from the body-B local raw-authority rotation under the verified ragdoll motor contract.'
...
Reject-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationRows\(transformBRotation' 'Proxy angular setup must keep transform local frames in Havok column storage.'
