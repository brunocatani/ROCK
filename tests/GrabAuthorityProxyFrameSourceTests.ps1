...
Require-Text 'src/physics-interaction/hand/HandColliderTypes.h' 'generatedColliderFrameToGrabAuthorityFrame[\s\S]*return\s+colliderFrame;' 'Generated collider frames and grab authority must share the same convention in the full-convention diagnostic build.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'generatedColliderFrameToGrabAuthorityFrame\(outProxyWorld\)' 'Proxy authority must continue passing through the explicit convention boundary even when the adapter is identity.'
