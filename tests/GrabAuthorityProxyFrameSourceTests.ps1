...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::getGrabAuthorityProxyDebugSnapshot\(RE::hknpWorld\* world,\s*const RE::NiTransform& rawHandWorld[\s\S]*tryResolveLivePalmAnchorReference\(world,\s*palmReference\)[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*makeRawRotationPalmTranslationFrame\(rawHandWorld,\s*palmAuthorityBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(authorityWorld,\s*_isLeft\)' 'Idle proxy debug target must show the same raw-rotation/palm-translation authority frame as runtime grab.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'makeRawRotationPalmTranslationFrame[\s\S]*rawHandWorld\.rotate[\s\S]*rawHandWorld\.scale[\s\S]*makeGrabStartupCaptureAuthorityFrame[\s\S]*makeRawRotationPalmTranslationFrame' 'Grab authority must share the raw-rotation/palm-translation frame used by startup, runtime, and debug paths.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*makeRawRotationPalmTranslationFrame\(rawHandWorld,\s*proxyBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(authorityWorld,\s*_isLeft\)' 'Resolved hidden proxy frame must consume generated palm translation, raw hand rotation, then the configured local offset.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'pending\.proxyWorld\s*=\s*applyGrabAuthorityProxyLocalOffsetToFrame\(authorityWorld,\s*_isLeft\)' 'Per-frame hidden proxy target must keep consuming the configured local offset through the raw-rotation authority frame.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'tryComputeGrabAuthorityProxyFrameWorld\(RE::hknpWorld\* world,\s*const RE::NiTransform& rawHandWorld[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*makeRawRotationPalmTranslationFrame\(rawHandWorld,\s*proxyBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(authorityWorld,\s*_isLeft\)' 'Pivot-A must expose the offset hidden proxy frame derived from live palm translation and raw hand rotation.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*tryComputeGrabAuthorityProxyFrameWorld\(world,\s*fallbackHandWorldTransform,\s*proxyWorld\)[\s\S]*return proxyWorld\.translate' 'Pivot-A must use the same resolved proxy frame consumed by palm-pocket basis math.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteractionFrame.inl' 'makeRawRotationPalmTranslationFrame\(input\.rawHandWorld,\s*semanticHandFrame\.palmAnchorWorld\)[\s\S]*tryComputeGrabAuthorityProxyFrameWorld\(hknp,\s*input\.rawHandWorld,\s*input\.grabAuthorityWorld\)[\s\S]*directionAuthorityWorld[\s\S]*computeGrabAuthorityPalmFaceWorld\(directionAuthorityWorld\)[\s\S]*transformGrabAuthorityTuningDirection\(directionAuthorityWorld' 'Frame context close selection, far pointing, and pinch direction must use raw-rotation/palm-translation authority.'
