...
Require-Text 'src/physics-interaction/hand/Hand.h' 'struct\s+LivePalmAnchorReference' 'Dynamic grab diagnostics must keep an explicit live palm-anchor reference type.'
Require-Text 'src/physics-interaction/hand/Hand.h' 'tryResolveLivePalmAnchorReference' 'Dynamic grab diagnostics must still be able to read the actual palm anchor body.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabAuthorityFrameFromHandBasis[\s\S]*return handTransform' 'Proxy authority must resolve directly from the root-flattened hand frame.'
Reject-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabAuthorityFrameFromHandBasis[\s\S]*computeGrabPivotAPositionFromHandBasis\(handTransform,\s*isLeft\)' 'Dynamic grab authority must not consume the old INI-configured handspace PivotA.'
Reject-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabAuthorityFrameFromHandBasis[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame' 'Dynamic grab authority must not consume the hidden proxy offset while testing raw hand-bone authority.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)' 'Proxy authority must not derive the grab frame from generated palm-collider orientation.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*computeGrabAuthorityFrameFromHandBasis\(rawHandWorld,\s*_isLeft\)[\s\S]*rootFlattenedHandBoneAuthority' 'Resolved hidden proxy frame must consume the root-flattened hand frame directly.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'pending\.authorityWorld\s*=\s*computeGrabAuthorityFrameFromHandBasis\(pending\.rawHandWorld,\s*_isLeft\)' 'Per-frame hidden proxy target must keep consuming the root-flattened hand frame.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*computeGrabAuthorityFrameFromHandBasis\(fallbackHandWorldTransform,\s*_isLeft\)\.translate' 'Pivot-A must use the hand-bone-derived authority origin.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*rawHandFallback' 'Proxy authority must not silently fall back to a separate raw-controller path.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.pivotBConstraintLocalGame\s*=\s*constraintDrivePivotBBodyLocalGame\(_grabFrame\)' 'Proxy authority must make transform-B use the frozen selected contact point in solver body-B local space after frame capture.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'transformAFrameLocal\s*=\s*[\s\S]*makeIdentityTransform\(\)' 'Proxy target updates must keep transform-A as identity because body-A already carries raw hand rotation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'rawRotationProxyFrameWorldAtGrab\s*=\s*proxyFrameWorldAtGrab' 'Proxy authority capture must use the already-resolved hand-bone authority frame directly.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.rawRotationProxyHandSpace\s*=\s*grab_frame_math::objectInFrameSpace\(rawRotationProxyFrameWorldAtGrab,\s*desiredObjectWorld\)' 'Proxy authority must capture object relation in the root-flattened hand authority frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'kGrabObjectRotationReferenceName\s*=\s*"rootFlattenedHandBoneAuthority"' 'The validated root-flattened hand-bone authority must be the only production rotation reference.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'proxyBodyWorld\s*=\s*authorityWorldTransform' 'Proxy body-A must be created from the hand-bone-derived authority frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'pivotAProxyLocalGame\s*=\s*transform_math::worldPointToLocal\(proxyBodyWorld,\s*grabPivotAWorld\)' 'Proxy body-A PivotA local must be captured in the same raw-rotation proxy frame consumed by the constraint.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outActivePivotBBodyLocalGame\s*=\s*_grabAuthorityPivotBConstraintLocalGame' 'Proxy constraint target math must keep transform-B pinned to the frozen selected contact point instead of regenerating a second moving pivot from angular relation math.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'transformBTranslation\[0\]\s*=\s*outActivePivotBBodyLocalGame\.x\s*\*\s*gameToHkScale' 'Transform-B must be written from the frozen solver body-B local contact point.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outDesiredObjectWorld\s*=\s*multiplyTransforms\(authorityFrame,\s*_grabFrame\.rawRotationProxyHandSpace\)' 'Proxy desired object frame must always compose from the validated root-flattened hand relation.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'queueProxyGrabAuthorityTarget\(\s*authorityWorld' 'Held updates must queue the resolved raw-hand authority target.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'queueGeneratedKeyframedBodyTarget\(\s*_grabAuthorityProxyDriveState,\s*pending\.authorityWorld' 'Proxy authority must queue the live raw-hand authority frame through the generated keyframed-body drive path.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'driveGeneratedKeyframedBody\([\s\S]*_grabAuthorityProxy[\s\S]*"grab-authority-proxy"' 'Proxy authority must move with the native generated-body drive path while consuming the hand-bone authority frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeHardKeyframeVelocityForTarget\(\s*world,\s*proxyBodyId,\s*pending\.authorityWorld,\s*driveDelta,[\s\S]*angularVelocityHavok\)' 'Proxy authority may keep FO4VR hard-keyframe velocity as diagnostic fallback when palm motion velocity is unavailable.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeAngularVelocityRadiansPerSecond\(previousProxyWorld,\s*pending\.(?:proxyWorld|authorityWorld),\s*driveDelta,\s*angularVelocityHavok\)' 'Proxy authority must not return to the old matrix-column angular delta helper.'
