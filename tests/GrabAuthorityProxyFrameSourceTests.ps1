...
Require-Text 'src/physics-interaction/core/PhysicsInteractionFrame.inl' 'input\.grabAnchorWorld\s*=\s*hand\.computeStartupCaptureSeatPivotAWorld\(hknp,\s*input\.rawHandWorld\)' 'Frame debug/selection grab anchor must show the raw-roll palm-pocket startup point.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmPocketPivotAWorld\s*=\s*computeStartupCaptureSeatPivotAWorld\(world,\s*handWorldTransform\)' 'Close grab palm-pocket acquisition must seed from the startup raw-rotation capture point.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeRuntimeAuthorityPivotAWorld[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(proxyBaseWorld,\s*fallbackHandWorldTransform,\s*_isLeft\)\.translate' 'Pivot-A must use the live palm anchor base origin while routing the left runtime offset through raw hand basis.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeStartupCaptureSeatPivotAWorld[\s\S]*tryComputeGrabRawRollPalmPocketPivotAWorld\(world,\s*rawHandWorldTransform,\s*rawRollPalmPocketPivotWorld\)' 'Grab-start pivot capture must delegate to the shared raw-roll palm-pocket helper.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.bodyInCapturedNodeSpace\s*=\s*frozen\.bodyInCapturedNodeSpace' 'Proxy authority must store the captured visual-object to BODY relation through the frozen authority frame instead of assuming BODY equals the visible node.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.desiredNodeInAuthorityFrameSpace\s*=\s*frozen\.desiredNodeInAuthorityFrameSpace' 'Proxy authority must capture object relation in the hybrid raw-rotation/palm-translation frame through the frozen authority contract.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.desiredBodyInAuthorityFrameSpace\s*=\s*frozen\.desiredBodyInAuthorityFrameSpace' 'Proxy authority must capture BODY relation in the same hybrid raw-rotation/palm-translation frame used by the live angular target.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'kGrabObjectRotationReferenceName\s*=\s*"rawRotationAuthorityTranslation"' 'The validated raw-rotation/palm-translation authority must be the only production rotation reference.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyTransformAuthoritySpace\s*=\s*_grabFrame\.desiredBodyInAuthorityFrameSpace' 'Proxy constraint creation must seed the ragdoll angular target from the same raw-rotation proxy BODY relation used by held updates.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outDesiredObjectWorld\s*=\s*multiplyTransforms\(authorityFrame,\s*_grabFrame\.desiredNodeInAuthorityFrameSpace\)' 'Proxy desired object frame must always compose from the validated hybrid raw-rotation/palm-translation relation.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationColumns\(transformBRotation,\s*bodyToAuthorityRotation\)' 'FO4VR custom grab angular setup must write transform-B as Havok column blocks, matching HIGGS hkMatrix storage.'
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationRows\(targetBRelativeToA,\s*bodyToAuthorityRotation\)' 'FO4VR custom grab angular setup must write targetBRelativeToA in the solver-row view so top grabs see the inverse body-to-authority relation.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outDesiredObjectWorld\s*=\s*multiplyTransforms\(proxyWorldTransform,\s*_grabFrame\.desiredNodeInRawHandSpace\)' 'Proxy body-A convention must not use the raw controller hand-space relation.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.bodyInCapturedNodeSpace\s*=\s*makeIdentityTransform\(\)' 'Proxy authority must not erase the visual-object to BODY relation; doing so causes immediate angular correction after grab.'
