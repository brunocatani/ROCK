...
Require-Text 'src/physics-interaction/hand/HandColliderTypes.h' 'generatedColliderFrameToGrabAuthorityFrame[\s\S]*transposeStoredRotation\(colliderFrame\.rotate\)' 'Grab authority must adapt generated collider frames at the explicit boundary instead of changing collider placement convention.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'generatedColliderFrameToGrabAuthorityFrame\(outProxyWorld\)' 'Proxy authority must continue passing through the explicit convention boundary.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'generatedColliderFrameToGrabAuthorityFrame\(\*fallbackPalmAnchorWorld\)' 'Palm-anchor fallback must use the same explicit convention boundary as the primary proxy target.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintUsesMotionBodyAtGrab\s*=\s*hasMotionBodyWorldAtGrab\s*&&\s*motionBodySourceAtGrab\s*==\s*body_frame::BodyFrameSource::MotionCenterOfMass' 'Proxy authority must choose the hknp solver body-B frame from live MOTION readback when available.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintBodyWorldAtGrab\s*=\s*constraintUsesMotionBodyAtGrab\s*\?\s*motionBodyWorldAtGrab\s*:\s*grabBodyWorldAtGrab' 'Proxy authority must encode transform-B and angular target data against the solver body-B frame, not hardcoded BODY.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryGetGrabDriveObjectWorldTransform[\s\S]*HeldObjectDriveMode::ProxyConstraint[\s\S]*tryResolveLiveBodyWorldTransform[\s\S]*BodyFrameSource::MotionCenterOfMass[\s\S]*return\s+tryGetGrabAuthorityBodyWorldTransform' 'Proxy runtime error and pivot telemetry must read body-B through the active solver frame before falling back to BODY.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeAngularVelocityRadiansPerSecond\(previousProxyWorld,\s*pending\.proxyWorld,\s*driveDelta,\s*angularVelocityHavok\)' 'Proxy authority must write angular velocity from root-flattened palm frame deltas, not a zero angular velocity.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constexpr\s+bool\s+constraintUsesMotionBodyAtGrab\s*=\s*false' 'Proxy authority must not force body-B constraint data into BODY when hknp exposes a MOTION solver frame.'
