...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'RAGDOLL ANGULAR PROBE[\s\S]*beforeErr[\s\S]*afterErr[\s\S]*reduce[\s\S]*axisDot[\s\S]*forceA[\s\S]*targetRowsInv[\s\S]*targetRowsDesiredBRca[\s\S]*desiredBRcaIdentity[\s\S]*gripBefore[\s\S]*pivotLever' 'Proxy after-solve telemetry must prove whether the ragdoll angular atom reduces body rotation error independently from the linear pivot and whether target_bRca matches the desired constraint-frame relation.'
