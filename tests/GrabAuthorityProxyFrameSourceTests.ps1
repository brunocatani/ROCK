...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'pending\.authorityWorld\s*=\s*makeRawRotationPalmTranslationFrame\(pending\.rawHandWorld,\s*palmProxyWorld\)' 'The hidden authority body must carry raw hand rotation with palm-collider translation, so transform-A does not become a second angular authority.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'proxyBodyWorld\s*=\s*[\s\S]*makeRawRotationPalmTranslationFrame\(rawHandWorldTransform,\s*palmProxyWorldTransform\)' 'Proxy body-A must be created from raw hand rotation plus palm/proxy translation.'
