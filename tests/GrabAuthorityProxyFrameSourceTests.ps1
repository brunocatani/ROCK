...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'PROXY GRAB FRAME MISMATCH[\s\S]*rawToProxyTarget[\s\S]*targetRowsInv[\s\S]*targetColsToTransformB' 'Proxy after-solve telemetry must expose the raw/proxy frame mismatch and constraint-frame invariants without relying on verbose grab logging.'
