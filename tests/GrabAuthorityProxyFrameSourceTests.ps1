...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeConstraintAFrameProxySpace[\s\S]*computeConstraintATransformBodyASpace' 'Proxy constraint side A must keep the proxy body but adapt the local transform-A rotation into raw hand authority space.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintATransformProxySpace\s*=[\s\S]*computeConstraintAFrameProxySpace\(rawHandWorldTransform,\s*proxyWorldTransform\)[\s\S]*createGrabConstraint\(world,[\s\S]*proxyWorldTransform,[\s\S]*constraintPivotAWorld,[\s\S]*constraintATransformProxySpace' 'Proxy constraint creation must write the raw-hand transform-A adapter without changing proxy body drive or frozen body-B relation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'PROXY GRAB FRAME MISMATCH[\s\S]*rawToProxyTarget[\s\S]*targetAxisDeg[\s\S]*liveAxisDeg[\s\S]*determinantTarget[\s\S]*transformAColsRaw[\s\S]*solverARaw[\s\S]*targetRowsInv[\s\S]*targetColsToTransformB' 'Proxy after-solve telemetry must expose the raw/proxy frame mismatch, transform-A adapter, palm-basis axes, and constraint-frame invariants without relying on verbose grab logging.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeConstraintARotation[\s\S]*writeHavokRotationColumns\(transformARotation,\s*constraintATransformBodyASpace\.rotate\)' 'Transform-A rotation must use the set-local-transforms atom column storage, matching transform-B rather than target_bRca rows.'
