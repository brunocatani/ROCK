...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'computeGrabAuthorityFrameFromHandBasis[\s\S]*computeGrabPivotAPositionFromHandBasis\(handTransform,\s*isLeft\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(authority,\s*isLeft\)' 'Proxy authority must resolve from the root-flattened hand frame and handspace palm point.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*computeGrabAuthorityFrameFromHandBasis\(rawHandWorld,\s*_isLeft\)[\s\S]*rootFlattenedHandPalmAuthority' 'Resolved hidden proxy frame must consume the root-flattened hand frame and handspace palm offset.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.rawRotationProxyHandSpace\s*=\s*grab_frame_math::objectInFrameSpace\(rawRotationProxyFrameWorldAtGrab,\s*desiredObjectWorld\)' 'Proxy authority must capture object relation in the root-flattened hand/palm authority frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'kGrabObjectRotationReferenceName\s*=\s*"rootFlattenedHandPalmAuthority"' 'The validated root-flattened hand/palm authority must be the only production rotation reference.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outDesiredObjectWorld\s*=\s*multiplyTransforms\(authorityFrame,\s*_grabFrame\.rawRotationProxyHandSpace\)' 'Proxy desired object frame must always compose from the validated root-flattened hand/palm relation.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'queueProxyGrabAuthorityTarget\(\s*authorityWorld' 'Held updates must queue the resolved raw-hand/palm authority target.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'queueGeneratedKeyframedBodyTarget\(\s*_grabAuthorityProxyDriveState,\s*pending\.authorityWorld' 'Proxy authority must queue the live raw-hand/palm authority frame through the generated keyframed-body drive path.'
