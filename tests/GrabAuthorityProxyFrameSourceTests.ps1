...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'generatedProxyLocalVectorToWorld[\s\S]*matrix\.entry\[0\]\[1\]\s*\*\s*localVector\.y[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame[\s\S]*generatedProxyLocalVectorToWorld\(proxyFrameWorld,\s*localOffset\)' 'Hidden grab authority proxy offset must be applied through generated/proxy stored-column local space.'
