...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'makeActualProxyLocalDesiredBodyTransform\([\s\S]*grab_frame_math::objectInFrameSpace\(actualProxyWorld,\s*desiredBodyWorld\)' 'Proxy constraint angular targets must be converted into the actual hidden proxy body-A frame before writing the hknp atom.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'createProxyConstraintGrabDrive[\s\S]*desiredBodyWorld\s*=\s*multiplyTransforms\(authorityFrame,\s*_grabFrame\.rawRotationProxyBodyHandSpace\)[\s\S]*desiredBodyTransformProxySpace\s*=\s*makeActualProxyLocalDesiredBodyTransform\(proxyWorldTransform,\s*desiredBodyWorld\)' 'Proxy constraint creation must seed the ragdoll angular target from the raw desired BODY world target expressed in actual proxy-local space.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'updateProxyConstraintGrabDriveTarget[\s\S]*outDesiredBodyWorld\s*=\s*multiplyTransforms\(authorityFrame,\s*_grabFrame\.rawRotationProxyBodyHandSpace\)[\s\S]*desiredBodyTransformProxySpace\s*=\s*makeActualProxyLocalDesiredBodyTransform\(proxyWorldTransform,\s*outDesiredBodyWorld\)[\s\S]*writeInitialGrabAngularFrame\(transformBRotation,\s*targetBRca,\s*desiredBodyTransformProxySpace\)' 'Proxy held updates must keep raw-rotation desired BODY authority but write the hknp ragdoll atom in actual proxy-local space.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyTransformProxySpace\s*=\s*_grabFrame\.rawRotationProxyBodyHandSpace' 'Proxy constraint writes must not pass the stored raw-rotation BODY relation directly as actual proxy-local space.'
