...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.rawRotationProxyBodyHandSpace\s*=\s*frozen\.rawRotationProxyBodyHandSpace' 'Proxy authority must capture BODY relation in the frozen raw-rotation/palm-translation frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'computeGrabConstraintBodyRelationInProxySpace[\s\S]*computeBodyRelationForActualProxyFrame' 'Proxy constraint writes must convert the frozen raw-rotation BODY relation into the actual generated proxy body-A frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyTransformProxySpace\s*=\s*[\s\S]*computeGrabConstraintBodyRelationInProxySpace\([\s\S]*rawHandWorldTransform,[\s\S]*proxyWorldTransform,[\s\S]*_grabFrame\.rawRotationProxyBodyHandSpace' 'Proxy constraint creation must seed the ragdoll angular target from the actual proxy-space conversion of the frozen raw-rotation BODY relation.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'updateProxyConstraintGrabDriveTarget[\s\S]*outDesiredBodyWorld\s*=\s*multiplyTransforms\(authorityFrame,\s*_grabFrame\.rawRotationProxyBodyHandSpace\)[\s\S]*desiredBodyTransformProxySpace\s*=[\s\S]*computeGrabConstraintBodyRelationInProxySpace' 'Held updates must preserve raw-authority desired body telemetry while writing solver angular target in actual proxy body-A space.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'createProxyConstraintGrabDrive[\s\S]*desiredBodyTransformProxySpace\s*=\s*_grabFrame\.rawRotationProxyBodyHandSpace' 'Proxy constraint creation must not hand the frozen raw authority relation directly to hknp body A.'
