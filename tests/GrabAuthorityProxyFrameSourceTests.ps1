...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintUsesMotionBodyAtGrab\s*=\s*hasMotionBodyWorldAtGrab\s*&&\s*motionBodySourceAtGrab\s*==\s*body_frame::BodyFrameSource::MotionCenterOfMass' 'Proxy authority must use the live solver frame for hknp constraint body-B data when FO4VR exposes it.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintBodyWorldAtGrab\s*=\s*constraintUsesMotionBodyAtGrab\s*\?\s*motionBodyWorldAtGrab\s*:\s*grabBodyWorldAtGrab' 'Proxy authority must encode transform-B and angular target data against the live solver frame, with BODY only as fallback.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'writeGrabTransformARotation\(transformARotation,\s*transformAFrameLocal\.rotate\)' 'Proxy target updates must refresh transform-A from the raw-hand angular authority frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'writeGrabRagdollAngularTarget\(targetBRca,\s*desiredBodyTransformBodyASpace,\s*transformAFrameLocal\.rotate\)' 'Proxy target updates must refresh target_bRca from the body-A/object relation plus raw-hand transform-A composition.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyTransformBodyASpace\s*=\s*_grabFrame\.constraintBodyHandSpace' 'Ragdoll atom target math must use the object-to-proxy body-pair relation and leave raw-hand authority to transform-A.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryGetGrabDriveObjectWorldTransform[\s\S]*return\s+tryResolveLiveBodyWorldTransform\(world,\s*bodyId,\s*outTransform\)' 'Proxy runtime error and pivot telemetry must read body-B through the live solver frame used by the constraint.'
...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintUsesMotionBodyAtGrab\s*=\s*false' 'Proxy authority must not force hknp constraint body-B data into BODY when the live solver frame is readable.'
