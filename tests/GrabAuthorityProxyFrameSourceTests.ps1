...
Require-Text 'src/physics-interaction/hand/Hand.h' 'tryResolveLivePalmAnchorGrabReference' 'Dynamic grab must resolve the grab-only palm anchor body as the hand-side reference.'
...
Require-Text 'src/physics-interaction/hand/HandBoneColliderSet.cpp' '_latestPalmAnchorTarget\s*=\s*anchorFrame\.transform[\s\S]*_latestPalmAnchorGrabTarget\s*=\s*grabAnchorFrame\.transform' 'Generated palm target and raw-rotation grab anchor target must be captured separately.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::tryResolveLivePalmAnchorReference[\s\S]*tryResolveLiveBodyWorldTransform\(world,\s*_handBody\.getBodyId\(\)' 'Regular live palm reference must still read the normal palm-anchor hknp body frame.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::tryResolveLivePalmAnchorGrabReference[\s\S]*tryResolveLiveBodyWorldTransform\(world,\s*_grabPalmAnchorBody\.getBodyId\(\)' 'Grab live palm authority must read the raw-rotation PalmAnchorGrab hknp body frame.'
Require-Text 'src/physics-interaction/hand/HandBoneColliderSet.h' 'publishAtomicBodyIds\(const BethesdaPhysicsBody& palmAnchorBody,\s*bool isLeft\)' 'PalmAnchorGrab must stay out of the published hand collider/contact body set.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::getGrabAuthorityProxyDebugSnapshot\(RE::hknpWorld\* world,\s*const RE::NiTransform& rawHandWorld[\s\S]*tryResolveLivePalmAnchorGrabReference\(world,\s*palmReference\)[\s\S]*makePalmAnchorGrabAuthorityBaseFrame\(palmReference\.world\)[\s\S]*makeGrabStartupCaptureAuthorityFrame\(rawHandWorld,\s*palmAuthorityBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(startupCaptureFrameWorld,\s*_isLeft\)' 'Idle proxy debug target must show the raw-hand grab-start capture offset without changing the runtime proxy drive frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*tryResolveLivePalmAnchorGrabReference[\s\S]*livePalmAnchorGrabMotionFrame' 'Proxy authority must resolve from the live PalmAnchorGrab body frame.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'applyRuntimeGrabAuthorityProxyOffsetToFrame[\s\S]*Do not rebind rotation here[\s\S]*\(void\)rawHandWorld[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyFrameWorld,\s*isLeft\)' 'Runtime proxy offset must preserve the resolved PalmAnchorGrab rotation before applying the hidden seat offset.'
...
Require-Text 'src/physics-interaction/hand/Hand.h' 'makePalmAnchorGrabAuthorityBaseFrame' 'Proxy authority must have one explicit boundary that consumes the PalmAnchorGrab base before the configured offset is attached.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*makePalmAnchorGrabAuthorityBaseFrame\(palmReference\.world\)[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(proxyBaseWorld,\s*rawHandWorld,\s*_isLeft\)' 'Resolved hidden proxy frame must keep the live PalmAnchorGrab origin and rotation while applying the local offset.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*tryResolveLivePalmAnchorGrabReference\(world,\s*palmReference\)[\s\S]*makePalmAnchorGrabAuthorityBaseFrame\(palmReference\.world\)[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(proxyBaseWorld,\s*fallbackHandWorldTransform,\s*_isLeft\)\.translate' 'Pivot-A must use the live PalmAnchorGrab origin and rotation while routing the local offset through the resolved grab reference frame.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::tryComputeGrabRawRollPalmPocketPivotAWorld[\s\S]*tryResolveLivePalmAnchorGrabReference\(world,\s*palmReference\)[\s\S]*makePalmAnchorGrabAuthorityBaseFrame\(palmReference\.world\)[\s\S]*makeGrabStartupCaptureAuthorityFrame\(rawHandWorldTransform,\s*palmAuthorityBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(startupCaptureFrameWorld,\s*_isLeft\)\.translate' 'Palm-pocket pivot capture must apply the configured offset through the raw-rotation PalmAnchorGrab authority frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'PROXY GRAB FRAME MISMATCH[\s\S]*rawToProxyTarget[\s\S]*targetAxisDeg[\s\S]*liveAxisDeg[\s\S]*determinantTarget[\s\S]*targetRowsI[\s\S]*transformBInv' 'Proxy after-solve telemetry must expose the raw/proxy frame mismatch, palm-basis axes, and constraint-frame invariants without relying on verbose grab logging.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'RAGDOLL ANGULAR PROBE[\s\S]*beforeErr[\s\S]*afterErr[\s\S]*reduce[\s\S]*axisDot[\s\S]*forceA[\s\S]*targetRowsI[\s\S]*transformBInv[\s\S]*gripBefore[\s\S]*pivotLever' 'Proxy after-solve telemetry must prove whether the ragdoll angular atom reduces body rotation error independently from the linear pivot.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationRows\(targetBRca,\s*targetIdentityRotation\)' 'FO4VR custom grab angular setup must write target_bRca as the zero-error identity relation because transform-B already carries body-to-hand alignment.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteractionDebugOverlay.inl' 'addProxySemanticAxes[\s\S]*tryGetPalmAnchorTarget\(generatedPalmWorld\)[\s\S]*tryResolveLivePalmAnchorGrabReference\(hknp,\s*livePalm\)[\s\S]*tryGetPalmAnchorGrabTarget\(palmAnchorGrabTargetWorld\)[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(palmAnchorGrabBaseWorld,\s*rawHandWorld,\s*isLeft\)[\s\S]*RightGrabPalmGeneratedDirect[\s\S]*RightGrabSemanticHandFrame' 'Narrow proxy semantic overlay must draw generated palm, resolved PalmAnchorGrab authority, and semantic grip axes together.'
