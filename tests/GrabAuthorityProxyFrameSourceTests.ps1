...
Require-Text 'src/physics-interaction/core/PhysicsInteractionFrame.inl' 'tryComputeGrabProxyLocalPalmPocketFrameWorld\(hknp,\s*proxyFrameWorld\)[\s\S]*input\.grabAnchorWorld\s*=\s*proxyFrameWorld\.translate[\s\S]*closeSelectionBasisWorld\s*=\s*makeGeneratedProxyAuthorityRelationFrame\(proxyFrameWorld\)[\s\S]*input\.palmNormalWorld\s*=\s*computePalmNormalFromHandBasis\(closeSelectionBasisWorld,\s*isLeft\)[\s\S]*input\.pointingWorld\s*=\s*computePointingVectorFromHandBasis\(input\.rawHandWorld,\s*isLeft\)[\s\S]*input\.closeSelectionDirectionWorld\s*=\s*computeCloseSelectionDirectionFromHandBasis\(closeSelectionBasisWorld,\s*isLeft\)[\s\S]*input\.farSelectionDirectionWorld\s*=\s*computeFarSelectionDirectionFromHandBasis\(input\.rawHandWorld,\s*isLeft\)[\s\S]*input\.pinchDirectionWorld\s*=\s*computePinchDetectionDirectionFromHandBasis\(closeSelectionBasisWorld,\s*isLeft\)' 'Close selection direction must use generated/proxy origin and basis while far selection direction remains raw/controller driven.'
...
Require-Text 'src/RockConfig.cpp' 'bDebugGrabTimelineTrace[\s\S]*iDebugGrabTimelineTraceIntervalFrames' 'RockConfig must read structured grab timeline tracing from INI.'
