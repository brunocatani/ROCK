...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(proxyBaseWorld,\s*rawHandWorld,\s*_isLeft\)[\s\S]*makeRawRotationPalmTranslationFrame\(rawHandWorld,\s*proxySeatWorld\)' 'Resolved hidden proxy frame must keep generated palm translation/offset while driving body-A rotation from raw hand authority.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'generatedProxyWorld\s*=\s*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyBaseWorld,\s*_isLeft\)[\s\S]*rawRotationProxyWorld\s*=[\s\S]*makeRawRotationPalmTranslationFrame\(pending\.rawHandWorld,\s*generatedProxyWorld\)[\s\S]*pending\.proxyWorld\.rotate\s*=\s*rawRotationProxyWorld\.rotate[\s\S]*resolveActiveGrabAuthorityPivotAWorld\(world,\s*pending\.rawHandWorld,\s*activeProxyPivotAWorld\)[\s\S]*pending\.proxyWorld\.translate\s*=\s*activeProxyPivotAWorld' 'Physics flush must preserve the active raw-roll/pinch pivot translation while driving proxy/body-A rotation from raw hand authority.'
