...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'PROXY GRAB FRAME MISMATCH[\s\S]*rawToProxyTarget[\s\S]*targetAxisDeg[\s\S]*liveAxisDeg[\s\S]*determinantTarget[\s\S]*targetRowsIdentity[\s\S]*targetColsIdentity' 'Proxy after-solve telemetry must expose the raw/proxy frame mismatch, palm-basis axes, and identity target invariants without relying on verbose grab logging.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'RAGDOLL ANGULAR PROBE[\s\S]*beforeErr[\s\S]*afterErr[\s\S]*reduce[\s\S]*axisDot[\s\S]*bodyErrProxyAngle[\s\S]*bodyErrProxyAxis[\s\S]*bRcaErrProxyAngle[\s\S]*bRcaErrProxyAxis[\s\S]*feedErrProxyAngle[\s\S]*feedErrProxyAxis[\s\S]*bodyToBRcaAxisDot[\s\S]*feedToBRcaAxisDot[\s\S]*forceA[\s\S]*targetRowsIdentity[\s\S]*gripBefore[\s\S]*pivotLever' 'Proxy after-solve telemetry must decompose intended body correction, live ragdoll correction, and motor-feed mismatch in the same proxy-local axis basis.'
...
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeIdentityRagdollTarget[\s\S]*targetBRca\[0\]\s*=\s*1\.0f[\s\S]*targetBRca\[5\]\s*=\s*1\.0f[\s\S]*targetBRca\[10\]\s*=\s*1\.0f' 'FO4VR custom grab angular setup must keep target_bRca as identity under the raw-authority frame contract.'
