...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'applyRuntimeGrabAuthorityProxyOffsetToFrame[\s\S]*computeGrabAuthorityProxyOffsetLocalGame\(isLeft\)[\s\S]*localVectorToWorld\(rawHandWorld,\s*localOffset\)' 'Runtime proxy offset must apply the hidden proxy seat offset through raw hand basis for both hands.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(proxyBaseWorld,\s*rawHandWorld,\s*_isLeft\)' 'Resolved hidden proxy frame must keep the generated palm base frame while applying the live offset through raw hand basis.'
