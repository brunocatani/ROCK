...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyProxyConstraintAngularVelocityDrive|setHeldAngularVelocity|makeNativeAngularBoundaryTargetFromVisualTarget' 'Proxy authority must not keep the removed direct held-body angular velocity writer.'
Require-Text 'src/physics-interaction/grab/GrabConstraint.cpp' 'setGrabMotorAtomsActive\(header,\s*true,\s*true\)' 'Proxy authority must enable the FO4VR ragdoll angular atom together with the linear motors.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'writeGrabTransformARotation\(transformARotation,\s*transformAFrameLocal\.rotate\)' 'Proxy authority must write transform-A rotation from the proxy-to-raw-hand angular frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'writeGrabRagdollAngularTarget\(targetBRca,\s*desiredBodyTransformAuthoritySpace,\s*transformAFrameLocal\.rotate\)' 'Proxy authority must update target_bRca through the FO4VR ragdoll angular target equation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'tryGetBodyArrayWorldTransform\(world,\s*proxyBodyId,\s*proxyReadbackBetween\)' 'Proxy authority must consume BODY readback for body-A instead of silently switching to MOTION/COM.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'proxyReadbackSourceBetween\s*=\s*proxyReadbackBetweenOk\s*\?\s*body_frame::BodyFrameSource::BodyTransform\s*:\s*body_frame::BodyFrameSource::Fallback' 'Proxy authority logs must make the body-A readback source explicit.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'ComputeHardKeyFrame_t[\s\S]*offsets::kFunc_ComputeHardKeyFrame' 'Proxy velocity diagnostics may use the FO4VR native hard-keyframe velocity boundary when palm motion velocity is unavailable.'
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'writeHavokRotationColumns\(transformBRotation,\s*bodyToAuthorityRotation\)' 'FO4VR custom grab angular setup must write transform-B as Havok column blocks, matching the body-to-authority relation.'
Require-Text 'src/physics-interaction/grab/GrabConstraintMath.h' 'composeRagdollAngularTargetRotation[\s\S]*multiplyStoredRotations\(bodyToAuthorityRotation,\s*transformALocalRotation\)' 'FO4VR custom grab angular setup must compose target_bRca with transform-A rotation like the native ragdoll updater.'
