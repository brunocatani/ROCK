...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::getGrabAuthorityProxyDebugSnapshot[\s\S]*tryResolveLivePalmAnchorReference\(world,\s*palmReference\)[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*makeGrabAuthorityProxyBaseFrame\(rawHandWorld,\s*palmAuthorityBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(out\.palmAuthorityBaseWorld,\s*_isLeft\)' 'Idle proxy debug target must use live palm-anchor translation with raw hand rotation before applying the offset.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'makeGrabAuthorityProxyBaseFrame[\s\S]*rawHandWorld\.rotate[\s\S]*rawHandWorld\.scale' 'Hidden proxy base frame must use raw hand rotation/scale with palm-anchor translation before local offset application.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*makeGrabAuthorityProxyBaseFrame\(rawHandWorld,\s*palmAuthorityBaseWorld\)' 'Resolved hidden proxy frame must use raw hand rotation, not live palm BODY rotation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'makeGrabAuthorityProxyBaseFrame\(pending\.rawHandWorld,\s*palmAuthorityBaseWorld\)[\s\S]*pending\.proxyWorld\s*=\s*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyBaseWorld,\s*_isLeft\)' 'Per-frame hidden proxy target must preserve raw hand rotation before applying the local offset.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*makeGrabAuthorityProxyBaseFrame\(fallbackHandWorldTransform,\s*palmAuthorityBaseWorld\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyBaseWorld,\s*_isLeft\)\.translate' 'Pivot-A must use live palm-anchor translation with raw hand rotation before applying the configured offset.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteractionDebugOverlay.inl' 'drawGrabAuthorityProxy[\s\S]*getGrabAuthorityProxyDebugSnapshot\(hknp,\s*rawHandWorld,\s*snapshot\)[\s\S]*RightGrabAuthorityProxyTarget' 'Proxy overlay must draw the computed target frame from the raw hand rotation before a grab creates a readback proxy body.'
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outDesiredObjectWorld\s*=\s*multiplyTransforms\(proxyWorldTransform,\s*_grabFrame\.rawHandSpace\)' 'Proxy body-A convention must not reuse the retired raw-controller object relation path.'
