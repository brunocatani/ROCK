...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmProxyWorld\s*=\s*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyBaseWorld,\s*_isLeft\)[\s\S]*pending\.proxyWorld\s*=\s*makeConstraintProxyBodyFrame\(pending\.rawHandWorld,\s*palmProxyWorld\)' 'Per-frame hidden proxy target must keep the configured local offset while rotating the solver body through raw-rotation authority.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.constraintBodyHandSpace\s*=\s*_grabFrame\.rawRotationProxyBodyHandSpace' 'Proxy authority must store the active solver-space BODY relation in the same raw-rotation frame used by hknp body-A.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'makeConstraintProxyBodyFrame[\s\S]*return\s+makeRawRotationPalmTranslationFrame\(rawHandWorld,\s*palmProxyWorld\)' 'Hidden no-contact proxy body-A must use the raw-rotation/palm-translation solver frame instead of generated palm rotation.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.pivotAHandBodyLocalGame\s*=\s*grab_frame_math::computePivotAHandBodyLocal\(rawRotationProxyFrameWorldAtGrab,\s*grabPivotAWorld\)' 'Frozen pivot-A local telemetry must be expressed in the same raw-rotation solver proxy frame used by hknp body-A.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintProxyWorldTransform\s*=\s*makeConstraintProxyBodyFrame\(rawHandWorldTransform,\s*proxyWorldTransform\)[\s\S]*createGrabConstraint\(world,[\s\S]*constraintProxyWorldTransform' 'Proxy constraint creation must create hknp body-A in the raw-rotation/palm-translation solver frame.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'proxyAuthorityWorld\s*=\s*makeConstraintProxyBodyFrame\(handWorldTransform,\s*palmProxyAuthorityWorld\)[\s\S]*queueProxyGrabAuthorityTarget\(\s*proxyAuthorityWorld' 'Held updates must queue the raw-rotation/palm-translation solver proxy target.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'queueGeneratedKeyframedBodyTarget\(\s*_grabAuthorityProxyDriveState,\s*pending\.proxyWorld' 'Proxy authority must queue the raw-rotation solver proxy frame through the generated keyframed-body drive path.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'grabPivotAForPrimaryChoice\s*=\s*computeGrabStartupCapturePivotAWorld\(world,\s*handWorldTransform\)' 'Close-grab acquisition must use the raw-rotation startup capture pivot before body and mesh authority are frozen.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'acquisitionGrabPivotAWorld\s*=\s*computeGrabStartupCapturePivotAWorld\(world,\s*handWorldTransform\)' 'Three-phase acquisition must use the same raw-rotation startup capture pivot.'
