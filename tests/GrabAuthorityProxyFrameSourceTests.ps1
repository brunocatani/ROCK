...
Require-Text 'src/physics-interaction/core/PhysicsInteraction.h' 'tryGetGrabAuthorityAnchorWorld' 'Shared runtime/API palm seat readers must expose one live-or-fallback grab authority anchor helper.'
...
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::getGrabAuthorityProxyDebugSnapshot\(RE::hknpWorld\* world,\s*const RE::NiTransform& rawHandWorld[\s\S]*out\.proxyTargetWorld\s*=\s*applyGrabAuthorityProxyLocalOffsetToFrame\(rawHandWorld,\s*_isLeft\)' 'Idle proxy debug target must fall back to the corrected raw-hand proxy seat when live palm-body readback is unavailable.'
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'PhysicsInteraction::tryGetGrabAuthorityAnchorWorld\(bool isLeft,\s*RE::NiPoint3& outAnchorWorld\) const[\s\S]*tryGetRootFlattenedHandTransform\(isLeft,\s*rawHandWorld\)[\s\S]*computeGrabPivotAWorld\(world,\s*rawHandWorld\)' 'Shared runtime/API grab authority anchor helper must resolve the same live-or-fallback pivot A path as dynamic grab.'
...
Require-Text 'src/api/ROCKApi.cpp' 'apiGetPalmPosition\([\s\S]*tryGetGrabAuthorityAnchorWorld\(isLeft,\s*palmPosition\)' 'ROCK API palm position must use the same live-or-fallback grab authority seat as runtime dynamic grab.'
Reject-Text 'src/api/ROCKApi.cpp' 'apiGetPalmPosition\([\s\S]*computeFallbackGrabAuthorityProxySeatWorld' 'ROCK API palm position must not bypass live palm authority with a fallback-only seat read.'
Require-Text 'src/physics-interaction/grab/GrabFinger.h' 'liveThumbAlternateCurlNormalWorld\([\s\S]*grabAnchorWorld - baseWorld[\s\S]*primaryNormalWorld[\s\S]*stablePerpendicularDirection\(openVectorWorld\)' 'Thumb alternate curl fallback must stay geometric and derived from live palm/grab inputs instead of handed world defaults.'
Reject-Text 'src/physics-interaction/grab/GrabFinger.h' 'RE::NiPoint3\(0\.0f,\s*isLeft \? -1\.0f : 1\.0f,\s*0\.0f\)|RE::NiPoint3\(0\.0f,\s*isLeft \? 1\.0f : -1\.0f,\s*0\.0f\)' 'Thumb alternate curl fallback must not resurrect handed world +/-Y defaults after the palm-axis correction.'
