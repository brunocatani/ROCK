...
Require-Text 'src/physics-interaction/core/PhysicsInteractionDebugOverlay.inl' 'drawGrabProxySemanticAxesOnly[\s\S]*makeSemanticGripFrame[\s\S]*buildGrabPocketFrameWithPalmCenter[\s\S]*matrixFromAxes<RE::NiMatrix3>\([\s\S]*semanticPocket\.fingerForwardWorld[\s\S]*semanticPocket\.thumbSideWorld[\s\S]*semanticPocket\.palmNormalWorld' 'Narrow proxy semantic overlay must build its semantic frame from finger-forward, thumb-side, and palm-normal axes.'
Require-Text 'src/physics-interaction/core/PhysicsInteractionDebugOverlay.inl' 'addProxySemanticAxes[\s\S]*tryGetPalmAnchorTarget\(generatedPalmWorld\)[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(generatedPalmWorld,\s*rawHandWorld,\s*isLeft\)[\s\S]*RightGrabPalmGeneratedDirect[\s\S]*RightGrabProxyReadback[\s\S]*RightGrabSemanticHandFrame' 'Narrow proxy semantic overlay must draw generated palm, runtime generated proxy, and semantic grip axes together.'
