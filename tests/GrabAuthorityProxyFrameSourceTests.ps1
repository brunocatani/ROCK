...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'makeGrabStartupCaptureAuthorityFrame[\s\S]*return\s+palmAnchorWorld' 'Grab-start capture must apply the configured local offset through the corrected palm proxy authority basis.'
...
Require-Text 'src/physics-interaction/hand/HandFrame.h' 'applyRuntimeGrabAuthorityProxyOffsetToFrame[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyFrameWorld,\s*isLeft\)' 'Runtime proxy offset must apply both hand offsets through corrected palm proxy space.'
...
Require-Text 'src/physics-interaction/hand/Hand.h' 'makePalmAnchorGrabAuthorityBaseFrame' 'Proxy authority must have one explicit boundary that adapts live palm translation and latest generated target rotation.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'resolveGrabAuthorityProxyFrame[\s\S]*makePalmAnchorGrabAuthorityBaseFrame\(palmReference\.world\)[\s\S]*applyRuntimeGrabAuthorityProxyOffsetToFrame\(proxyBaseWorld,\s*rawHandWorld,\s*_isLeft\)' 'Resolved hidden proxy frame must keep the live palm base origin while applying the corrected proxy-space local offset.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'makeCorrectedPalmProxyAuthorityFrame\(proxyFrameWorldAtGrab\)' 'Proxy authority must freeze object and BODY relations through the corrected palm proxy frame.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.rawRotationProxyHandSpace\s*=\s*frozen\.rawRotationProxyHandSpace' 'Proxy authority must capture object relation in the corrected palm proxy frame through the frozen authority contract.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'applyFrozenGrabAuthorityFrameToGrabFrame[\s\S]*frame\.rawRotationProxyBodyHandSpace\s*=\s*frozen\.rawRotationProxyBodyHandSpace' 'Proxy authority must capture BODY relation in the same corrected palm proxy frame used by the live angular target.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'kGrabObjectRotationReferenceName\s*=\s*"correctedPalmProxy"' 'The corrected palm proxy authority must be the only production rotation reference.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'desiredBodyTransformProxySpace\s*=\s*_grabFrame\.rawRotationProxyBodyHandSpace' 'Proxy constraint creation must seed the ragdoll angular target from the same corrected proxy BODY relation used by held updates.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'outDesiredObjectWorld\s*=\s*multiplyTransforms\(authorityFrame,\s*_grabFrame\.rawRotationProxyHandSpace\)' 'Proxy desired object frame must always compose from the validated corrected palm proxy relation.'
