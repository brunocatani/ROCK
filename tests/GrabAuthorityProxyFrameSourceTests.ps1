...
Require-Text 'src/physics-interaction/core/PhysicsInteractionFrame.inl' 'input\.grabAnchorWorld\s*=\s*hand\.computeGrabStartupCapturePivotAWorld\(hknp,\s*input\.rawHandWorld\)' 'Frame debug/selection grab anchor must show the generated/proxy-local palm-pocket startup point.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'palmPocketPivotAWorld\s*=\s*computeGrabStartupCapturePivotAWorld\(world,\s*handWorldTransform\)' 'Close grab palm-pocket acquisition must seed from the generated/proxy-local startup point.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabPivotAWorld[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(proxyBaseWorld,\s*_isLeft\)\.translate' 'Pivot-A must use the live palm anchor base origin while routing the runtime offset through generated/proxy local space.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::tryComputeGrabProxyLocalPalmPocketPivotAWorld[\s\S]*generatedColliderFrameToGrabAuthorityFrame\(palmReference\.world\)[\s\S]*applyGrabAuthorityProxyLocalOffsetToFrame\(palmAuthorityBaseWorld,\s*_isLeft\)\.translate' 'Proxy-local palm-pocket pivot capture must apply the configured offset through generated/proxy local space.'
Require-Text 'src/physics-interaction/hand/Hand.cpp' 'Hand::computeGrabStartupCapturePivotAWorld[\s\S]*tryComputeGrabProxyLocalPalmPocketPivotAWorld\(world,\s*proxyLocalPalmPocketPivotWorld\)' 'Grab-start pivot capture must delegate to the shared generated/proxy-local palm-pocket helper.'
