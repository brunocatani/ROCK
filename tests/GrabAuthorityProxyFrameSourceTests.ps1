...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'RAGDOLL ANGULAR PROBE[\s\S]*beforeErr[\s\S]*afterErr[\s\S]*reduce[\s\S]*axisDot[\s\S]*bodyErrProxyAngle[\s\S]*bodyErrProxyAxis[\s\S]*bRcaErrProxyAngle[\s\S]*bRcaErrProxyAxis[\s\S]*feedErrProxyAngle[\s\S]*feedErrProxyAxis[\s\S]*bodyToBRcaAxisDot[\s\S]*feedToBRcaAxisDot[\s\S]*forceA[\s\S]*targetRowsInv[\s\S]*gripBefore[\s\S]*pivotLever' 'Proxy after-solve telemetry must decompose intended body correction, live ragdoll correction, and motor-feed mismatch in the same proxy-local axis basis.'
