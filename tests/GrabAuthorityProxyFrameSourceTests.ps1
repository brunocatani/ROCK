...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'proxyFrameWorldAtGrab\s*=\s*makeProxyFrameWithPivotOrigin\(proxyFrameWorldAtGrab,\s*grabPivotAWorld\)[\s\S]*buildSplitGrabFrameFromDesiredObject\(\s*handWorldTransform,\s*proxyFrameWorldAtGrab' 'Grab-start hand/object relation must be captured from the active pocket-origin proxy frame, not stale hknp readback.'
...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_grabFrame\.rawRotationProxyBodyHandSpace\s*=\s*grab_frame_math::objectInFrameSpace\(rawRotationProxyFrameWorldAtGrab,\s*desiredBodyWorld\)' 'Proxy authority must capture BODY relation in the same hybrid raw-rotation/palm-translation frame used by the live angular target.'
