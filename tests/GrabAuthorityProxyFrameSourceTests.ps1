...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'livePivotAWorld\s*=\s*proxyAuthorityWorld\.translate' 'TouchHeld final freeze must not fall back to the generated proxy origin as the pocket pivot.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'finalizeTouchHeldSamePointAuthority[\s\S]*gripEvidencePointWorld\(_grabFrame,\s*currentNodeWorld\)[\s\S]*shiftObjectToAlignGripWithPocket\(currentNodeWorld,\s*activeProxyPivotAWorld,\s*currentGripWorld\)[\s\S]*_grabAuthorityPivotAProxyLocalGame\s*=\s*\{\}' 'TouchHeld final freeze must keep the same selected evidence point, seat it at the live active proxy pivot, and keep proxy-local pivot A at zero.'
