...
Require-Text 'src/physics-interaction/native/GeneratedKeyframedBodyDrive.cpp' 'const RE::hkTransformf targetHavok = makeHavokTransform\(state\.pendingTarget\)' 'Normal generated-body movement must convert game-space targets to Havok-space hkTransformf before Bethesda keyframe drive.'
...
Require-Text 'src/physics-interaction/core/PhysicsInteraction.cpp' 'driveGeneratedBodiesFromPhysicsStep' 'PhysicsInteraction must own the actual generated-body step-drive callback.'
Require-Text 'src/physics-interaction/native/HavokPhysicsTiming.h' 'return timing\.rawDeltaSeconds' 'Generated-body keyframe drive must use the whole bhkWorld frame delta, not an implicit multiplied substep delta.'
...
Reject-Text 'src/physics-interaction/weapon/WeaponCollision.cpp' 'result\.rotate\s*=\s*weapon_collision_geometry_math::transposeRotation\(weaponRootTransform\.rotate\)' 'Weapon generated body targets must remain ordinary Ni world transforms; Havok rotation conversion belongs in the shared generated-body drive.'
