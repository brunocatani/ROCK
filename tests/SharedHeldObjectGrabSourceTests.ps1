...
Require-Text 'src/physics-interaction/hand/HandSelection.h' 'kNearCastRadiusGameUnits\s*=\s*3\.5f[\s\S]*kNearCastDistanceGameUnits\s*=\s*7\.0f' 'Near selection cast dimensions must remain fixed compiled policy.'
Require-Text 'src/physics-interaction/object/ObjectDetection.cpp' 'kNearCastDistanceGameUnits[\s\S]*kNearCastRadiusGameUnits' 'Runtime near selection must consume the fixed cast policy.'
