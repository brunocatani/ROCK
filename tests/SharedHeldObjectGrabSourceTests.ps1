...
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' '_heldBodyIds\s*=\s*buildCommittedHeldBodyIds\(objectBodyId\.value,\s*preparedBodySet,\s*sharedContext,\s*adoptedPeerHeldBodySet\)' 'Grab commit must use the shared body-set helper for held-body activation, flag leases, release, and restoration.'
...
Require-Text 'data/config/ROCK.ini' 'fGrabNearbyDampingRadius\s*=\s*7\.0' 'Packaged ROCK.ini must keep nearby grab damping local to the hand.'
Require-Text 'src/RockConfig.cpp' '"fGrabNearbyDampingRadius"[\s\S]*kDefaultNearbyGrabDampingRadiusGameUnits[\s\S]*0\.0f[\s\S]*kDefaultNearbyGrabDampingRadiusGameUnits' 'Runtime config loading must clamp old broad nearby damping radii down to the local hand radius.'
