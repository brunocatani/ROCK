...
Reject-Text 'src/physics-interaction/hand/HandGrab.cpp' 'constraintDrivePivotBBodyLocalGame' 'Constraint grabs must not keep the stale computed transform-B helper.'
Require-Text 'src/physics-interaction/hand/HandGrab.cpp' 'activeProxyConstraintPivotBLocalGame\(\)' 'Constraint grabs must create and measure against the frozen active transform-B pivot, not a recomputed visual/object point.'
