...
int main()
{
    ...
    RE::NiTransform contactLive = rock::transform_math::makeIdentityTransform<RE::NiTransform>();
    RE::NiTransform penetratedRequest = contactLive;
    penetratedRequest.translate.x = 10.0f;
    penetratedRequest.rotate = rotationZ90();
    const RE::NiTransform contactAnchor = makeBoundedContactAnchor(
        contactLive,
        penetratedRequest,
        0.25f,
        0.5f);
    ok &= expectNear("contact anchor discards translation debt", contactAnchor.translate.x, 0.05556f, 0.002f);
    ok &= rotationDeltaDegrees(contactLive, contactAnchor) <= 0.51f;

    RE::NiTransform previousRaw = rock::transform_math::makeIdentityTransform<RE::NiTransform>();
    previousRaw.translate.x = 10.0f;
    RE::NiTransform retreatingRaw = previousRaw;
    retreatingRaw.translate.x = 9.0f;
    const RE::NiTransform firstRetreatTarget = advanceSurfaceCoupledTarget(
        previousRaw,
        retreatingRaw,
        contactAnchor);
    ok &= expectNear(
        "contact clutch applies first retreat immediately",
        firstRetreatTarget.translate.x,
        contactAnchor.translate.x - 1.0f);

    const auto opposedRecovery = recoverSurfaceCoupledTarget(
        firstRetreatTarget,
        previousRaw,
        retreatingRaw,
        0.2f,
        0.2f,
        0.25f);
    const float opposedErrorBefore = translationDeltaGameUnits(firstRetreatTarget, retreatingRaw);
    const float opposedErrorAfter = translationDeltaGameUnits(opposedRecovery.target, retreatingRaw);
    ok &= opposedRecovery.translationOpposedRawMotion;
    ok &= opposedErrorAfter < opposedErrorBefore;
    ok &= expectNear(
        "opposed recovery consumes at most one quarter retreat",
        opposedRecovery.target.translate.x - firstRetreatTarget.translate.x,
        0.25f);
    ok &= expectNear(
        "opposed recovery preserves three quarters retreat",
        opposedRecovery.target.translate.x - contactAnchor.translate.x,
        -0.75f);

    RE::NiTransform stableRaw = rock::transform_math::makeIdentityTransform<RE::NiTransform>();
    stableRaw.translate.x = 10.0f;
    RE::NiTransform stableCoupled = stableRaw;
    stableCoupled.translate.x = 0.0f;
    const auto continuousRecovery = recoverSurfaceCoupledTarget(
        stableCoupled,
        stableRaw,
        stableRaw,
        0.2f,
        0.2f,
        0.25f);
    ok &= expectNear("continuous bounded-step recovery", continuousRecovery.target.translate.x, 2.92893f);
    ok &= translationDeltaGameUnits(continuousRecovery.target, stableRaw) <
          translationDeltaGameUnits(stableCoupled, stableRaw);

    RE::NiTransform nextRaw = retreatingRaw;
    nextRaw.translate.x = 8.0f;
    const RE::NiTransform nextFollowed = advanceSurfaceCoupledTarget(
        retreatingRaw,
        nextRaw,
        opposedRecovery.target);
    const auto nextRecovery = recoverSurfaceCoupledTarget(
        nextFollowed,
        retreatingRaw,
        nextRaw,
        0.2f,
        0.2f,
        0.25f);
    ok &= translationDeltaGameUnits(nextRecovery.target, nextRaw) <
          translationDeltaGameUnits(nextFollowed, nextRaw);
