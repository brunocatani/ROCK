...
Require-Pattern $policy 'targetColliders\s*=\s*colliders\s*&&\s*input\.targetColliders[\s\S]*colliderPhaseDiagnostics\s*=\s*[\s]*colliders\s*&&\s*input\.colliderPhaseDiagnostics[\s\S]*handColliders\s*=\s*colliders\s*&&\s*input\.handColliders[\s\S]*handBoneColliders\s*=\s*colliders\s*&&\s*input\.handBoneColliders[\s\S]*bodyBoneColliders\s*=\s*colliders\s*&&\s*input\.bodyBoneColliders[\s\S]*dynamicHandColliders\s*=\s*colliders\s*&&\s*input\.dynamicHandColliders[\s\S]*weaponColliders\s*=\s*colliders\s*&&\s*input\.weaponColliders[\s\S]*dynamicWeaponColliders\s*=\s*[\s]*colliders\s*&&\s*input\.dynamicWeaponColliders' 'Every ROCK-owned collider class must resolve through the collider master.'
...
foreach ($path in @(
    ...
    Require-Pattern $path 'DebugDrawColliderPhaseDiagnostics[\s\S]*DebugDrawBodyBoneColliders[\s\S]*DebugDrawWeaponAuthority[\s\S]*DebugDrawLooseWeaponGripZones' "$path must contain the explicit visualization owners."
