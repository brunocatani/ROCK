...
int main()
{
    ...
    ok &= expectBool("body slot rejects ids beyond readable high water",
        ...
        false);
    ok &= expectBool("body slot accepts long-session high water",
        body_frame::bodySlotCanBeRead(42, 9000),
        true);
    ok &= expectBool("body slot rejects suspicious high water beyond readable bound",
        body_frame::bodySlotCanBeRead(42, body_frame::kMaxReadableBodyIndex + 1),
        false);
