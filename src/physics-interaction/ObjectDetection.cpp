    ...
    namespace
    {
            ...
            int& outDuplicateBodies)
        {
            ...
            for (int i = 0; i < numHits; ++i) {
                ...
                ++outCandidates;
                const RE::NiPoint3 hitPoint = hkVectorToNiPoint(hits[i].position);
                ...
                result.distance = isFarSelection ? alongDistance : lateralDistance;
        ...
        RE::TESObjectREFR* otherHandRef)
    {
        ...
        if (logNearMetric) {
            ROCK_LOG_DEBUG(Hand,
                ...
                result.refr ? result.refr->GetFormID() : 0, result.isValid() ? result.distance : -1.0f);
        }
