...
Near/far convergence now has two promotion paths. Exact touch still promotes immediately, but a stable object inside the hand pocket can also promote after the converge timeout. Stability is frame-based and rejects fast separating pocket drift unless there is fresh held-body collision evidence. During that window ROCK may publish a pre-touch acquisition finger pose and visual-only hand target inside a bounded acquisition visual envelope; final mesh local-transform corrections still wait until `TouchHeld` so the object-hand relation is not recaptured while the physics object is settling. Near-converge capture also freezes the current object-to-hand relation instead of forcing the selected mesh point to the palm pivot, and that frozen relation is kept through promotion for that grab; this keeps long or thin objects from receiving an off-center startup correction before the grab is actually seated.
