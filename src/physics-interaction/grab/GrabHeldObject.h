...
namespace rock::grab_held_response
{
    ...
    inline float responseFactor(float positionErrorGameUnits, float fullErrorGameUnits, bool heldBodyColliding, float collisionLeadScale)
    {
        const float fullError = safePositive(fullErrorGameUnits, 20.0f);
        const float errorFactor = clamp01(std::abs(finiteOr(positionErrorGameUnits, 0.0f)) / fullError);
        float factor = 0.25f + 0.75f * errorFactor;
        if (heldBodyColliding) {
            factor *= clamp01(collisionLeadScale);
        }
        return factor;
    }

    template <class Vec3>
    struct AdaptiveHeldLeadInput
    {
        bool enabled = true;
        ...
        float fullErrorGameUnits = 20.0f;
        float leadTimeMaxSeconds = 0.035f;
        float maxLeadDistanceGameUnits = 8.0f;
        float maxAngularLeadDegrees = 18.0f;
        float havokToGameScale = physics_scale::kFallbackHavokToGame;
        float collisionLeadScale = 0.25f;
    };

    template <class Vec3>
    struct AdaptiveHeldLeadOutput
    {
        Vec3 linearLeadGameUnits{};
        float angularLeadRadians = 0.0f;
        float factor = 0.0f;
        bool appliedLinear = false;
        bool appliedAngular = false;
    };

    template <class Vec3>
    inline AdaptiveHeldLeadOutput<Vec3> solveAdaptiveHeldLead(const AdaptiveHeldLeadInput<Vec3>& input)
    {
        AdaptiveHeldLeadOutput<Vec3> out{};
        if (!input.enabled) {
            return out;
        }

        const float leadSeconds = safePositive(input.leadTimeMaxSeconds, 0.0f);
        if (leadSeconds <= 0.0f) {
            return out;
        }

        out.factor = responseFactor(input.positionErrorGameUnits, input.fullErrorGameUnits, input.heldBodyColliding, input.collisionLeadScale);

        if (input.hasHandLocalVelocity) {
            const Vec3 relativeVelocity = input.hasObjectLocalVelocity ?
                sub(input.handLocalVelocityHavok, input.objectLocalVelocityHavok) :
                input.handLocalVelocityHavok;
            const float scaleToGame = safePositive(input.havokToGameScale, physics_scale::kFallbackHavokToGame);
            out.linearLeadGameUnits =
                clampMagnitude(scale(scale(relativeVelocity, leadSeconds * out.factor), scaleToGame), input.maxLeadDistanceGameUnits);
            out.appliedLinear = lengthSquared(out.linearLeadGameUnits) > 0.000001f;
        }

        if (input.hasHandAngularVelocity) {
            constexpr float kDegreesToRadians = 0.01745329251994329577f;
            const float angularSpeed = length(input.handAngularVelocityRadiansPerSecond);
            const float maxAngularLead = std::max(0.0f, finiteOr(input.maxAngularLeadDegrees, 0.0f)) * kDegreesToRadians;
            out.angularLeadRadians = std::min(angularSpeed * leadSeconds * out.factor, maxAngularLead);
            out.appliedAngular = out.angularLeadRadians > 0.000001f;
        }
