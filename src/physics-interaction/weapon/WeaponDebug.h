...
/*
 * Weapon debug helpers are grouped here to keep notification text and diagnostic names out of the core collision/runtime owners.
 */


// ---- WeaponDebugNotificationPolicy.h ----
...
namespace rock::weapon_debug_notification_policy
{
     ...
     * without coupling HUD spam control to weapon authority logic.
     */
    ...
    inline std::string formatWeaponGripNotification(WeaponGripNotificationEvent event, const WeaponNotificationKey& key, const WeaponInteractionDebugInfo& debugInfo)
    {
        ...
        if (event == WeaponGripNotificationEvent::Started) {
            return std::string("[ROCK] WPN GRIP START weapon='") + debugTextOrUnknown(debugInfo.weaponName) + "' form=" + formatFormId(debugInfo.weaponFormId) +
                   " node='" + debugTextOrUnknown(debugInfo.weaponNodeName) + "' driveRoot='" + debugTextOrUnknown(debugInfo.interactionRootName) + "' sourceRoot='" +
                   debugTextOrUnknown(debugInfo.sourceRootName) + "' nif='" + debugTextOrUnknown(debugInfo.sourceName) + "' part=" + nameOf(key.partKind) +
                   " route=" + nameOf(key.interactionKind) + " pose=" + nameOf(key.gripPose) + " body=" + std::to_string(key.bodyId);
        }
