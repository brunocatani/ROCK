...
namespace rock
{
    namespace weapon_collision_internal
    {
        ...
        inline bool weaponVisualNodeVisible(const RE::NiAVObject* node)
        {
            ...
            // NiObject exposes only a non-const IsTriShape accessor.
            const auto* shape = const_cast<RE::NiAVObject*>(node)->IsTriShape();
            return !shape || !weapon_material_visibility::isHidden(shape);
        }
        ...
        using weapon_hierarchy::weaponTransformFinite;
        using weapon_hierarchy::tryResolveDescendantLocalTransform;
        using weapon_hierarchy::tryResolveDescendantWorldTransform;
