...
namespace rock::weapon_grip_transfer
{
    ...
    struct HandGrip
    {
        ...
        {
            const auto finiteFrame = [](const RE::NiTransform& value) {
            ...
            if (!hasFingerPose || !finiteFrame(handWeaponLocal) || (fingerMask & ~0x7FFFu) ||
            ...
            for (std::size_t i = 0; i < fingerValues.size(); ++i) {
                if (!std::isfinite(fingerValues[i]) || ((fingerMask & (1u << i)) && !finiteFrame(fingerLocals[i]))) return false;
            }
            return true;
        }
    };
