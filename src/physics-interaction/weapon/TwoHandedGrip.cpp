...
namespace rock
{
        ...
        const WeaponProviderPartAuthority& providerPartAuthority)
    {
        ...
        bool gunstockBaselineActive = false;
        if (g_rockConfig.rockGunstockModeEnabled &&
            ...
            if (!weapon_support_acquisition_math::
                ...
                ROCK_LOG_WARN(Weapon,
                    "TwoHandedGrip: gunstock support grip start failed closed because the attach baseline was invalid hand={} authored={} provider={} generation={:016X}",
                    supportHandIsLeft ? "left" : "right",
            ...
            supportGrip.gunstockBaseline.inputToGripTargetLocal =
                inputToGripTargetLocal;
            supportGrip.gunstockBaseline.weaponWorldAtCapture =
                weaponNode->world;
            supportGrip.gunstockBaseline.active = true;
            supportGrip.gunstockBaseline.firstPublicationPending = true;
            gunstockBaselineActive = true;
            ROCK_LOG_INFO(Weapon,
                "TwoHandedGrip: gunstock support baseline captured hand={} authored={} provider={} grip={} generation={:016X}; primary weapon transform retained at attach",
                supportHandIsLeft ? "left" : "right",
                supportGrip.authoredSupportGrip ? "yes" : "no",
                supportGrip.providerPartAuthority.active ? "yes" : "no",
                supportGrip.gripSequence,
        ...
        ROCK_LOG_INFO(Weapon,
            ...
            "gripSeparation={:.3f}, primaryGripSource={}, primaryGripConfidence={:.2f}, partKind={}, pose={}, authorityMode={}, gunstockBaseline={}, generation={:016X}",
            ...
            _primaryGripConfidence, static_cast<int>(supportGrip.partKind), static_cast<int>(supportGrip.gripPose), static_cast<int>(_authorityMode), gunstockBaselineActive ? "active" : "inactive", _activeWeaponGenerationKey);
    ...
    void TwoHandedGrip::updateFullWeaponAuthorityGrip(RE::NiNode* weaponNode, float dt)
    {
        ...
        const bool gunstockBaselineActive =
            supportGrip.gunstockBaseline.active;
        ...
        if (gunstockBaselineActive &&
            !weapon_support_acquisition_math::
                tryResolveGunstockSupportTarget(
                    supportTransform,
                    supportGrip.gunstockBaseline.
                        inputToGripTargetLocal,
                    calibratedSupportTransform)) {
            _hasSolvedWeaponTransform = false;
            ROCK_LOG_WARN(Weapon,
                "TwoHandedGrip: clearing gunstock support grip because its captured damped-hand baseline became invalid hand={} grip={} generation={:016X}",
                supportHandIsLeft ? "left" : "right",
                supportGrip.gripSequence,
                supportGrip.weaponGenerationKey);
            transitionToInactive(false);
        ...
        if (g_rockConfig.rockDebugDrawGunstockAlignment) {
            ...
            snapshot.attachPublication =
                gunstockBaselineActive &&
                supportGrip.gunstockBaseline.firstPublicationPending;
        ...
        if (!solved.solved) {
            if (dynamicAcquisition || gunstockBaselineActive) {
                clearDynamicSupportAcquisition(
        ...
        const bool gunstockAttachPublication =
            gunstockBaselineActive &&
            supportGrip.gunstockBaseline.firstPublicationPending;
        if (gunstockAttachPublication) {
            /*
             * This is the central gunstock invariant: acquisition may move the
             * visual support hand, but the first support publication cannot
             * alter the primary-owned weapon transform. Starting next frame,
             * the calibrated support input contributes its normal tandem delta.
             */
            appliedWeaponWorld =
                supportGrip.gunstockBaseline.weaponWorldAtCapture;
        }
        ...
        if (gunstockAttachPublication) {
            ...
            if (attachInvariantHeld) {
                ROCK_LOG_INFO(Weapon,
                    "TwoHandedGrip: gunstock support baseline published hand={} grip={} generation={:016X} attachWeaponDelta=({:.5f}gu,{:.5f}deg) tandemDeltaAuthority=armed",
                    supportHandIsLeft ? "left" : "right",
                    supportGrip.gripSequence,
                    supportGrip.weaponGenerationKey,
                    attachTranslationGameUnits,
                ...
                ROCK_LOG_WARN(Weapon,
                    "TwoHandedGrip: gunstock attach invariant exceeded hand={} grip={} generation={:016X} attachWeaponDelta=({:.5f}gu,{:.5f}deg)",
                    supportHandIsLeft ? "left" : "right",
                    supportGrip.gripSequence,
                    supportGrip.weaponGenerationKey,
                    attachTranslationGameUnits,
            ...
            supportGrip.gunstockBaseline.firstPublicationPending = false;
        }
            ...
            if (!_firingHandIsLeft) {
                const RE::NiTransform leftHandWorld =
                    frik_visual_authority::getHandWorldTransform(
                        handFromBool(true));
                if (isUsableHandAuthorityTransform(leftHandWorld)) {
        ...
        const bool authorityBlocked)
    {
        ...
        struct AppliedHandCorrection
        {
            ...
            RE::NiTransform appliedWorldDelta{};
            bool isLeft{ false };
        ...
        const auto applyHandCorrection = [this, &rolePublishedThisFrame](
            ...
            if (leftManualFiringRecoil) {
                ...
                if (out.reusedGripRole) {
                    ...
                    recoilRequestedWorld =
                        _lastPublishedHandWorld[index];
                    recoilAppliedWorld = originalWorld;
                } else {
            ...
            if (out.reusedGripRole) {
                recordPublishedHandWorld(isLeft, requestedWorld);
            } else {
                _gunstockDedicatedHandAuthorityActive[index] = true;
            }
        ...
        const auto restoreHandCorrection = [this](
            ...
            if (applied.reusedGripRole) {
                recordPublishedHandWorld(applied.isLeft, restoreTarget);
            } else {
                (void)frik_visual_authority::clearExternalHandWorldTransform(
                    GUNSTOCK_ALIGNMENT_TAG,
                    handFromBool(applied.isLeft));
        ...
        const RE::NiTransform firingInWeaponBefore =
            transform_math::composeTransforms(
                transform_math::invertTransform(
                    weaponWorldBeforeCorrection),
                firingGroupHandWorld);
        const RE::NiTransform firingInWeaponAfter =
            transform_math::composeTransforms(
                transform_math::invertTransform(correctedWeaponWorld),
                correctedFiringHandWorld);
        const float firingRelationPositionError = weaponSolverLength(
            weaponSolverSub(
                firingInWeaponAfter.translate,
                firingInWeaponBefore.translate));
        const float firingRelationRotationError =
        ...
        ROCK_LOG_SAMPLE_DEBUG(
            Weapon,
            2000,
            "TwoHandedGrip: gunstock group source={} pivot=damped-driver projectileWitness={} weaponReturn={} handReturn={} firingRelationError=({:.4f}gu,{:.3f}deg)",
            firingGroupSource,
            projectileWitnessUsable ? "live" : "latched",
            isWeaponVisualReturnActive() ? "active" : "inactive",
            isHandVisualReturnActive(_firingHandIsLeft) ?
                "active" : "inactive",
            firingRelationPositionError,
            firingRelationRotationError);
        _gunstockAlignmentAppliedThisFrame = true;
        return true;
    }
