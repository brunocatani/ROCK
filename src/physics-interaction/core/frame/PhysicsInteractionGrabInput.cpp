...
namespace rock
{
        ...
        const GrabInputFrame& input)
    {
        ...
        // A sheath or unsheath already consumed this hand's press this frame.
        // Let it fall through to grab input and the same press would grab the
        // thing the player just put away.
        ...
        // A provider force grab already owns this hand this frame. Player input
        // must not fight a command the consumer issued.
        ...
        // No usable hand frame. Fail closed and drop candidates rather than act
        // on a stale transform.
        ...
        const bool providerBlocksNewGrabPress =
            ...
            hand.getState() != HandState::SelectionLocked &&
            hand.getState() != HandState::Pulled;
        // The provider owns grab state for this hand, so ROCK reads input but
        // does not act on it. The two cases differ: one freezes the current
        // state, the other only blocks a NEW press so an existing hold survives.
        ...
        // A release that arrived while input was suppressed is replayed here,
        // not dropped. Dropping it would strand the hand in a hold the player
        // already let go of.
        ...
        // Touch grab is unavailable this frame, so the press falls through to
        // the normal grab path below instead of being swallowed.
        ...
        // The real grab press. Everything above decided whether this press is
        ...
        // A synthetic press is generated by intent resolution, not by the
        // player's finger. It runs the same commit path so a synthetic grab and
        // a real one cannot diverge.
        ...
        // Shared-object retry: the other hand still holds the target, so this
        // hand retries on a timer instead of failing the grab outright.
