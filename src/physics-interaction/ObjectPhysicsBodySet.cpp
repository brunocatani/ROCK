                ...
                if (fieldAt20 && reinterpret_cast<std::uintptr_t>(fieldAt20) > kMinValidPointer) {
                    ...
                    if (instance && reinterpret_cast<std::uintptr_t>(instance) > kMinValidPointer && instance->world == hknpWorld && instance->bodyIds) {
                        ...
                        for (std::int32_t i = 0; i < count; ++i) {
                            ...
                            auto* bodyArray = hknpWorld->GetBodyArray();
                            if (!bodyArray) {
                                continue;
                            }
                            ...
                            auto* motion = hknpWorld->GetBodyMotion(bodyId);
                            ...
                            const auto* bodyFloats = reinterpret_cast<const float*>(&bodyArray[rawBodyId]);
                            record.positionGame.x = bodyFloats[12] * kHavokToGameScale;
                            record.positionGame.y = bodyFloats[13] * kHavokToGameScale;
                            record.positionGame.z = bodyFloats[14] * kHavokToGameScale;
