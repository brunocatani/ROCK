...
namespace frik::rock::physics_recursive_wrappers
{
    ...
    inline constexpr SetMotionCommand makeSetMotionCommand(MotionPreset preset, bool activate, bool force, bool propagate)
    {
        return SetMotionCommand{ .presetValue = toNativeMotionPreset(preset), .activate = activate, .force = force, .propagate = propagate };
    }
