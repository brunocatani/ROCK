...
namespace rock
{
    namespace
    {
        ...
        constexpr const char* kGrabObjectRotationReferenceName = "rawHandAuthorityLocal";
            ...
            const grab_authority_frame_math::FrozenGrabAuthorityFrame<RE::NiTransform>& frozen)
        {
            ...
            frame.rawBodyHandSpace = frozen.rawBodyHandSpace;
    ...
    bool Hand::getGrabForceTorqueDebugSnapshot(RE::hknpWorld* world, const RE::NiTransform& rawHandWorld, GrabForceTorqueDebugSnapshot& out) const
    {
         ...
         * raw hand authority relation used by the active ragdoll-motor grab. Its
        ...
        const RE::NiTransform desiredBodyWorld = multiplyTransforms(rawHandWorld, _grabFrame.rawBodyHandSpace);
        if (!grab_authority_frame_math::isFiniteTransform(desiredBodyWorld)) {
            return false;
        }
        ...
        grab_transform_telemetry::RuntimeSample& out) const
    {
        ...
        if (_activeConstraint.constraintData) {
            ...
            // The ragdoll motor target is seeded from raw-hand BODY authority,
            // then converted through the generated proxy only at the constraint
            // atom boundary before comparing the atom bytes.
            ...
            const RE::NiTransform desiredBodyWorldForConstraintTelemetry =
                multiplyTransforms(rawHandWorld, _grabFrame.rawBodyHandSpace);
            if (!grab_authority_frame_math::isFiniteTransform(desiredBodyWorldForConstraintTelemetry)) {
                return false;
            }
        ...
        const char* reason)
    {
        ...
        const RE::NiTransform desiredBodyWorldAtCreation =
            _grabFrame.hasTelemetryCapture ?
                ...
                multiplyTransforms(rawHandWorldTransform, _grabFrame.rawBodyHandSpace);
        const RE::NiTransform desiredBodyTransformProxySpace =
...

        {
            ...
            _grabAuthorityPendingTarget = GrabAuthorityProxyPendingTarget{
                ...
                .desiredObjectWorld = _grabFrame.desiredObjectWorldAtGrab,
                .desiredBodyWorld = _grabFrame.desiredBodyWorldAtGrab,
    ...
    {
        outDesiredObjectWorld = desiredObjectWorldAuthority;
        outDesiredBodyWorld = desiredBodyWorldAuthority;
        if (!grab_authority_frame_math::isFiniteTransform(outDesiredObjectWorld) ||
            !grab_authority_frame_math::isFiniteTransform(outDesiredBodyWorld)) {
            return false;
        }
        const RE::NiTransform desiredBodyTransformProxySpace =
        ...
        const int ragdollDecompositionMode =
            grab_constraint_math::sanitizeGrabRagdollDecompositionMode(_activeConstraint.ragdollDecompositionMode);
        grab_constraint_math::writeGrabConstraintHeldTargetAtoms(
            transformBRotation,
        ...
        if (grabTimelineTraceEnabled() && shouldLogGrabTimelineSequence(targetWriteSequence)) {
            ...
            ROCK_LOG_INFO(Hand,
                "{} GRAB_TRACE stage=target_write trace={} writeSeq={} flushNext={} queued={} constraint={} proxyBody={} objBody={} decompConfig={}({}) decomp={}({}) col={:.2f}deg proxyPos=({:.2f},{:.2f},{:.2f}) desiredBodyPos=({:.2f},{:.2f},{:.2f}) targetPoint=({:.2f},{:.2f},{:.2f}) pivotAProxy=({:.2f},{:.2f},{:.2f}) pivotBSelected=({:.2f},{:.2f},{:.2f}) relationPivotB=({:.2f},{:.2f},{:.2f}) selectedPivotRelationDelta={:.3f}gu pivotBRelationDelta={:.3f}gu targetToHiggsRelation={:.2f}deg transformBFrozenDelta={:.2f}deg",
                handName(),
                _grabFrame.traceId,
                targetWriteSequence,
        ...
        bool heldBodyColliding)
    {
        ...
        if (!grab_authority_frame_math::isFiniteTransform(desiredObjectWorldAuthority) ||
            !grab_authority_frame_math::isFiniteTransform(desiredBodyWorldAuthority)) {
            return;
        }

        _grabAuthorityPendingTarget.proxyWorld = proxyWorldTransform;
        _grabAuthorityPendingTarget.rawHandWorld = rawHandWorldTransform;
        _grabAuthorityPendingTarget.desiredObjectWorld = desiredObjectWorldAuthority;
        _grabAuthorityPendingTarget.desiredBodyWorld = desiredBodyWorldAuthority;
    ...
    bool Hand::startDynamicPull(RE::hknpWorld* world, const RE::NiTransform& handWorldTransform)
    {
        ...
        if (_pullCatchIntent.active && _pullCatchIntent.primaryBodyId == _pulledPrimaryBodyId) {
            _pullCatchIntent.transitPointWorld = selectedPointWorld;
            _pullCatchIntent.transitPointSource = body_frame::bodyFrameSourceName(selectedPointAnchor.source);
            _pullCatchIntent.hasTransitPointWorld = true;
            if (hasPrimaryBodyWorld) {
                _pullCatchIntent.transitPointBodyLocal =
                    transform_math::worldPointToLocal(primaryBodyWorld, selectedPointWorld);
                _pullCatchIntent.hasTransitPointBodyLocal = true;
            }
        }
    ...
    bool Hand::updateDynamicPull(RE::hknpWorld* world, const RE::NiTransform& handWorldTransform, float deltaTime)
    {
        ...
        if (distanceGameUnits <= arrivalDistance) {
            const RE::NiPoint3 arrivalTransitPointWorld{
            ...
            const bool hasArrivalBodyWorld =
                tryGetBodyWorldTransform(world, RE::hknpBodyId{ _pulledPrimaryBodyId }, arrivalBodyWorld) ||
                tryResolveLiveBodyWorldTransform(world, RE::hknpBodyId{ _pulledPrimaryBodyId }, arrivalBodyWorld);
            if (_pullCatchIntent.active && _pullCatchIntent.primaryBodyId == _pulledPrimaryBodyId) {
                _pullCatchIntent.arrivalTransitPointWorld = arrivalTransitPointWorld;
                _pullCatchIntent.hasArrivalTransitPointWorld = true;
                if (hasArrivalBodyWorld) {
                    _pullCatchIntent.arrivalBodyWorld = arrivalBodyWorld;
                    _pullCatchIntent.hasArrivalBodyWorld = true;
                    _pullCatchIntent.transitPointBodyLocal =
                        transform_math::worldPointToLocal(arrivalBodyWorld, arrivalTransitPointWorld);
                    _pullCatchIntent.hasTransitPointBodyLocal = true;
                }
            }
            _currentSelection.isFarSelection = false;
            _currentSelection.hitPointWorld = arrivalTransitPointWorld;
        ...
        const GrabSharedObjectContext& sharedContext)
    {
        ...
        const RE::NiTransform grabAuthorityFrameWorldAtGrab = makeRawHandGrabAuthorityFrame(handWorldTransform, _isLeft);
        RE::NiTransform proxyFrameWorldAtGrab = grabAuthorityFrameWorldAtGrab;
        ...
        if (!hasPalmProxyFrameAtGrab) {
            ROCK_LOG_SAMPLE_DEBUG(Hand,
                g_rockConfig.rockLogSampleMilliseconds,
                "{} hand grab authority: live palm anchor proxy unavailable before capture; using raw hand body-A fallback bodyId={} handBody={} source={} formID={:08X}",
                handName(),
                objectBodyId.value,
                _handBody.isValid() ? _handBody.getBodyId().value : INVALID_BODY_ID,
                proxyFrameSourceAtGrab,
                sel.refr ? sel.refr->GetFormID() : 0);
            proxyFrameWorldAtGrab = grabAuthorityFrameWorldAtGrab;
            proxyFrameSourceAtGrab = "rawHandAuthorityFallback";
        }
        /*
         * Dynamic acquisition uses raw hand authority for the palm pocket and
         * hand/object relation. The hidden proxy frame is retained only as body
         * A transport for the constraint.
         */
        const RE::NiPoint3 grabAuthorityPivotAWorld = grabAuthorityFrameWorldAtGrab.translate;
        const RE::NiPoint3 palmPocketPivotAWorld = grabAuthorityFrameWorldAtGrab.translate;
        const float palmPocketToProxyDeltaGameUnits = pointDistanceGameUnits(proxyFrameWorldAtGrab.translate, palmPocketPivotAWorld);
        ...
        const RE::NiTransform proxyAuthorityFrameWorldAtGrab = grabAuthorityFrameWorldAtGrab;

        if (grabPalmBasisDelta.rotationDegrees > kGrabFrameMismatchRawProxyRotationWarnDegrees) {
            ROCK_LOG_SAMPLE_INFO(Hand,
                g_rockConfig.rockLogSampleMilliseconds,
                "{} PROXY GRAB PALM BASIS BYPASSED: ref='{}' formID={:08X} source={} rawToProxy={:.1f}deg axisDeg=({:.1f},{:.1f},{:.1f}) determinant=({:.3f},{:.3f}) proxyPivot=({:.1f},{:.1f},{:.1f}) rawPalmPivot=({:.1f},{:.1f},{:.1f}) pocketProxyDelta={:.2f}gu phase={}",
                handName(),
                objName,
                sel.refr ? sel.refr->GetFormID() : 0,
                proxyFrameSourceAtGrab,
        ...
        const bool currentSeatTrustedSurface =
            meshGrabFound &&
            grabSurfaceHit.valid &&
            grabSurfaceHit.sourceKind != GrabSurfaceSourceKind::CollisionQuery;
        const float currentSeatPocketDistance =
            acquisitionPocket.valid ? pointDistanceGameUnits(grabGripPoint, acquisitionPocket.palmCenterWorld) : std::numeric_limits<float>::max();
        const float palmPocketSurfaceDistance =
            (acquisitionPocket.valid && palmPocketMeshAvailable) ?
                pointDistanceGameUnits(palmPocketSurfaceHit.position, acquisitionPocket.palmCenterWorld) :
                std::numeric_limits<float>::max();
        const auto pullCatchSeatSourceDecision =
            grab_three_phase::choosePullCatchSeatSource(grab_three_phase::PullCatchSeatSourceInput{
                .grabbedFromPullCatch = grabbedFromPullCatch,
                .usingPinchPocket = pinchPocketCandidate.valid,
                .hasPalmPocketSurface = palmPocketMeshAvailable,
                .palmPocketNormalTrusted = palmPocketMeshAvailable && lengthSquared(palmPocketSurfaceHit.normal) > 0.000001f,
                .hasCurrentSeat = meshGrabFound || sel.hasHitPoint,
                .currentSeatTrustedSurface = currentSeatTrustedSurface,
                .currentSeatPositionOnly = pivotAuthorityPositionOnly,
                .hasTransitPoint =
                    _pullCatchIntent.hasTransitPointWorld ||
                    _pullCatchIntent.hasTransitPointBodyLocal ||
                    _pullCatchIntent.hasArrivalTransitPointWorld,
                .palmPocketDistanceGameUnits = palmPocketSurfaceDistance,
                .currentSeatDistanceGameUnits = currentSeatPocketDistance,
        ...
        const char* previousGrabPointMode = grabPointMode;
        if (pullCatchSeatSourceDecision.usePalmPocketSurface && !pinchPocketCandidate.valid) {
            grabGripPoint = palmPocketSurfaceHit.position;
            grabSurfaceHit = palmPocketSurfaceHit;
            surfaceOwnerNode = grabSurfaceHit.sourceNode;
            selectionToMeshDistanceGameUnits = grabSurfaceHit.selectionToMeshDistanceGameUnits;
            ...
            meshGrabFound = true;
            grabPointMode = "palmPocketMeshSurface";
            grabFallbackReason = pullCatchSeatSourceDecision.reason;
            canonicalPivotPointWorld = grabGripPoint;
            canonicalPivotNormalWorld = grabSurfaceHit.normal;
            ...
            pivotAuthorityPositionOnly = !pullCatchSeatSourceDecision.normalTrusted;
            pivotAuthorityNormalTrusted = pullCatchSeatSourceDecision.normalTrusted;
            pivotAuthorityPositionConfidence = pullCatchSeatSourceDecision.normalTrusted ? 0.9f : 0.55f;
            pivotAuthorityPocketDistanceGameUnits = palmPocketSurfaceDistance;
            pivotAuthoritySelectionDeltaGameUnits =
                sel.hasHitPoint ? pointDistanceGameUnits(sel.hitPointWorld, grabGripPoint) : std::numeric_limits<float>::max();
            ROCK_LOG_INFO(Hand,
                "{} hand PULL CATCH SEAT SOURCE: source={} reason={} transitPreserved={} seat=({:.1f},{:.1f},{:.1f}) pocket={:.2f}gu normalTrusted={} currentSeat={} currentPocket={:.2f}gu",
                handName(),
                grab_three_phase::pullCatchSeatSourceName(pullCatchSeatSourceDecision.source),
                pullCatchSeatSourceDecision.reason,
                pullCatchSeatSourceDecision.preserveTransitEvidence ? "yes" : "no",
                grabGripPoint.x,
                grabGripPoint.y,
                grabGripPoint.z,
                palmPocketSurfaceDistance,
                pullCatchSeatSourceDecision.normalTrusted ? "yes" : "no",
                previousGrabPointMode,
                currentSeatPocketDistance);
        } else if (grabbedFromPullCatch && pullCatchSeatSourceDecision.preserveTransitEvidence && g_rockConfig.rockDebugGrabFrameLogging) {
            ROCK_LOG_DEBUG(Hand,
                "{} hand PULL CATCH SEAT SOURCE: source={} reason={} currentSeat={} currentPocket={:.2f}gu palmPocketAvailable={} transitWorld={} transitLocal={} arrivalTransit={}",
                handName(),
                grab_three_phase::pullCatchSeatSourceName(pullCatchSeatSourceDecision.source),
                pullCatchSeatSourceDecision.reason,
                grabPointMode,
                ...
                _pullCatchIntent.hasTransitPointWorld ? "yes" : "no",
                _pullCatchIntent.hasTransitPointBodyLocal ? "yes" : "no",
                _pullCatchIntent.hasArrivalTransitPointWorld ? "yes" : "no");
        }
...

        {
            ...
            if (grabTimelineTraceEnabled()) {
                ...
                std::array<float, 4> traceTransformBTranslation{};
                std::array<float, 12> traceTargetBRca{};
                float traceRagdollDecompositionColumnDeltaDegrees = -1.0f;
                const int traceRagdollDecompositionConfigMode =
                    grab_constraint_math::sanitizeGrabRagdollDecompositionConfigMode(g_rockConfig.rockGrabRagdollDecompositionMode);
                const int traceRagdollDecompositionMode = grab_constraint_math::resolveGrabRagdollDecompositionMode(
                    traceRagdollDecompositionConfigMode,
                    frozenAuthorityFrame.proxyAuthorityBodyHandSpace,
                    &traceRagdollDecompositionColumnDeltaDegrees);
                grab_constraint_math::writeGrabConstraintCreationAtoms(
                    traceTransformBRotation.data(),
                    traceTransformBTranslation.data(),
                    traceTargetBRca.data(),
                ...
                ROCK_LOG_INFO(Hand,
                    "{} GRAB_TRACE stage=capture_frames trace={} rawPos=({:.2f},{:.2f},{:.2f}) proxyPos=({:.2f},{:.2f},{:.2f}) objectPos=({:.2f},{:.2f},{:.2f}) bodyPos=({:.2f},{:.2f},{:.2f}) desiredBodyPos=({:.2f},{:.2f},{:.2f}) rawProxyRot={:.2f}deg rawProxyAxisMax={:.2f}deg objectDesiredAxisMax={:.2f}deg desiredBodyGrabBodyAxisMax={:.2f}deg decompConfig={}({}) decomp={}({}) col={:.2f}deg targetToHiggsRelation={:.2f}deg transformBFrozenDelta={:.2f}deg",
                    handName(),
                    _grabFrame.traceId,
                    ...
                    traceRagdollDecompositionConfigMode,
                    grab_constraint_math::grabRagdollDecompositionModeName(traceRagdollDecompositionConfigMode),
                    traceRagdollDecompositionMode,
                    grab_constraint_math::grabRagdollDecompositionModeName(traceRagdollDecompositionMode),
                    traceRagdollDecompositionColumnDeltaDegrees,
            ...
            if (g_rockConfig.rockDebugGrabFrameLogging) {
                ...
                float freezeRagdollDecompositionColumnDeltaDegrees = -1.0f;
                const int freezeRagdollDecompositionConfigMode =
                    grab_constraint_math::sanitizeGrabRagdollDecompositionConfigMode(g_rockConfig.rockGrabRagdollDecompositionMode);
                const int freezeRagdollDecompositionMode = grab_constraint_math::resolveGrabRagdollDecompositionMode(
                    freezeRagdollDecompositionConfigMode,
                    frozenAuthorityFrame.proxyAuthorityBodyHandSpace,
                    &freezeRagdollDecompositionColumnDeltaDegrees);
                grab_constraint_math::writeGrabConstraintCreationAtoms(
                    ...
                    frozenAuthorityFrame.pivotAHandBodyLocalGame,
                    gameToHavokScale(),
                    freezeRagdollDecompositionMode);
                ...
                ROCK_LOG_INFO(Hand,
                    "{} GRAB FREEZE AUTHORITY: formID={:08X} name='{}' body={} mode={} decompConfig={}({}) decomp={}({}) col={:.2f}deg pivotAuthority={} frozenSource={} resolverSource={} "
                    ...
                    _grabFrame.activeGrabPointMode ? _grabFrame.activeGrabPointMode : "none",
                    freezeRagdollDecompositionConfigMode,
                    grab_constraint_math::grabRagdollDecompositionModeName(freezeRagdollDecompositionConfigMode),
                    freezeRagdollDecompositionMode,
                    grab_constraint_math::grabRagdollDecompositionModeName(freezeRagdollDecompositionMode),
                    freezeRagdollDecompositionColumnDeltaDegrees,
                    _grabFrame.pivotAuthoritySource ? _grabFrame.pivotAuthoritySource : "none",
                    grab_authority_frame_math::grabAuthorityPivotSourceName(frozenAuthorityFrame.source),
                    grab_authority_frame_math::grabAuthorityPivotSourceName(resolvedAuthorityPivotSourceForFreeze),
        ...
        if (_grabFingerPosePublished && _hasGrabFingerPose) {
            ...
            const bool publishPullCatchAcquisitionPose =
                ...
                _grabObjectGripAtGrab.valid &&
                _grabAcquisitionPhase != grab_three_phase::AcquisitionPhase::TouchHeld;
            RE::NiTransform fingerPoseObjectWorld = objectWorldTransform;
            if (publishPullCatchAcquisitionPose && grab_three_phase::isFinite(_grabFrame.desiredObjectWorldAtGrab)) {
                fingerPoseObjectWorld = _grabFrame.desiredObjectWorldAtGrab;
            }
            const auto fingerPoseWorldTriangles =
                publishPullCatchAcquisitionPose ?
                    rebuildFingerPoseWorldTrianglesFromGrabFrame(_grabFrame, fingerPoseObjectWorld) :
            ...
            const auto initialFingerPoseTargets = rebuildFingerPoseTargetsFromGrabFrame(_grabFrame, fingerPoseObjectWorld);
            ...
            auto fingerPose = grab_finger_pose_runtime::solveGrabFingerPoseFromTriangles(
                fingerPoseWorldTriangles, initialFingerHandTransform, _isLeft,
            ...
            (void)grab_finger_pose_runtime::refineGrabFingerPoseWithPadProbes(
                fingerPose,
                fingerPoseWorldTriangles,
                initialFingerPoseTargets,
                liveFingerSnapshotAtGrab,
                fingerPoseObjectWorld,
                g_rockConfig.rockGrabMeshFingerPoseEnabled,
                _grabFingerPosePublished,
                padCaptureEvidence,
                true,
                false);
            grab_finger_pose_runtime::captureSurfaceAimObjectLocal(fingerPose, fingerPoseObjectWorld);
            ...
            auto publishFingerPose =
                grab_finger_pose_runtime::resolveSurfaceAimObjectLocal(_grabFingerPose, fingerPoseObjectWorld);
            ...
            (void)grab_finger_pose_runtime::refineGrabFingerPoseWithPadProbes(
                ...
                false,
                true);
            if (publishPullCatchAcquisitionPose) {
                publishFingerPose = buildAcquisitionFingerPose(publishFingerPose, 0.35f);
            }
            ...
            if (publishPullCatchAcquisitionPose) {
                ROCK_LOG_DEBUG(Hand,
                    "{} THREE-PHASE GRAB POSE: pull-catch acquisition pose published from desired relation phase={} candidateTris={} poseTargets={}",
                    handName(),
                    grab_three_phase::phaseName(_grabAcquisitionPhase),
                    publishFingerPose.candidateTriangleCount,
                    publishFingerPose.poseTargetCount);
            }
        ...
        const GrabReleaseContext& releaseContext)
    {
        ...
        /*
         * ROCK freezes the dynamic grab relation in raw hand space. The hidden
         * proxy remains body A for the finite-force constraint, but it is not a
         * desired-pose authority; the physics flush converts this raw desired
         * BODY world target into proxy-local atoms at the boundary.
         */
        RE::NiTransform proxyAuthorityWorld = makeRawHandGrabAuthorityFrame(handWorldTransform, _isLeft);
        const char* proxyAuthoritySource = "rawHandAuthorityFallback";
        ...
        if (!hasProxyAuthorityFrame) {
            ROCK_LOG_SAMPLE_DEBUG(Hand,
                g_rockConfig.rockLogSampleMilliseconds,
                "{} hand grab authority: palm anchor proxy frame unavailable while held; using raw hand body-A fallback source={}",
                handName(),
                proxyAuthoritySource);
            proxyAuthorityWorld = makeRawHandGrabAuthorityFrame(handWorldTransform, _isLeft);
            proxyAuthoritySource = "rawHandAuthorityFallback";
        }
        RE::NiTransform desiredBodyWorld = multiplyTransforms(handWorldTransform, _grabFrame.rawBodyHandSpace);
        RE::NiTransform desiredObjectWorld = deriveNodeWorldFromBodyWorld(desiredBodyWorld, _grabFrame.bodyLocal);
        if (!grab_authority_frame_math::isFiniteTransform(desiredBodyWorld) ||
            !grab_authority_frame_math::isFiniteTransform(desiredObjectWorld)) {
            ROCK_LOG_WARN(Hand,
                "{} hand release: raw hand grab authority target became non-finite while held phase={}",
                handName(),
                grab_three_phase::phaseName(_grabAcquisitionPhase));
            releaseGrabbedObject(world, GrabReleaseCollisionRestoreMode::Immediate, releaseContext);
            return;
        ...
        if (convergingAcquisitionPhase && _grabObjectGripAtGrab.valid) {
            ...
            if (pivotNeedsSeatedReacquire && hasGrabBody) {
                const RE::NiTransform authorityFrame = makeRawHandGrabAuthorityFrame(handWorldTransform, _isLeft);
                const RE::NiPoint3 livePivotAWorld = authorityFrame.translate;
                if (!grab_three_phase::isFinite(livePivotAWorld)) {
                    timeoutReacquireReason = "invalidRawHandPalmPocketPivot";
                        ...
                        if (promotionDecision.promotePivot) {
                            ...
                            const auto frozenSeatAuthorityFrame = grab_authority_frame_math::freezeGrabAuthorityFrame<RE::NiTransform>(
                                grab_authority_frame_math::GrabAuthorityFrameFreezeInput<RE::NiTransform>{
                                    ...
                                    .proxyAuthorityFrameWorld = authorityFrame,
            ...
            if (reachedTouchMayPromote || timeoutMayPromote) {
                ...
                if (!_grabFrame.syntheticLooseWeaponPrimaryAttach && g_rockConfig.rockGrabMeshFingerPoseEnabled && _hasGrabFingerPose && !_grabFingerPosePublished) {
                    ...
                    RE::NiPoint3 fingerPosePivotWorld =
                        _grabFrame.hasTelemetryCapture ? _grabFrame.grabPivotWorldAtGrab : computeGrabPivotAWorld(world, handWorldTransform);
                    const RE::NiTransform rawAuthorityFrame = makeRawHandGrabAuthorityFrame(handWorldTransform, _isLeft);
                    if (grab_three_phase::isFinite(rawAuthorityFrame.translate)) {
                        fingerPosePivotWorld = rawAuthorityFrame.translate;
                    }
        ...
        if (!_grabFrame.syntheticLooseWeaponPrimaryAttach && g_rockConfig.rockGrabMeshFingerPoseEnabled && _hasGrabFingerPose && _grabFingerPosePublished) {
            ...
            if (_grabFingerPoseFrameCounter >= updateInterval) {
                ...
                if (convergingAcquisitionPhase && _grabObjectGripAtGrab.valid) {
                    ...
                    const float nearDistance = (std::max)(touchDistance, g_rockConfig.rockGrabNearConvergeDistanceGameUnits);
                    const float progressDenominator = (std::max)(0.001f, nearDistance - touchDistance);
                    const float acquisitionProgress =
                        hasPivotTrackingError && std::isfinite(pivotTrackingErrorGameUnits) ?
                            1.0f - std::clamp((pivotTrackingErrorGameUnits - touchDistance) / progressDenominator, 0.0f, 1.0f) :
                            0.35f;
                    publishFingerPose = buildAcquisitionFingerPose(publishFingerPose, acquisitionProgress);
                }
                applyRockGrabHandPose(_isLeft,
                    publishFingerPose,
                    _grabFingerJointPose,
        ...
        if (_heldLogCounter >= 45) {
            ...
            if (g_rockConfig.rockDebugGrabFrameLogging) {
                const RE::NiTransform desiredBodyWorldRaw = multiplyTransforms(handWorldTransform, _grabFrame.rawBodyHandSpace);
                const RE::NiTransform desiredNodeWorldRaw = deriveNodeWorldFromBodyWorld(desiredBodyWorldRaw, _grabFrame.bodyLocal);
    ...
    void Hand::flushPendingCustomGrabAuthority(RE::hknpWorld* world, const havok_physics_timing::PhysicsTimingSample& timing)
    {
        ...
        GrabAngularAuthority angularAuthority = GrabAngularAuthority::HknpRagdollMotorAtom;
        {
             ...
             * Keep live palm as health/telemetry only; raw hand authority owns
             * the queued proxy target when palm readback is missing or suspect.
             */
            ...
            if (!proxyDriveOk) {
                ...
                targetUpdateOk = updateProxyConstraintGrabDriveTarget(
                    world,
                    pending.proxyWorld,
                    pending.desiredObjectWorld,
                    pending.desiredBodyWorld,
                    desiredObjectWorld,
                    desiredBodyWorld,
    ...
    void Hand::observeCustomGrabAuthorityAfterSolve(RE::hknpWorld* world, const havok_physics_timing::PhysicsTimingSample& timing)
    {
        ...
        float transformBFrozenDeltaDegrees = -1.0f;
        {
            ...
            if (_activeConstraint.constraintData) {
                ...
                const RE::NiTransform desiredBodyWorldForConstraintMetrics =
                    multiplyTransforms(targetRawHandWorld, _grabFrame.rawBodyHandSpace);
                const RE::NiTransform desiredBodyTransformHandSpace =
        ...
        const RE::NiTransform desiredBodyFromTarget = multiplyTransforms(targetRawHandWorld, _grabFrame.rawBodyHandSpace);
        const RE::NiTransform desiredObjectFromTarget = deriveNodeWorldFromBodyWorld(desiredBodyFromTarget, _grabFrame.bodyLocal);
        const RE::NiTransform desiredObjectFromLiveProxy = desiredObjectFromTarget;
        const RE::NiTransform desiredBodyFromLiveProxy = desiredBodyFromTarget;
