...
{
    /*
     * Hand state transitions are kept as a pure policy layer so the current
     * grab, pull, and selection behavior can be preserved while removing
     * scattered direct state writes from runtime code. ROCK keeps side effects
     * in Hand and makes transition legality independently testable.
     */
    ...
    {
        return state == HandState::HeldInit || state == HandState::HeldBody || state == HandState::StashCandidate ||
