...
namespace frik::rock
{
	...
	void PhysicsInteraction::update()
	{
		...
		_deltaTime = std::chrono::duration<float>(now - _prevFrameTime).count();
		_prevFrameTime = now;

		// Clamp delta time to avoid crazy velocities on hitches
		...
		if (_collisionLayerRegistered && _expectedHandLayerMask != 0) {
			auto dispatcherData = *reinterpret_cast<std::uintptr_t*>(
				reinterpret_cast<std::uintptr_t>(hknp) + 0x150);
			if (dispatcherData) {
				auto* filterPtr = *reinterpret_cast<void**>(dispatcherData + 0x5E8);
				if (filterPtr) {
					auto* matrix = reinterpret_cast<std::uint64_t*>(
						reinterpret_cast<std::uintptr_t>(filterPtr) + 0x1A0);
		...
		// objects (P-Receiver, WeaponBolt, WeaponMagazine, etc.).
		{
			...
			if (g_rockConfig.rockDebugVerboseLogging) {
				static int wpnNodeLogCounter = 0;
				if (++wpnNodeLogCounter >= 90) {
					wpnNodeLogCounter = 0;
					if (weaponNode) {
	...
	void PhysicsInteraction::shutdown()
	{
		...
		// Release any held objects before destroying bodies
		auto* currentBhk = getPlayerBhkWorld();
		if (_cachedBhkWorld && currentBhk == _cachedBhkWorld) {
			auto* hknp = getHknpWorld(_cachedBhkWorld);
			// Release grabbed objects (destroys constraints, restores state)
			if (_rightHand.isHolding()) _rightHand.releaseGrabbedObject(hknp);
			if (_leftHand.isHolding()) _leftHand.releaseGrabbedObject(hknp);
			// Reset two-handed grip (restores FRIK offhand grip)
			_twoHandedGrip.reset();
			// Destroy weapon collision body + clear cached state
			_weaponCollision.destroyWeaponBody(hknp);
			_weaponCollision.shutdown();
			// Destroy hand collision bodies
			destroyHandCollisions(hknp);
		} else {
			// World changed or is null — don't touch old world, just clear our state
			ROCK_LOG_WARN(Init, "Shutdown: bhkWorld stale or null — skipping body destruction");
			_rightHand.destroyDebugSphere();
			_leftHand.destroyDebugSphere();
		}

		// Note: contact event subscriptions are auto-cleaned when hknpWorld is destroyed.
		...
		// Free vtable shellcode (prevents executable memory leak on DLL hot-reload)
		cleanupGrabConstraintVtable();

		ROCK_LOG_INFO(Init, "ROCK physics module shut down");
	}

	void PhysicsInteraction::shutdownWithoutDestroy()
	{
		if (!_initialized) {
			return;
		}

		ROCK_LOG_INFO(Init, "Shutting down ROCK physics module (cell change — skipping body destroy)...");

		// Notify listeners before clearing state
		dispatchPhysicsMessage(kPhysMsg_OnPhysicsShutdown, false);

		// Old world is gone — just clean up our side without touching Havok
		_rightHand.destroyDebugSphere();
		_leftHand.destroyDebugSphere();

		// Two-handed grip — reset state
		_twoHandedGrip.reset();

		// Weapon collision — shutdown without destroy (world is gone)
		_weaponCollision.shutdown();
		...
		ROCK_LOG_INFO(Init, "ROCK physics module shut down (cell change path)");
	}
	...
	void PhysicsInteraction::updateHandCollisions(RE::hknpWorld* world)
	{
		...
		if (g_rockConfig.rockDebugVerboseLogging) {
			static int wandDiagFrame = 0;
			wandDiagFrame++;
			if (leftWand && (wandDiagFrame % 180 == 0) && wandDiagFrame < 5400) {
									...
									if (sel.bodyId.value != 0x7FFF'FFFF) {
										...
										if (motion) {
											auto motionProps = *reinterpret_cast<std::uint16_t*>(
												reinterpret_cast<char*>(motion) + 0x38);
	...
	RE::hknpWorld* PhysicsInteraction::getHknpWorld(RE::bhkWorld* bhk)
	{
		...
		return *reinterpret_cast<RE::hknpWorld**>(
			reinterpret_cast<uintptr_t>(bhk) + 0x60);
	}
	...
	void PhysicsInteraction::installCCRadiusHook()
	{
	...
	void PhysicsInteraction::installBumpHook()
	{
	...
	void PhysicsInteraction::installNativeGrabHook()
	{
		static bool installed = false;
		if (installed) return;
		installed = true;
	...
	void PhysicsInteraction::installRefreshManifoldHook()
	{
		static bool installed = false;
		if (installed) return;
		installed = true;
