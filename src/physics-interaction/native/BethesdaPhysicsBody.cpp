...
namespace rock
{
    ...
    namespace
    {
        struct RetiredBethesdaPhysicsBodyGroup
        {
            std::array<void*, kMaxBethesdaPhysicsBodyGroupMembers> collisionObjects{};
            std::array<void*, kMaxBethesdaPhysicsBodyGroupMembers> niNodes{};
            std::size_t memberCount = 0;
            std::uint32_t remainingPhysicsSteps = 0;

            [[nodiscard]] bool occupied() const { return memberCount != 0; }
        };

        inline constexpr std::size_t kMaxRetiredBethesdaPhysicsBodyGroups = 16;
        std::mutex s_retiredGroupMutex;
        std::array<RetiredBethesdaPhysicsBodyGroup, kMaxRetiredBethesdaPhysicsBodyGroups> s_retiredGroups{};

        void* createAndLinkGeneratedNiNode(void* collisionObject, const char* name)
        {
            if (!collisionObject) {
                return nullptr;
            }

            void* node = bethesdaAlloc(offsets::kNiNodeSize);
            if (!node) {
                return nullptr;
            }
            std::memset(node, 0, offsets::kNiNodeSize);
            ...
            if (name && name[0]) {
                alignas(8) char bsStr[8]{};
                using BSFixedStringCreate_t = void (*)(void*, const char*);
                static REL::Relocation<BSFixedStringCreate_t> createString{ REL::Offset(offsets::kFunc_BSFixedString_Create) };
                createString(bsStr, name);
                using NiNodeSetName_t = void (*)(void*, const void*);
                static REL::Relocation<NiNodeSetName_t> setName{ REL::Offset(offsets::kFunc_NiNode_SetName) };
                setName(node, bsStr);
                std::memset(bsStr, 0, sizeof(bsStr));
            ...
            static REL::Relocation<LinkObject_t> linkObject{ REL::Offset(offsets::kFunc_CollisionObject_LinkObject) };
            linkObject(collisionObject, node);
            return node;
        }

        void releaseRetiredGroup(RetiredBethesdaPhysicsBodyGroup& retired)
        {
            for (std::size_t index = 0; index < retired.memberCount; ++index) {
                detachAndReleaseNiNode(retired.collisionObjects[index], retired.niNodes[index]);
                if (retired.collisionObjects[index]) {
                    releaseRefCounted(retired.collisionObjects[index]);
                }
            }
            retired = {};
        }
    ...
    bool BethesdaPhysicsBodyGroup::create(
        RE::hknpWorld* world,
        void* bhkWorld,
    ...
    {
        if (isValid() || _physicsSystem || _memberCount != 0) {
            ROCK_LOG_ERROR(BethesdaBody, "Body group create called on a non-empty owner");
            return false;
        }
        if (!world || !bhkWorld || !members || memberCount == 0 || memberCount > kMaxBethesdaPhysicsBodyGroupMembers) {
            ROCK_LOG_ERROR(BethesdaBody, "Body group create invalid inputs world={:p} bhkWorld={:p} members={:p} count={}",
                static_cast<void*>(world), bhkWorld, static_cast<const void*>(members), memberCount);
            return false;
        }
        for (std::size_t index = 0; index < memberCount; ++index) {
            if (!members[index].shape) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group member {} has no shape", index);
                return false;
            }
        }
        ...
        for (auto& bodyId : _bodyIds) {
            bodyId.value = kInvalidGeneratedId;
        }
        ...
        static REL::Relocation<PhysicsSystemDataCtor_t> systemDataCtor{ REL::Offset(offsets::kFunc_PhysicsSystemData_Ctor) };
        systemDataCtor(_systemData);
        bool ownsSystemDataLocalRef = true;
        auto releaseSystemData = [&]() {
            if (ownsSystemDataLocalRef && _systemData) {
                havok_ref_count::release(_systemData);
                ownsSystemDataLocalRef = false;
            }
        };
        auto append = [&](char* arrayBase, int stride) -> char* {
            auto*& data = *reinterpret_cast<char**>(arrayBase);
            auto& size = *reinterpret_cast<std::int32_t*>(arrayBase + 0x08);
            auto& capacityFlags = *reinterpret_cast<std::int32_t*>(arrayBase + 0x0C);
            if (size >= (capacityFlags & 0x3FFF'FFFF) && !havok_runtime::hkArrayReserveMore(arrayBase, stride)) {
                return nullptr;
            }
            if (!data) {
                return nullptr;
            }
            char* entry = data + static_cast<std::ptrdiff_t>(size) * stride;
            ++size;
            return entry;
        };
        auto failBeforeSystem = [&]() {
            releaseSystemData();
            reset();
            return false;
        };

        auto* motionArray = reinterpret_cast<char*>(_systemData) + offsets::kSysData_MotionCinfos;
        auto* motionCinfo = append(motionArray, 0x70);
        if (!motionCinfo) {
            ROCK_LOG_ERROR(BethesdaBody, "Body group failed to append its shared motion cinfo");
            return failBeforeSystem();
        }
        static REL::Relocation<MotionCinfoCtor_t> motionCinfoCtor{ REL::Offset(offsets::kFunc_MotionCinfo_Ctor) };
        motionCinfoCtor(motionCinfo);

        auto* materialsArray = reinterpret_cast<char*>(_systemData) + offsets::kSysData_Materials;
        auto* material = append(materialsArray, 0x50);
        if (!material) {
            return failBeforeSystem();
        }
        static REL::Relocation<MaterialCtor_t> materialCtor{ REL::Offset(offsets::kFunc_MaterialCtor) };
        ...
        auto* bodyArray = reinterpret_cast<char*>(_systemData) + offsets::kSysData_BodyCinfos;
        auto* shapeArray = reinterpret_cast<char*>(_systemData) + offsets::kSysData_Shapes;
        static REL::Relocation<BodyCinfoCtor_t> bodyCinfoCtor{ REL::Offset(offsets::kFunc_BodyCinfo_Ctor) };
        for (std::size_t index = 0; index < memberCount; ++index) {
            auto* bodyCinfo = append(bodyArray, 0x60);
            auto* shapeSlot = append(shapeArray, 8);
            if (!bodyCinfo || !shapeSlot) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group array growth failed at member {}", index);
                return failBeforeSystem();
            }
            bodyCinfoCtor(bodyCinfo);
            auto* bytes = reinterpret_cast<std::uint8_t*>(bodyCinfo);
            *reinterpret_cast<RE::hknpShape**>(bytes + 0x00) = members[index].shape;
            *reinterpret_cast<std::uint32_t*>(bytes + 0x08) = kInvalidGeneratedId;
            *reinterpret_cast<std::uint32_t*>(bytes + 0x0C) = 0;
            *reinterpret_cast<std::uint16_t*>(bytes + 0x10) = generatedInitialQualityId(BethesdaMotionType::Dynamic);
            *reinterpret_cast<std::uint16_t*>(bytes + 0x12) = kGeneratedSystemLocalMaterialIndex;
            *reinterpret_cast<std::uint32_t*>(bytes + 0x14) = members[index].filterInfo;
            *reinterpret_cast<const char**>(bytes + 0x20) = members[index].name ? members[index].name : systemName;
            *reinterpret_cast<std::uintptr_t*>(bytes + 0x28) = 0;
            auto* position = reinterpret_cast<float*>(bytes + 0x30);
            position[0] = members[index].initialWorld.translate.x * gameToHavok;
            position[1] = members[index].initialWorld.translate.y * gameToHavok;
            position[2] = members[index].initialWorld.translate.z * gameToHavok;
            position[3] = 0.0f;
            const auto havokColumns = transform_math::niRowsToHavokColumns(members[index].initialWorld.rotate);
            transform_math::niRowsToHavokQuaternion(havokColumns, reinterpret_cast<float*>(bytes + 0x40));

            *reinterpret_cast<RE::hknpShape**>(shapeSlot) = members[index].shape;
            havok_ref_count::addRef(members[index].shape);
        }
        ...
        static REL::Relocation<PhysicsSystemCtor_t> physicsSystemCtor{ REL::Offset(offsets::kFunc_PhysicsSystem_Ctor) };
        physicsSystemCtor(_physicsSystem, _systemData);
        releaseSystemData();

        BethesdaAllocatorContextGuard allocatorContext{ 0x41 };
        if (!allocatorContext.active()) {
            releaseRefCounted(_physicsSystem);
            reset();
            return false;
        }
        static REL::Relocation<CollisionObjectCtor_t> collisionObjectCtor{ REL::Offset(offsets::kFunc_CollisionObject_Ctor) };
        for (std::size_t index = 0; index < memberCount; ++index) {
            _collisionObjects[index] = bethesdaAlloc(0x30);
            if (!_collisionObjects[index]) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group collision-object allocation failed at member {}", index);
                if (index == 0) {
                    releaseRefCounted(_physicsSystem);
                    reset();
                    return false;
            ...
            collisionObjectCtor(_collisionObjects[index], static_cast<std::uint32_t>(index), _physicsSystem);
            _niNodes[index] = createAndLinkGeneratedNiNode(_collisionObjects[index], members[index].name ? members[index].name : systemName);
            if (!_niNodes[index]) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group owner-node creation failed at member {}", index);
                destroy(bhkWorld);
                return false;
            }
        }
        ...
        static REL::Relocation<CollisionObjectAddToWorld_t> addToWorld{ REL::Offset(offsets::kFunc_CollisionObject_AddToWorld) };
        for (std::size_t index = 0; index < memberCount; ++index) {
            addToWorld(_collisionObjects[index], bhkWorld);
        }

        static REL::Relocation<PhysicsSystemGetBodyId_t> getBodyId{ REL::Offset(offsets::kFunc_PhysicsSystem_GetBodyId) };
        std::uint32_t sharedMotion = kInvalidGeneratedId;
        for (std::size_t index = 0; index < memberCount; ++index) {
            getBodyId(_physicsSystem, &_bodyIds[index], static_cast<std::int32_t>(index));
            const auto snapshot = havok_runtime::snapshotBody(world, _bodyIds[index]);
            if (_bodyIds[index].value == kInvalidGeneratedId || !snapshot.valid || !isUsableGeneratedMotion(snapshot.motionIndex)) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group member {} failed runtime validation body={} motion={}",
                    index, _bodyIds[index].value, snapshot.motionIndex);
                destroy(bhkWorld);
                return false;
            }
            if (index == 0) {
                sharedMotion = snapshot.motionIndex;
            } else if (snapshot.motionIndex != sharedMotion) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group did not preserve shared motion: member={} expected={} observed={}",
                    index, sharedMotion, snapshot.motionIndex);
                destroy(bhkWorld);
                return false;
            }
            if (havok_runtime::getCollisionObjectFromBody(world, _bodyIds[index]) != _collisionObjects[index]) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group member {} collision-object selector/back-pointer mismatch", index);
                destroy(bhkWorld);
                return false;
            }
            if (!applyGeneratedBodyMaterial(world, _bodyIds[index], members[index].materialId)) {
                destroy(bhkWorld);
                return false;
            }
            if (!havok_runtime::setFilterInfo(world, _bodyIds[index], members[index].filterInfo, 1)) {
                ROCK_LOG_ERROR(BethesdaBody, "Body group member {} initial collision-filter publication failed", index);
                destroy(bhkWorld);
                return false;
            }
            static REL::Relocation<EnableBodyFlags_t> enableFlags{ REL::Offset(offsets::kFunc_EnableBodyFlags) };
            enableFlags(world, _bodyIds[index].value, 0x08020000, 1);
        }
        ...
        activate(world);
        ROCK_LOG_INFO(BethesdaBody, "Created shared-motion body group '{}' members={} motion={}",
            systemName ? systemName : "(null)", memberCount, sharedMotion);
        return true;
    }

    void BethesdaPhysicsBodyGroup::destroy(void* bhkWorld)
    {
        if (_bhkWorld && bhkWorld && _bhkWorld != bhkWorld) {
            ROCK_LOG_ERROR(BethesdaBody, "Body group destroy rejected a mismatched bhkWorld; leaking the stale group rather than removing it from the wrong world");
            reset();
            return;
        }
        auto* instance = nativePhysicsSystemInstance(_physicsSystem);
        void* removalWorld = bhkWorld ? bhkWorld : _bhkWorld;
        if (removalWorld && instance) {
            static REL::Relocation<RemovePhysicsSystem_t> removePhysicsSystem{ REL::Offset(offsets::kFunc_BhkWorld_RemovePhysicsSystemInstance) };
            removePhysicsSystem(removalWorld, instance);
        }
        for (std::size_t index = 0; index < _memberCount; ++index) {
            detachAndReleaseNiNode(_collisionObjects[index], _niNodes[index]);
            if (_collisionObjects[index]) {
                releaseRefCounted(_collisionObjects[index]);
            }
        }
        if (_physicsSystem && _memberCount == 0) {
            releaseRefCounted(_physicsSystem);
        }
        reset();
    }

    void BethesdaPhysicsBodyGroup::retireDeferred(void* bhkWorld)
    {
        if (_memberCount == 0) {
            return;
        }
        if (_bhkWorld && bhkWorld && _bhkWorld != bhkWorld) {
            ROCK_LOG_ERROR(BethesdaBody, "Body group retirement rejected a mismatched bhkWorld; leaking the stale group rather than removing it from the wrong world");
            reset();
            return;
        }
        auto* instance = nativePhysicsSystemInstance(_physicsSystem);
        ...
        RetiredBethesdaPhysicsBodyGroup payload{};
        payload.memberCount = _memberCount;
        payload.remainingPhysicsSteps = kRetiredDeferredBodyGraceSteps;
        payload.collisionObjects = _collisionObjects;
        payload.niNodes = _niNodes;
        reset();

        std::scoped_lock lock(s_retiredGroupMutex);
        for (auto& slot : s_retiredGroups) {
            if (!slot.occupied()) {
                slot = payload;
                return;
            }
        }
        ROCK_LOG_ERROR(BethesdaBody, "Retired body-group queue full; intentionally leaking {} removed collision objects", payload.memberCount);
    }

    void BethesdaPhysicsBodyGroup::abandonAfterWorldLoss()
    {
        if (_memberCount != 0) {
            ROCK_LOG_WARN(BethesdaBody,
                "Abandoning stale shared-motion body group after world loss members={} motion={}",
                _memberCount,
                _sharedMotionIndex);
        }
        reset();
    ...
    void BethesdaPhysicsBodyGroup::serviceRetiredDeferredPayloads(std::uint32_t completedPhysicsSteps)
    {
        if (completedPhysicsSteps == 0) {
            return;
        }
        std::scoped_lock lock(s_retiredGroupMutex);
        for (auto& retired : s_retiredGroups) {
            if (!retired.occupied()) {
                continue;
            }
            retired.remainingPhysicsSteps = retired.remainingPhysicsSteps > completedPhysicsSteps
                ? retired.remainingPhysicsSteps - completedPhysicsSteps
                : 0;
            if (retired.remainingPhysicsSteps == 0) {
                releaseRetiredGroup(retired);
            }
        }
    }

    void BethesdaPhysicsBodyGroup::reset()
    {
        _collisionObjects.fill(nullptr);
        _niNodes.fill(nullptr);
        for (auto& bodyId : _bodyIds) {
            bodyId.value = kInvalidGeneratedId;
        }
        _world = nullptr;
        ...
        _systemData = nullptr;
        _memberCount = 0;
        _sharedMotionIndex = kInvalidGeneratedId;
        _created = false;
    }

    RE::hknpBodyId BethesdaPhysicsBodyGroup::getBodyId(std::size_t index) const
    {
        return index < _memberCount ? _bodyIds[index] : RE::hknpBodyId{ kInvalidGeneratedId };
    }
    ...
    bool BethesdaPhysicsBodyGroup::setMemberTransformDeferred(
        ...
        int mode)
    {
        return belongsToWorld(world) && index < _memberCount && havok_runtime::setBodyTransformDeferred(world, _bodyIds[index].value, transform, mode);
    }

    bool BethesdaPhysicsBodyGroup::driveMemberToKeyFrame(std::size_t index, const RE::hkTransformf& target, float dt)
    {
        if (!isValid() || index >= _memberCount) {
            return false;
        }
        static REL::Relocation<DriveToKeyFrame_t> drive{ REL::Offset(offsets::kFunc_CollisionObject_DriveToKeyFrame) };
        return drive(_collisionObjects[index], &target, dt) != 0;
    }

    bool BethesdaPhysicsBodyGroup::setSharedVelocity(const float* linearVelocity, const float* angularVelocity)
    {
        if (!isValid() || !linearVelocity || !angularVelocity) {
            return false;
        }
        static REL::Relocation<SetVelocity_t> setVelocity{ REL::Offset(offsets::kFunc_CollisionObject_SetVelocity) };
        return setVelocity(_collisionObjects[0], linearVelocity, angularVelocity) != 0;
    }

    bool BethesdaPhysicsBodyGroup::rebuildMassProperties(RE::hknpWorld* world, int rebuildMode)
    {
        return belongsToWorld(world) && isUsableGeneratedMotion(_sharedMotionIndex) &&
            havok_runtime::rebuildMotionMassProperties(world, _sharedMotionIndex, rebuildMode);
    }

    bool BethesdaPhysicsBodyGroup::setCollisionFilterInfo(
        RE::hknpWorld* world,
        std::size_t index,
        std::uint32_t filterInfo,
        std::uint32_t rebuildMode)
    {
        return belongsToWorld(world) && index < _memberCount &&
            havok_runtime::setFilterInfo(world, _bodyIds[index], filterInfo, rebuildMode);
    }

    void BethesdaPhysicsBodyGroup::activate(RE::hknpWorld* world)
    {
        if (belongsToWorld(world)) {
            (void)havok_runtime::activateBody(world, _bodyIds[0].value);
        }
    }

    bool BethesdaPhysicsBodyGroup::Member::isValid() const
    {
        return _owner && _owner->isValid() && _index < _owner->size();
    }

    RE::hknpBodyId BethesdaPhysicsBodyGroup::Member::getBodyId() const
    {
        return _owner ? _owner->getBodyId(_index) : RE::hknpBodyId{ kInvalidGeneratedId };
    }

    void* BethesdaPhysicsBodyGroup::Member::getCollisionObject() const
    {
        return _owner ? _owner->getCollisionObject(_index) : nullptr;
    }

    bool BethesdaPhysicsBodyGroup::Member::setVelocity(const float* linearVelocity, const float* angularVelocity)
    {
        return _owner && _index == 0 && _owner->setSharedVelocity(linearVelocity, angularVelocity);
    }

    bool BethesdaPhysicsBodyGroup::Member::driveToKeyFrame(const RE::hkTransformf& target, float dt)
    {
        return _owner && _owner->driveMemberToKeyFrame(_index, target, dt);
    }
