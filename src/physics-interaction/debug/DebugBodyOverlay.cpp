...
namespace rock::debug
{
    namespace
    {
        ...
        struct PublishedBodyEntry
        {
            ...
            DirectX::XMMATRIX worldMatrix = DirectX::XMMatrixIdentity();
            /*
             * ROCK animated compounds (dynamic hand/weapon) rewrite their
             * child slot transforms every physics substep, so their children
             * publish as separate entries: worldMatrix holds the BODY frame
             * and childLocalMatrix the live slot transform, composed at draw
             * time so applied-transform overrides still land on the body.
             */
            DirectX::XMMATRIX childLocalMatrix = DirectX::XMMatrixIdentity();
        ...
        /*
         * ROCK's animated compounds (dynamic hand / dynamic weapon) rewrite
         * their child slot transforms every physics substep. Tessellating the
         * compound shape bakes those transforms into a cached mesh keyed by a
         * fingerprint of the same transforms, so an animated compound can
         * never produce a stable Ready cache entry — the overlay degrades to
         * the world-AABB proxy box forever. These bodies are instead expanded
         * into one published entry per active child: the child hull shapes
         * are stable (borrowed keyframed twin hulls), so their meshes cache
         * once, and the live slot transform is composed at draw time.
         */
        ...
        std::int32_t captureCompoundChildSlotsUnsafe(
            std::uintptr_t shapeAddress,
            const OverlayRenderSettings& settings,
            std::array<CapturedCompoundChild, kMaxExpandedCompoundChildren>& outChildren)
        {
            try {
                const auto slotArray = *reinterpret_cast<const std::uintptr_t*>(shapeAddress + kCompoundSlotArrayOffset);
                const auto slotCount = *reinterpret_cast<const std::int32_t*>(shapeAddress + kCompoundSlotCountOffset);
                if (!slotArray || slotCount <= 0 ||
                    static_cast<std::uint32_t>(slotCount) > settings.limits.maxCompoundChildren ||
                    static_cast<std::size_t>(slotCount) > outChildren.size()) {
                    return -1;
                }
                ...
                for (std::int32_t index = 0; index < slotCount; ++index) {
                    ...
                    auto& child = outChildren[static_cast<std::size_t>(captured++)];
                    child.shapeAddress = childShapeAddress;
                    std::copy_n(transform, child.transform.size(), child.transform.begin());
                    std::copy_n(childScale, child.scale.size(), child.scale.begin());
                }
                return captured;
            } catch (...) {
                return -1;
            ...
            std::array<CapturedCompoundChild, kMaxExpandedCompoundChildren>* outChildren)
        {
            ...
            __try {
                return captureCompoundChildSlotsUnsafe(shapeAddress, *settings, *outChildren);
            } __except (EXCEPTION_EXECUTE_HANDLER) {
                return -1;
            }
        }

        DirectX::XMMATRIX compoundChildLocalMatrix(const CapturedCompoundChild& child)
        {
            // Same math the baked path used per vertex: scale, rotate by the
            // column-major slot basis, translate in game units.
            return DirectX::XMMatrixMultiply(
                DirectX::XMMatrixScaling(child.scale[0], child.scale[1], child.scale[2]),
        ...
        CapturedShapeIdentity captureShapeIdentityForFrame(PublishedOverlayFrame& frame, std::uintptr_t shapeAddress)
        {
            ...
            const CapturedShapeIdentity captured{ shapeAddress, key, detailUniformScale, shapeType };
        ...
        bool buildPublishedFrame(const BodyOverlayFrame& source, PublishedOverlayFrame& destination)
        {
            ...
            for (std::uint32_t index = 0; index < bodyCount; ++index) {
                ...
                if (rockRole && (shapeIdentity.shapeType == 7 || shapeIdentity.shapeType == 8)) {
                    /*
                     * ROCK animated compound: publish one entry per active
                     * child with the live slot transform instead of baking a
                     * per-frame mesh of the whole compound (see
                     * CapturedCompoundChild above). Falls through to the
                     * legacy single-entry path only if the slots are
                     * unreadable.
                     */
                    std::array<CapturedCompoundChild, kMaxExpandedCompoundChildren> children{};
            ...
            const AppliedGeneratedBodyTransformFrame* appliedTransforms)
        {
            ...
            appendAxisTripod(batch, origin, transformPoint(worldMatrix, length, 0.0f, 0.0f),
                transformPoint(worldMatrix, 0.0f, length, 0.0f),
                transformPoint(worldMatrix, 0.0f, 0.0f, length), entry.role);
            ...
            const AppliedGeneratedBodyTransformFrame* appliedTransforms)
        {
            ...
            for (const auto& published : frame.axes) {
                if (published.entry.source == AxisOverlaySource::Body) {
                    collectBodyAxisEntry(batch, published, appliedTransforms);
                } else {
            ...
            OverlayRuntimeStats& stats)
        {
            ...
            for (const auto& entry : frame.bodies) {
                ...
                if (entry.hasChildLocalMatrix) {
                    // Expanded animated-compound child: the live slot
                    // transform composes onto the (possibly applied) BODY
                    // frame so physics-fresh overrides still land correctly.
                    model = DirectX::XMMatrixMultiply(entry.childLocalMatrix, model);
                }
        ...
        void drawOverlayToSubmittedTexture(const vr::Texture_t* texture)
        {
...

            {
                ...
                if (hasBodiesToDraw) {
                    drawBodyBatch(context, *frame, appliedTransforms, stats);
                }
                ...
                collectAxisOverlays(lineBatch, *frame, appliedTransforms);
    ...
    void PublishFrame(const BodyOverlayFrame& frame)
    {
        ...
        auto publicationSequence =
            s_nextPublicationSequence.fetch_add(1, std::memory_order_relaxed) + 1;
        ...
        RE::hknpWorld* world) noexcept
    {
        ...
        next->count = 0;
        next->worldIdentity = source->worldIdentity;
