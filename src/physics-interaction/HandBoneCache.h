...
namespace frik::rock
{
	...
	public:
		bool resolve()
		{
			...
			if (changed) {
				ROCK_LOG_INFO(Hand, "HandBoneCache resolved fpSkeleton={:p} RArm_Hand={:p} LArm_Hand={:p}",
					static_cast<void*>(_skeleton), static_cast<void*>(_rightHand), static_cast<void*>(_leftHand));
			}
		...
		{
			return _ready && _skeleton && _rightHand && _leftHand;
		}
		...
		[[nodiscard]] RE::NiNode* getNode(bool isLeft) const
		{
