...
Implemented after the Ghidra pass:

- Re-enabled the custom grab ragdoll angular atom:
  - `setGrabMotorAtomsActive(header, true, true)`;
  - creation log now reports `angularAuthority=ragdollAtom`.
- Split angular frame writes:
  - transform A rotation is proxy-body-local raw hand/controller authority;
  - transform B rotation is creation-time body-B local angular basis;
  - `target_bRca` is updated through the FO4VR-native
    `bRa * transformA.rotation` equation.
- Updated held flush:
  - proxy body still comes from the generated palm collider drive;
  - transform A is written against the proxy readback frame when available;
  - raw controller / flattened-root hand rotation remains angular authority;
  - transform B translation still uses the selected body-local grip pivot.
- Removed production direct held-object angular velocity:
  - removed `applyProxyConstraintAngularVelocityDrive(...)`;
  - removed `setHeldAngularVelocity(...)`;
  - removed the native hard-keyframe held-body angular target adapter.
- Long-object handling was moved from direct angular speed cap semantics to
  angular motor authority scaling:
  - `computeLongObjectAngularAuthorityScale(...)`;
  - initial and per-frame angular motor max force are scaled by the captured
    grip-to-mesh lever length.
- Updated source-boundary and C++ policy tests to enforce the new single-writer
  angular authority.

Validation run after implementation:

- `powershell -ExecutionPolicy Bypass -File tests\GrabAuthorityProxyFrameSourceTests.ps1`
  - passed.
- `powershell -ExecutionPolicy Bypass -File tests\HandGrabNativeBoundarySourceTests.ps1`
  - passed.
...
- `cmake --build build-fast --config Release --target ROCK -- /m`
  - passed and deployed `ROCK.dll`/`ROCK.pdb` to
